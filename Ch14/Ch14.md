...
>
> Since different servers can use different keys, you might want to create the new key at a new server, then tell
...
So far we talked primarily about the server master key, but the database encryption key also deserve some attention. 
...
setting up the database. The database creation wizard make this process seamless, but it is important to understand what
...
> allow you to execute arbitrary scripts in the context of RavenDB itself. The same functionality is also exposed via
...
keys. We'll speak more about backup later in this book, but there are some things about backing up encrypted database that 
...
* Snapshot - compressed binary snapshot of the database file at a given point in time, as well as additional data from the 
  cluster level that belongs to the database (identities, compare exchange values, etc).
* Full backup - compressed json of all the data in the database, as well as the cluster level data.
* Incremental backup - compress json of all the data in the database since the last full backup, as well as teh cluster 
  level data.

For encrypted database, it is important to consider what parts of the backup are encrypted, and in what manner. With a 
...
Alongside the snatshop data, there is also the cluster level data. This is typically much smaller, but this is _not_ 
...
This chapter may be hard to dechiper, but I hope you got the right keys from it. In a more serious tone, we have gone
over a lot of information about how RavenDB is using high end encryption to safely protect your data. 
RavenDB use the `XChaCha20Poly1305` algorithm to encrypt any and all data on disk, and will only decrypt information 
during an active transaction. RavenDB will also immediately wipe the decrypted contents in memory on transaction closure
reducing the time that sensitive infomration is avaialble. During this time, the memory holding the decrypted data is 
...
Finally, we discussed backup concerns for encrypted databases, in particular, the safe keeping of the encryption keys.
I find it ironic that what is probably the most secured method is printing the encryption key and then storing it offline
in a locked cupboard at your offices, but I've found it to be one of the most efficent ways to handle such an issue. Having
...
of a password reset invalidating a `DPAPI` encrypted value, losing the main harddisk of the machine, but still having the
...
data. That is the _point_ of encryption. So please remember that when you create encrypted database, keep your encryption
