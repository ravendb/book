...
official government ID. Figure 14.1 might make this easier to understand.

![Self signed vs. CA certificates, in real world terms](./Ch14/img01.png)
...
you will be asked to make important decisions about how RavenDB will be deployed. You can see what this looks like in Figure 14.2. 

![The setup wizard is the first screen you'll see when accessing RavenDB for the first time](./Ch14/img02.png)
...
You should see a screen similar to the one in Figure 14.2. 
...
get a free license by registering on the RavenDB website) and click next. You can see the next screen in Figure 14.3.

![The domain selection page allows you to select the name of your domain.](./Ch14/img03.png)

The domain selection screen in Figure 14.3 shows the screen where you'll select your domain name. This will also be how you'll typically refer to
...
You'll be faced with the cluster setup, as shown in Figure 14.4. We're setting a local cluster, so set the `HTTP Port` to `443` and
...
that you will run this exercise on your own machine to start with. You can see what this should look like in Figure 14.4. 

![Setting up a cluster of three machines, including setting of DNS records](./Ch14/img04.png)

If you look at the left portion of Figure 14.4, you'll see a list of nodes (`A`, `B` and `C`). As we have already seen in 
Chapter 7, we usually use the letters of the alphabet to name the nodes in the cluster. Click on `Add node` 
...
be saved carefully. You can see the contents of this file in Figure 14.5.

![Contents of the setup `.zip` file with everything you need to get your cluster started.](./Ch14/img05.png)
...
and a README file with additional setup instructions. In Figure 14.5, you can also see that there are three directories there: `A`,`B` 
...
in Figure 14.6 and you should select the appropriate certificate (if you have more than one).^[If you make a mistake and choose the wrong certificate (or cancel the dialog), you'll need to close Chrome and restart it to make it forget this 
decision. Creating an incognito window also works, and might be easier.]

![Chrome certificate selection dialog for your RavenDB instance](./Ch14/img06.png)
...
what permissions are available to you. You can see an example of this in Figure 14.7.

![The "Who am I" feature gives you details about how you authenticated to the server and the permissions you've been granted.](./Ch14/img07.png)
...
`Continue cluster setup for new node`, as you can see in Figure 14.2. Click on that button and then upload the `.zip` file that you 
got from the first node, select node `B` in the drop-down, and then click `next`.  You can see what this looks like in Figure 14.8.

![Completing the setup process for the second node in the cluster](./Ch14/img08.png)
...
define certificates through an API, so this is easily automated.) You can see what this looks like in Figure 14.9.

![Client certificate screen in the RavenDB Studio](./Ch14/img09.png)
...
![Generate a new client certificate and assign permissions to it.](./Ch14/img10.png)

In Figure 14.10, you can see the creation of a new certificate, `uat.northwind`, and that we have granted it read/write access to
...
>
> If you look closely at Figure 14.9, you'll see that there is another clearance level that I didn't mention: 
...
manually with your own certificate using the `Replace cluster certificate` button, as shown in Figure 14.9.
...
In Figure 14.9, you can see the `Export cluster certificates` button. Clicking this button will download a `.pfx` file
...
You can see an example of how this can be done in Listing 14.1. 
...
process and read the certificate from the standard output. Listing 14.2 shows the implementation of the 
`get-cert-by-id.ps1`.
