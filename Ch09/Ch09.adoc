...
You might be familiar with the term "murder of crows" as a way to refer to a group of crows.footnote:[If you're interested in learning why, I found this answer fascinating: https://www.quora.com/Why-is-a-group-of-crows-called-a-murder] It's
...
A RavenDB cluster is three or more machinesfootnote:[It doesn't make a lot of sense to have a cluster with just two nodes since we'll require both of the nodes to always be up, in most cases.
There are certain advanced scenarios where such topology might make sense, and we'll touch on that briefly in <<cloud-n-edge>>.] that have been joined together.
...
the https://raft.github.io/[Raft consensus protocol] and having the cluster members vote to select a strong leader
among themselves.
...
in the presence of a partition), but the database layer is AP by default (it's always available, even if there's a partition, but
it's eventually consistent). However, you can opt into CP behavior at the database layer on a per-transaction basis using
cluster-wide transactions.

What makes this particularly powerful is that RavenDB lets you choose, on a _per-transaction_ basis, which tradeoff you
want. Normal writes go through the AP database layer -- they're fast, always available and eventually consistent. But when
you have a high-value operation that demands consistency (such as ensuring a unique constraint or coordinating critical state),
you can use a cluster-wide transaction and get the CP guarantees of the cluster layer for that specific operation.

This means you don't have to make a single, global decision about consistency vs. availability for your entire application.
The vast majority of your operations can enjoy the speed and availability of the AP layer, while the few operations that
truly need it can pay the extra cost of a cluster-wide consensus round trip. You get to make that choice based on the
business value of each operation, not as a one-size-fits-all architectural decision.
****

The cluster layer uses consensus, so if a majority of the nodes aren't available, cluster-wide operations can't proceed.
Another issue with consensus algorithms is that they incur additional network round trips for each operation.
For cluster maintenance and the configuration of databases, RavenDB uses the Raft consensus protocol. The RavenDB
implementation of Raft is codenamed Rachis.footnote:[Rachis is the central shaft of pennaceous feathers.]

Databases operations, on the other hand, are treated quite differently by RavenDB. Each node in the cluster has a full 
copy of the topology, which specifies which nodes host which databases. That information is managed by Rachis, but each 
node is able to act upon it independently.
...
A write to any of those nodes will be automatically (and asynchronously) replicated to all the other nodes.
...
The design of RavenDB was heavily influenced by the http://dl.acm.org/citation.cfm?id=1294281[Dynamo paper], and one of
...
.Downloading RavenDB
****
In <<welcome-to-ravendb>>, we went over how to download and set up RavenDB for various environments.
If you skipped that step, you can go to the RavenDB download page at
https://ravendb.net/download and download the package for your platform.
...
it "Spade." The result should look like <<simple-db-node>>.

[[simple-db-node]]
.A simple database on a single node
image::./Ch09/img01.png[]
...
Another difference between the two nodes can be seen at the bottom right, as shown in <<node-tags>>.

[[node-tags]]
.The node tag in two nodes
image::./Ch09/img02.png[]
...
cluster management screen is shown in <<single-node-cluster>>.

[[single-node-cluster]]
.A single node cluster in the management screen
image::./Ch09/img03.png[]

Click `Add Node to Cluster` and enter `http://127.0.0.2:8080`. The new node will show up as a cluster member, as shown in <<two-node-cluster>>.

[[two-node-cluster]]
.Our cluster after adding the second node
image::./Ch09/img04.png[]
...
Now, you can re-start the first node by using the following command:
...
The node will start and reconnect into the cluster. If you check the cluster state, you'll see something similar to <<leader-after-recovery>>.

[[leader-after-recovery]]
.New leader selected after recovery of failed node
image::./Ch09/img05.png[]
...
a backup task with the cluster down,footnote:[Of course, we'll be able to create a backup manually. See the discussion about cluster-wide tasks later in this chapter.] and you'll not be able to deploy new indexes or modify database configurations.
...
Normal read/write/query operations from each database instance will proceed normally, and your applications should failover automatically and immediately. On the other hand, background operations, such as subscriptions or ETL, won't be able
to proceed until the cluster is back up, which requires a majority of the nodes to be able to talk to one another.

Cluster-wide transactions are also unavailable when the cluster is down. 
The entire _point_ of a cluster-wide transaction is to prefer consistency over availability, and that guarantee is only meaningful when a majority of the nodes can participate in the consensus.
If you need writes to go through regardless of the cluster state, use normal (database-level) transactions,
they'll keep working as long as at least one node is up & running.
****
...
open the Studio on node `D`, as shown in <<spade-from-node-d>>.

[[spade-from-node-d]]
.The Spade database as seen from node `D`
image::./Ch09/img08.png[]
...
Now, let's actually use our cluster and create a new database. We'll call it "Pitchfork."footnote:[I'm on a garden tools naming streak, it appears.] Everyone knows that a proper pitchfork has three tines; the four-tine pitchfork is used for shoveling, while
...
nodes of its choice. This means we now have a five-node cluster with two databases, as you can see in <<spade-and-pitchfork>>.

[[spade-and-pitchfork]]
.The Spade and Pitchfork databases in our RavenDB cluster
image::./Ch09/img09.png[]

<<spade-and-pitchfork>> shows the databases from the Studio in node `D`. So we have Pitchfork on three nodes and Spade on two. You can go ahead and
...
consider the situation in <<garden-shed-databases>>.

[[garden-shed-databases]]
.A cluster hosting a whole garden shed of databases
image::./Ch09/img10.png[]

In <<garden-shed-databases>>, you can see that we created eight databases and that they're all spread out throughout the cluster. This means
...
discuss this feature in <<cloud-n-edge>>, which covers external
replication.
****
...
and you ought to read about proper deployment and security of your cluster in <<securing-your-ravens>>, before you
...
* *Database instance* -- exists on a single node, usually part of a larger database group. Typically referred to as "the database instance
  on node `A`."
* *Database group* -- the grouping of all the different instances, typically used to explicitly refer to its distributed nature. For example, "The
...
* *Database* -- the named database we're talking about, regardless of whether we're speaking about a specific instance or the whole group. For example, "We use the 'Spade' database to store that information."
...
cooperating to get to the best result. We'll start our exploration with the code in <<cluster-document-store>>.

[[cluster-document-store]]
.Creating a document store to talk to a cluster
[source,csharp]
----
...
The code in <<cluster-document-store>> lists all the nodes in our cluster and will talk with the "Spade" database. This is interesting because
the "Spade" database only exists on nodes B and A. (You can see the database topology in <<garden-shed-databases>>.) Why are we listing all the nodes
in this manner?
...
topology from the cluster, and the cluster ensures that we'll always report the same topology to the clients.footnote:[Of course, it's possible that a client has an _outdated view_ of the topology, but there are mechanisms in place to ensure that clients will figure out that their
...
From the client's point of view, there isn't really much to be said about the cluster. As far as your code is concerned, you
...
do that using write assurance, which is available using the `WaitForReplicationAfterSaveChanges` method. An example of that is shown in <<write-assurance-example>>.


[[write-assurance-example]]
.Saving a very important task to RavenDB, ensuring it resides in multiple nodes
[source,csharp]
----
...
The code in <<write-assurance-example>> should look familiar. There isn't much to change when you move from a single node to a cluster. But here we are
...
wrote it to _some_ of them. In the case of <<write-assurance-example>> and the "Spade" database, if node `A` is down, we'll be able to write to node `B`. But
...
[[client-read-balancing]]
.Configuring client read balancing behavior from the server side
image::./Ch09/img11.png[]

The system administrator can configure such behavior globally, as shown in <<client-read-balancing>>, or the client can specify that using the conventions.
...
diagnosing issues. You can see an example of that in <<replication-with-node-down>>.

[[replication-with-node-down]]
.A database group with three nodes, one of which is down
image::./Ch09/img12.png[]
...
In <<replication-with-node-down>>, you can see that node C is down. The cluster has detected it and designated node B as the mentor node
...
several times in short order -- fast enough that it was modified several times before the document could be sent to the other database
...
end up in the same _order_. It's possible that revisions created on different servers will show up in a different
...
Consider for a moment the code in <<bank-transfer>>. It's a simple bank transaction that moves $10 from my account to yoursfootnote:[This isn't even close to how money transfers really work, of course, but it's a classic example and easy to reason about.], which I'm sure will be a
delightful surprise.

[[bank-transfer]]
.The classic bank transaction example
[source,csharp]
----
using (var session = store.OpenSession())
{
...
There's nothing very interesting about <<bank-transfer>> in itself. The interesting bit is what will happen over the network, in this case.
...
What RavenDB guarantees is that writes made in the same transactions will always be sent to the other database instances in a single
...
the other database instances as a single transaction. There's one caveat, however. Consider the code in <<bank-transfer-tax>>, which transfers money from your account to the tax authority.


[[bank-transfer-tax]]
.The bank transaction example -- paying the taxman
[source,csharp]
----
using (var session = store.OpenSession())
{
...
The code in <<bank-transfer-tax>> will probably sadden you a bit since no one usually likes to pay the taxman. However, it also exposes an issue with
...
Let's assume that we now need to replicate everything from <<bank-transfer>> onward (meaning my transfer of money to you and your payment of
taxes).
...
generated for <<bank-transfer>>, we'll replicate `accounts/4321-C` but _not_ `accounts/1234-A`. Why is that?

The `accounts/1234-A` document will not be replicated because, though it was changed in the transaction for <<bank-transfer>>, it
was also changed by a later transaction (the one from <<bank-transfer-tax>>). Since we replicate only the current state of the documents
...
modified in <<bank-transfer>> -- not documents that were modified in the transaction for <<bank-transfer>> and then later modified by
another transaction.
...
transaction in <<bank-transfer>> as it was so that a client observing a second node can never see a partial transaction.
...
isolated, independent and coherent tenets, which usually means you don't _need_ this feature. But when you do, and most certainly it will
 come up,footnote:[It's not something you'll use for everything, but in most applications there's one or two places where it can be _really_ useful.] the key here is that you aren't paying for tracking all the intermediate values unless you actually need this.
...
A third option, which we already explored in <<document-crud>>, is to use cluster-wide transactions. By opening a session with
`TransactionMode.ClusterWide`, the entire transaction goes through the Raft consensus protocol, ensuring that either all changes are
accepted by the cluster or none are. This gives you the strongest guarantee -- full ACID across the cluster -- but at the cost of
requiring a majority of nodes to be available and additional latency for the consensus round trips. Cluster-wide transactions are
best reserved for cases where you absolutely must have consistency across the cluster, such as ensuring unique constraints or
coordinating state that cannot tolerate conflicts.
...
node, you can see that the database instance also includes the change vector from the other instance as well. Take a look at this example in <<change-vector-example>>.

[[change-vector-example]]
.A document change vector in a cluster
image::./Ch09/img13.png[]
...
The question is this -- how are we going to handle such a case? Let's generate a conflict manually and see how this is handled.
...
recent version.footnote:[Note that different nodes may have different clocks. RavenDB doesn't attempt to compensate for this and will use
the latest timestamp from the conflicts in its automatic resolution.] You can see
what this looks like in <<conflict-resolution-settings>>.

[[conflict-resolution-settings]]
.Configuring server side conflict resolution policy in the Studio
image::./Ch09/img14.png[]
...
so we'll take the heavy-handed approach and reproduce a conflict by shutting down nodes and modifying
...
* Modify the `users/1` document on node `A` so its name property will be "John," and save the document. You can see how this will look in <<conflict-version-a>>.
...
./Server/Raven.Server.exe `
    ...
    --DataDir=Data/B
----
* Modify the `users/1` document on node `B` so its name property will be "Doe", and save the document. You can see how this will look in <<conflict-version-b>>.
...
[[conflict-version-a]]
.First version of `users/1` document after modifications on node `A`
image::./Ch09/img15.png[]

[[conflict-version-b]]
.Second version of `users/1` document after modifications on node `B`
image::./Ch09/img16.png[]
...
In fact, you'll see _two_ versions of this document, as displayed in <<manual-conflict-resolution>>. For developers, the screen shown may look very familiar. It's
...
because we had a network hiccup. Consider the documents shown in <<shopping-cart-conflict>>.

[[shopping-cart-conflict]]
.Two conflicting versions of a shopping cart
[source,json]
----
// on node A
{
...
view that we looked at in <<conflict-resolution-settings>>. Click on the `Add` button to add a conflict resolution script for a particular
collection. <<merge-shopping-carts>> has the code to properly resolve such a conflict on the `ShoppingCart` collection.

[[merge-shopping-carts]]
.Script to merge conflicting shopping carts
[source,javascript]
----
var final = docs[0];
...
The code in <<merge-shopping-carts>> will merge the two different versions of the shopping cart, resulting in the document shown in <<merged-shopping-cart>>.
With a merge script such as the one in <<merge-shopping-carts>>, RavenDB can automatically resolve conflicts as they happen, with no service interruption.
The cart in <<merged-shopping-cart>> has been intelligently merged, so we don't have any duplicate products and we kept the higher number of milk
bottles between the versions of the cart that we merged.

[[merged-shopping-cart]]
.The merged shopping cart
[source,json]
----
{
    "Name": "Oren Eini",
...
In <<document-crud>>, I introduced the compare exchange feature (sometimes known as `cmpxchg`). This feature allows you to perform atomic
...
Consider the code in <<compare-exchange-unique>>,footnote:[The same code was already shown in <<document-crud>>.] which shows how to use this feature
to ensure unique username reservation at the cluster level.

[[compare-exchange-unique]]
...
for an operation to succeed. With 5 nodes in the cluster, you'll need 3, etc. See <<cloud-n-edge>> for full details on this.
...
<<register-engineer>> shows how an engineer can register to the pool of available engineers.

[[register-engineer]]
.Using compare exchange to register engineer's availability in a consistent manner
[source,csharp]
----
...
The code in <<register-engineer>> is very similar to how you would write multi-threaded code. You first get the value, then attempt
...
would look in <<compare-exchange-studio>>.

[[compare-exchange-studio]]
.The available engineers compare exchange value in the Studio
image::./Ch09/img18.png[]
...
The code in <<register-engineer>> is only doing half of the job. We still need to be able to get an engineer to help us handle
a support call. The code to complete this task is shown in <<pull-engineer>>.

[[pull-engineer]]
.Getting an available engineer using atomic and distributed compare exchange operations
[source,csharp]
----
const string key = "engineers/available";
...
The code in <<pull-engineer>> is a bit more complex. Here we read the available engineers from the server. If there are none, we'll
...
Note that in <<pull-engineer>> we use two different modes to update the value. If there are still more engineers in the available
...
<<pull-engineer>>, even if we have an available engineer, we'll not use that knowledge until we successfully wrote the new value.
