...
Go to `Settings` and then to `Connection Strings` and create a new RavenDB connection string. As you can see in Figure 9.1, I've defined a connection string to the `helpdesk` database in the live-test instance.

![Defining a connection string to another RavenDB instance.](./Ch09/img01.png)
...
`Manage Ongoing Tasks`. Click the `Add Task` button and then select `RavenDB ETL`. You can see how this looks in Figure 9.2.
Give it a name and select the previously defined connection string.

> **What about security?**
>
> We'll cover security in depth in Chapter 14, but given that we've shown a connection string, I wanted
...
![Defining an ETL process to another RavenDB instance](./Ch09/img02.png)
...
then on `Save`. The result should look like Figure 9.3.

![Defining a simple "copy the whole collection" ETL script](./Ch09/img03.png)
...
You can see what this looks like in Figure 9.4.

![Using ETL with a script to redact the results](./Ch09/img04.png)
...
However, when you provide your own script, you need to take responsibility for this yourself. Listing 9.1 shows a slightly more complex example.
...
The script in Listing 9.1 will send the employees, their title, their birth year and their manager over to the other side. You can see that the script is 
...
> Figure 9.5, to let RavenDB know that it needs to start the ETL process for this script from scratch, rather than apply the update only 
> to new or updated documents.
> 
> ![Resetting the ETL process after a script update](./Ch09/img05.png)

Looking on the other side, you'll be able to see the ETL'ed document, as shown in Listing 9.2.
...
There are a few interesting things in the document in Figure 9.2. First, we can see that it has only a single change vector entry (for the destination
...
Another interesting ETL example is when we want to push multiple values out of a single document, as shown in Listing 9.3.
...
The results of the script in Listing 9.3 can be seen in Figure 9.6. You can see that the `Employees` documents were sent, but there are also the addresses documents. 
For those, we use the prefix of the source document to be able to identify them.

![Multiple outputs from a single source document on the destination](./Ch09/img06.png)
...
Listing 9.1 is a good example of exposing just enough information to be useful to the other side without leaking implementation details or other aspects
...
parties. We'll discuss authorization in detail in Chapter 14, but for now, know that RavenDB allows you to define permissions at the
database level.
...
back to the server or to have each POS system contain all the sales across the system. Figure 9.7 shows the data flow between the various components of 
the system in such a scenario.

![Data flow diagram for a POS system with replication and ETL processes.](./Ch09/img07.png)
...
in Figure 9.1 was to add the URL of the destination. In this case, we have just one server. But if we have a cluster on the other side, we can list
...
with the appropriate table. You can see the table creation script in Listing 9.4.
...
a new SQL connection string, as shown in Figure 9.8. You'll need to provide your own connection string. Note that you can test that the connection string is
valid directly from the Studio. 

![Creating a new SQL connection string](./Ch09/img08.png)

> **Note the identity issue**
>
> In Figure 9.8, you can see a SQL connection string that uses `Integrated Security=true`. This is possible because the user that 
...
table is going to be used as the document ID column. Note that this doesn't have to be the primary key of the table in question. It just needs to be a column RavenDB can use to place its document ID in. You can see such a configuration in Figure 9.9.

![Specifying a table to be used in the ETL process](./Ch09/img09.png)

Select the `Employees` collection on the transformation script on the right and use the script from Listing 9.5.
...
`select * from Employees` in Figure 9.10.

![Result of SQL ETL script on the relational database side](./Ch09/img10.png)
...
The example in Listing 9.5 is pretty simple. We have a one-to-one match between the documents and the rows that represent them. We did have a bit of fun with
...
represent that in the SQL destination. We'll start by first defining the tables that we need on the receiving end, as shown in Listing 9.6.
...
Once we've created the tables in Listing 9.6, we can proceed to actually define the ETL process for orders. First, we need to add the new tables, as you can see
in Figure 9.11. 

![Adding additional tables to the SQL ETL process](./Ch09/img11.png)
...
to the relational database, these are `loadToOrderLines` and `loadToOrders`, as you can see in Listing 9.7.
...
The script in Listing 9.7 iterates over the `Lines` in the order, telling RavenDB that we need to send the object to that table as well as compute a running total
...
You might have noticed in Listing 9.7 and Listing 9.5 that we didn't specify the `OrderId` or `DocumentId` columns. That's because we implicitly define those when we set
...
If you look at the tables creation script in Listing 9.6, you'll note that we explicitly handled that. In the `Orders` table, the `OrderId` field is marked
...
Another type of error is when we fail to complete the ETL process for a single statement. For example, in Listing 9.5, we're sending a `Name` field that is 
...
table creation script in Listing 9.4), we'll not be able to process that document. In such a case, when there is an error that pertains only to a particular
...
characters and saving. RavenDB will attempt to send the update over but will fail, and you'll get the error shown in Figure 9.12.

![Showing SQL ETL errors in the Studio](./Ch09/img12.png)
