...
In Listing 9.11, we use the same `productQueryFacets` value as we used in Listing 9.9. We are passing a `SearchQuery` class, which holds the options for the search options that the user has made.
...
```{caption="{Adding the results of the facets query to the result we send to the user}" .cs}
