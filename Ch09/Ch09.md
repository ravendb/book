...
For example, an order without its order lines may not be very meaningful in a business sense to the user. But knowing that an order was made on Dec
...
Spatial searches allow you to search using geographical data. We'll explore them in more depth in the chapter about [static indexes](#static-indexes), 
...
that information to query for nearby employees that can pick you up if you come through the Seattle airport. Listing 9.21 shows just how to tickle
...
The query in Listing 9.21 results in two matches: Nancy and Andrew. It's pretty simple to understand what the query is doing, and I wish most spatial
...
The actual polygon in Listing 9.22 is unlikely to be a very accurate representation of the London Orbital (I drew it by hand, and quite roughly,
...
operation using the basic building blocks that RavenDB provides. We'll touch more on spatial queries in the [static indexes](#static-indexes) 
chapter, and they're covered in detail in the online documentation. 
