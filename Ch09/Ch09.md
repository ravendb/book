...
index and returning all the results smaller than the given date. That's cheap to do and the proper way to run such queries. 
...
you can't just modify the query. You need to tell RavenDB that it needs to do some computation during the indexing process. We'll see how that's done in the chapter about [static indexes](#static-indexes).
