...
range. `BETWEEN` is inclusive on the low end and exclusive on the high end. In other words, consider the query in Listing 9.3. 
...
The results of the query in Listing 9.3 are going to include employees hired in 1992, but _not_ those hired in 1994. Another curious thing you 
...
On the other hand, you could have written the query as shown in Listing 9.4 and find only the results that match the value and the casing used.
...
Listing 9.5 for the full query.
...
The results of the query in Listing 9.5 are really interesting. First, we have six companies that are located in London and then two which are 
...
for Lucene, which means that our Lucene indexes are also properly ACID and without giving up on the expected performnace. In general, all the operational
...
allow to find matches, all else, like sorting of projections is handled via code. This make it a great tool for finding information, but a poor one for
...
Consider the query in Listing 9.6, which express a fairly complex conditional and ranking requirement using `Boost()`
...
The query in Listing 9.6 will select companies based in London, Paris or anywhere in Germany or Sweden. The effect of `Boost()` on the results is that a 
...
from the server and use that.

The simplest query projection can be seen in Listing 9.7.
...
The query in Listing 9.7 will produce the results with three fields, Name, City and Country. A single projection form this query is shown in Figure 9.8.
...
In the case of the query in Listing 9.7 we are only projecting simple fields, but RQL is capable of much more. Listing 9.8 has a more complex projection 
...
back and rename it using aliases. RavenDB is a JSON database as smart as the `select` is, it is best for dealing with mostly flat data. That is why we 
have the ability to project object literals.

#### Projecting with object literals

With object literals projection, you can take full control over how RavenDB will 
