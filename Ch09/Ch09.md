...
The query in Listing 9.20 is very nearly the same exact query as the one in Listing 9.21. However, the sort order is different. Why does this matter? It matters because, in both cases, we used the same index: "Auto/Employees/ByAddress.CountryAndFirstNameAndLastName," in this case. This is one of those
...
to answer the query in Listing 9.20, but it isn't in Listing 9.21. RavenDB, however, doesn't use such single purpose indexes. It indexes each field independently. That means it's able to answer both queries (and any combination thereof) using a single index.
...
The `as double` in Listing 9.22 instructs RavenDB to sort the results as doubles. You can also use `as long` to specify that the data should
...
that information to query for nearby employees that can pick you up if you come through the Seattle airport. Listing 9.23 shows just how to tickle
...
The query in Listing 9.24 results in two matches: Nancy and Andrew. It's pretty simple to understand what the query is doing, and I wish most spatial
...
You can see the result in Listing 9.24.
...
The actual polygon in Listing 9.24 is unlikely to be a very accurate representation of the London Orbital (I drew it by hand, and quite roughly,
