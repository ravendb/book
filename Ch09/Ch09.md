...
If you click `View index terms` link, you'll be taken to the index terms page, where you'll see the index fields. Clicking this will show you what was
...
Spatial searches allow you to search using geographical data. We'll explore them in more depth in the chapter about [static indexes](#static-indexes) but
we can do quite a lot with spatial queries without any special preparation. Spatial queries requier that you'll have spatail data, of course, such as
...
Sptial queries are expressed using the following basic spatial operations: `spatial.within()`, `spatial.intersects()`, `spatial.contains()` and  `spatial.disjoint()`.
You can read more about the kind of spatial queries that are supported by RavenDB in the online documentation. For now, let's have some fun. The spatial
coordinates of the Seattle-Tacoma International Airport (SEA) are latitude 47.448 and longitude -122.309. We can use that information to query for nearby
employees that can pick you up if you come through the Seattle airport. Listing 9.21 shows just how to tickle RavenDB to divulge this information.	
...
The query in Listing 9.21 results in two matches, Nancy and Andrew. It is pretty simple to understand what the query is doing, and I wish that most
spatial queries were that simple. Unfortunatley, in the real world we have to deal with much more complex systems. A good example of that would be the 
London Orbital motorway (M25). It is _sort of_ a circle that surrounds London. I drew it out on a map as a polygon and then asked RavenDB to find all
...
You can see the result in Listing 9.22.
...
The actual polygon in Listing 9.22 is unlikely to be very accurate representation of the London Orbital (I drew it by hand and quite roughly, too) but
it is a good demonstrations of RavenDB spatial query capabilities. As it turns out, out of the employees in our data set, only Steven lives inside 
London. 

I find that pretty much the only way to reason about spatial queries is to draw them out on a map, otherwise I'm completely lost. Typically when
you have the need for spatial queries you also have pretty good idea about what you actually need and can translate that need into the appropriate
operation using the basic building blocks that RavenDB provides. We'll touch more on spatial queries in the [static indexes](#static-indexes) chapter
and they are covered in detail in the online documentation. 
...
completes the operation. Instead, it writes the document data and complete the write operation as soon as the transaction is written to disk, scheduling 
...
> There](https://sfu-db.github.io/dbsystems/Lectures/OLTP_Through_the_Looking_Glass.pdf)" by Stravos Harizopoulous, Daniel Abadi, 
Samuel Madden, Micheal Stonebraker.] shows that they spend over 30% of their time just managing locks of various kinds. This is a 
