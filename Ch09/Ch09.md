...
If you click that link, you'll be taken to the index terms page, where you'll see the index fields. Clicking this will show you what was
...
intersection instead of a union (which we use for `or`). The result of that will be (`employees/2`,`employees/8`), which appear on both 
...
In the case of RavenDB, a major factor in our Lucene usage is that we're able to have our storage engine (Voron) provide transactional guarantees, which means our Lucene indexes are also properly ACID and safe from corruption without us needing to sacrifice expected performance. In general, all the operational aspects of running Lucene indexes are handled for you, and they shouldn't really concern you. We'll discuss Lucene in more depth in the ["Static Indexes"](#static-indexes) chapter. But for now, I want to focus on the querying capabilities that Lucene provides.
...
The query in Listing 9.8 will produce the results with three fields: name, city and country. A single simple projection of fields and the use of aliases from this query is demonstrated in Figure 9.8. You can see that we didn't specify an alias for the `Address.City` field and that the
...
The `as double` in Listing 9.20 instructs RavenDB to sort the results as doubles. You can also use `as long` to specify that the data should
