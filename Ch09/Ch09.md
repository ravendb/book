...
> queries and looking at documents. We'll look at code to consume such queries from the client API in the next chpater.
...
select {
    TopProducts: o.Lines
        .sort((a, b) => 
        	(b.PricePerUnit * b.Quantity) - (a.PricePerUnit * a.Quantity) 
        )
        .map(x=>x.ProductName)
        .slice(0,2),
    Total: o.Lines.reduce((acc, l) => acc += l.PricePerUnit * l.Quantity, 0)
}
```
