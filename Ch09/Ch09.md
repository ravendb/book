...
`l.PricePerUnit * l.Quantity * (1 -l.Discount)`. That's easy enough, but it repeats three times in Listing 9.12, making it a perfect example of the 
"don't repeat yourself" principle. If we were writing code using any standard programming language, we would wrap this in a function call to make it both easier to understand and allow it to change a single location. Luckily, RQL also has such a provision, and it allows you to define functions. 
...
* Projecting values from the document using object literal expression gives you far more power and flexibility. You can utilize JavaScript expressions to 
  ...
  inside the function to avoid repetitions and to make it easy to build complex queries.
...
> seem like a strange thing to do. Surely it'd be better to just get the documents directly if we knew what their IDs were, no?
>
> Querying by ID is handled differently. Instead of going through an index, if the query optimizer can see that your query is using an ID, it will fetch
> the relevant documents directly using their ID and pass them to the rest of the query for processing. This applies when you are querying by ID and 
...
> effort since this is an exception to the rule that queries without an index to cover them will have one created for them. In this case, the 
...
modeling you shouldn't usually want to do that, and RavenDB has ways to enable that scenario, discussed in the next chapter. 
...
talked about the include feature in Chapter 4, [Advanced Client API](#advanced-client-api) at length. It allow us to ask RavenDB to send us related
...
explicitly and explain what it means. When you write a new document or update an existing one, RavenDB does not wait to update all the indexing before it
completes the operation. Instead, it writes the document data and return immediately, scheduling any index updates to occur in an async manner.
...
favor of another alternative, more indexes impact indexing speed, not the write speed. 

Another reason for this behavior is performance. RavenDB can apply multiple optimizations to indexes that are running in an async amnner, to start with, 
instead of having to update all the indexes on every write, we can update them in batches. A single slow or expensive index isn't going to impact any other
index or the overall writes in the system.

There are a few additional reasons, such as being able to add indexes dynamically and on the fly to busy production systems and the ability to update indexes
on a side by side manner. In the next chapter, we'll discuss a bit more about the actual indexing architecture of RavenDB and how they operate, for now, let's 
focus on the implications of this design choice.

Writes are faster and indexes are operating independently. However, it does mean that a client who is fast enough can modify the data and then query on that
data before it has been indexed. This is called a stale read and there are several strategies to handle them.

> **Stale queries, documents and replication, oh my!**
>
> From the point of view of a single node, a stale read can only occur when you are querying an index. Loading a document or querying by
...
> will always return non stale result.
>
> RavenDB make a distinction between indexes (which allow stale reads) and accessing documents (via `Load` or query by id) which can never be stale.
> 
> This is when you are running on a single node. When running in a cluster, you need to take into account that a write request to a particular node
> may not have been propogated to the node that you are currently using, which may result in what is globally a stale read, even though the data you got
> is the most recent that the node you queried has.
>
...
Stale reads sounds scary, but most of the time, you don't really need to pay the full cost of getting up-to-the-nanosecond updates. By the time you show
...
high indeed. That is why caches are introduced so often, because it is usually better to get _a_ response out, even if the information is slighly out of
date than wait until we get an authoritative answer that is not relevant anymore by the time we hand it to the end user.

RavenDB ensures that if you are getting a stale response a query, you'll know about it, and the query statistics's `IsStale` result will be set to true, 
allowing you to make decisions based on this fact. In addition to whateve the query result is stale, you'll also know what is the timestamp for this 
information. 

You can also request RavenDB to wait until the index is up to date with the current changes by calling `WaitForNonStaleResults` on the 
query. This option will ensure that you get the most up to date information, but at a slightly delay. 
Finally, you can chose to wait at write time, by calling `WaitForIndexesAfterSaveChanges`, which will cause RavenDB to wait until the indexing process
for these changes has been complete before acknowledging the write. 
...
>
> On most systems, the time between a document write and the indexes applying it is measures in single digit milliseconds. We talk a lot about this in
...
The most problematic scenario for stale indexes is the `Create`/`List` pattern. The application make a change (such as adding a new order) and then 
show the list of items that should include the newly created document (the customer's recent orders). In this case, the
user has a reasonable expectation to see the change that they just made, and the query immeidately after the save might be fast enough to allow the 
query to get the index before it had a chance to complete indexing the new data.

This is a good place to use `WaitForIndexesAfterSaveChanges`. In addition to saving the value, you'll also wait for the index and only then you'll 
query for the new data. But why not do this alll the time? The reason that this isn't the default is that this is almost always not what you want 
in terms of perfomrance.

Creating or modifying a document is not always followed by an immediate query for a list of items. In all of those cases, there is no real reason why 
you'll want to wait until indexes will run. Your data is now on RavenDB, and the ACID nature of the database ensures that it is safely stored on disk.

In addition to waiting on the write, you can also wait on the read, using `WaitForNonStaleResults`. This is usually not a good idea. If you need to 
wait, do the wait on the write (which is far rare than reads). The problem with waiting on the read side of things is that you may be waiting for 
something that isn't related to you. 
Consider the case of showing the list of recent orders for a customer that didn't just create a new order. Is there any sense in waiting for
all the _other_ orders (by other customers) to complete indexing before you get the results? 
Until the new documents has been indexed, RavenDB doesn't know to which customer they belong to, so it will mark the results as stale. But the same
results will be returned after the index is up to date anyway. 

> **RavenDB lets you not pay for what you don't use**
> 
> This section can be complex to understand, in particular with regards to why we do it. RavenDB chose to have async index by default because not
> do so would mean that you would have to wait for the indexes to complete on every write and require coordination with the queries to ensure that
> you are reading the latest information.
>
> The part about slowing the write is not so hot, but the part about getting the latest information is surely what you want, no? Well, that depend
> on what this costs. A study of relation databases^["[OLTP – Through the Looking Glass, and What We Found 
...
> truly stupendous amount of effort to spend on managing locks, and a large part of that is to ensure that you get the properly conssitency 
> guarantees. 
>
...
> indexes can do their own work without waiting for each other or for document writes. For the vast majority of the cases, what you'll notice is that
> things are faster and don't take so many resources.
>
> However, someone must pay the piper at some point, which is why RavenDB allow you to chose to pay the price and wait for these tasks on a case by
> case basis. This way, you don't pay the full cost when you almost never need the benefits. 
...
One of the major advantages that RavenDB has for production usage is that all indexes are always built in an online fashion. That means that adding 
an index doesn't take any locks and can be done even on busy systems. This allow RavenDB to create new indexes while the system is working and is one
of the key reasons why the RavenDB query optimizer is able to do its work in this manner. Adding a new index in production is no longer a task that 
you schedule for late night over the weekend but soemthing that can happen during normal operations.

This lets RavenDB analyze and optimize your queries and indexes on an ongoing basis. This is also applied to indexes that you create manually. We'll 
discuss them in detail in the next chapter, but it is worth noting that when you update an index, the existing one is going to be kept around while
the new version of the index is being built, only to be atomically replaced when the updated version has fully caught up.

### Summary

In this chapter we looked at how to query in RavenDB. We started by looking at the way RavenDB processes queries using the query optimizer, including
the way RavenDB is able to generate indexes on the fly to optimize your queries. We then looked into RQL and all the nice things that we can do with
it to query your data.

Beyond the simple equality and range queries, we showed how you can use advanced features such as full text search and spatial queries. All of the 
queries that we looked at in this chapter are all using RavenDB's automatic indexes, so while the features that are exposed are quite rich, note that
in the next chapter we'll introduce user defined indexes which allow you to take the kind of queries you can make to the next level.
...
While the query langauge is quite rich and you can do a lot of transformation using its capabilities, be aware that while computation during projection
is allowed, it does add to the cost of the query (we need to run the JavaScript code on the results). This is usually not an issue, but if you have 
...
network roundtrip to the database. This can be done using `load` which gives you access to a related document and using `include` which let RavenDB
know that you'll need this related document in the client shortly, so we might as well send it now.

From plain queries we moved to talk about aggregation, using `group by` in RQL to aggregate infomration about our documents. Aggregation in RavenDB is
done using map/reduce, which we'll explore more therally in the next chapter. Aggregation is done in the background and in an incremental fashion, 
allowing the database to perform aggregation query not when you are actually waiting for the results but beforehand. This make aggregation queries in
RavenDB _very_ fast and enable a whole new set of features, because you no longer have to work so hard to get an aggregated value from the server.

Where typically you'll worry that an aggregation query can consume too many resources and cache that (requiring you to manage the cache, updates to it, 
etc), with RavenDB you can just make the query, and the database will take care of giving the results back and updating the final tally whenever 
documents are modified or inserted. 

Finally, we talked about the async nature of RavenDB indexes. This nature allow us to optimize many aspects of RavenDB, allowing us to use batches,
enabling online index builds and opening the path to the query optimizer ability to define indexes on the fly. It does mean that you need to be aware
...
In the next chapter, we are going to talk about customer indexes (called static indexes in RavenDB) that you'll define and what kind of fun we can get 
away with them and what features and queries they enable.