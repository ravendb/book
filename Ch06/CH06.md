...
learning why, I found [this answer](https://www.quora.com/Why-is-a-group-of-crows-called-a-murder) fascinating] It's
...
be simpler but not necessarily better. Let's examine the pros and cons of each approach and how they are used by 
RavenDB.

Cluster consensus with strong leaders (Raft algorithm) provide strong consistency. They ensure that, as long as a 
...
> CA and AP on the same layer. With RavenDB, the cluster layer is CP (it's always consistent but may not be available
> in the precense of a partition), but the database layer is AP (it's always available, even if there's a partition, but
> it's eventually consistent).

However, if a majority of the nodes aren't available, we can't proceed. This is pretty much par the course for consensus 
...
they are direct connections between the various nodes that hold a particular database, and they form a multi-master mesh. 
A write to any of those nodes will be automatically replicated to all the other nodes.

This design results in a robust system. First, you define your cluster, distribute your databases between the nodes,
assign work, etc. Once the cluster is set up, each database instance on the various nodes is independent. That means it can accept writes without consulting the other instances of the same database.
...
nodes. Let's see how it works in practice. Close any instances of the RavenDB server you currently have running 
...
The first thing we want to do is run an instance of RavenDB. Assuming you're running in `PowerShell` in the RavenDB
...
The `$cmdLine` is the command line argument we'll pass to RavenDB. We use this construct because it allows 
us to customize the arguments easily. In the string, we use `{0}`, which is replaced by `1` later on using the
`$cmdLine -f 1` syntax. This is a somewhat convoluted way to ask RavenDB to listen to `127.0.0.1:8080`, write the logs to
`Logs/1` and write the data file to `Data/1`. We'll see exactly why to do it this way in a bit. 
...
This uses the previously defined `$cmdLine` with a new format string and results in another RavenDB instance, this time 
bound to `127.0.0.2:8080` with logs at `Logs/2` and the data in `Data/2`. You can go to `http://127.0.0.2:8080` and see the studio, as well. However, unlike the instance running in `127.0.0.1:8080`, there are no databases
...
You can see in `127.0.0.2:8080` that the node is marked with a question mark. That's because this is a new node that 
...
If you visit the `127.0.0.2:8080` in the browser, you'll observe several interesting things. First, at the bottom, you can see we're no longer unknown node. Instead, the `127.0.0.2:8080` server has been designated as node B.
...
is only set up to be held on Node A. Click the "Manage group" button on the "Spare" database, which will take you to
a page allowing you to add a node to the database. Select Node B and then choose "Add node to group."
...
that we have actually run them on two separate machines, we are safe from a single machine going up in flames. However, two nodes
...
see that both nodes there but none of them is marked as leader. This is because Node B is unable to communicate with Node A. 
...
push us down to this half-and-half state. Normally, you'll run three or more nodes, so let's expand our cluster even further.
...
a novel three-tine pitchfork is the favorite tool of Poseidon. As such, it is only natural that our "Pitchfork" database will 
have a replication factor of three. Once that's done, we'll just create the database and observe the results.

Since we didn't explictly specify where the new database will reside, the cluster will distribute it to three nodes in 
...
Typically, production clusters are setup with either three 
...
Sometimes you'll have a five-node cluster with the data replicated five times between the nodes. This is for maximum survivability. 
...
> discuss this feature in the ["Sharing Data and Making Friends with ETL"](#integrations) chapter, which covers external
> replication.
...
and you ought to read the ["Securing your Ravens"](#security) chapter on proper deployment and security of your cluster before you
...
* Database instance&mdash;exists on a single node, usually part of a larger database group. Typically refered to as "the database instance
  on Node A."
* Database group&mdash;the grouping of all the different instances, typically used to explicitly refer to its distributed nature. "The 
...
* Database&mdash;the named database we're talking about, regardless of whether we're speaking about a specific instance or the whole group. "We use the 'Spade' database to store that information."
...
As it turns out, we don't actually need to do so. It's enough that we list any node in the cluster to be able to properly 
...
very first thing that a client will do upon initialization is to query the `Urls` is defined and figure out what are the actual nodes
...
> able to get the initial topology. If the cluster sizes are small (three to five), you'll typically list all the nodes in the cluster. But for larger
...
>
> For extra reliability, the client will also cache the topology on disk, so even if you listed a single node and it's down and the 
> client was restated, we'll still remember where to look for our database. It's only a completely new client that needs to have the 
...
nodes in the topology are called the alternates. A failure of any of the alternate wouldn't even register for the typical client configuration since
...
other instances in the group. This happens in the background and is contiously running. Most of the time, you don't need to think about it.
...
asking the database instance we wrote to that it not confirm the write until we've replicated the data we wrote to at 
...
Using this friviously will likely slow your application and introduce problems when you don't have enough replicas. 
...
RavenDB allows you to change that behavior as well. Instead of all the clients talking to only the perferred node, you can ask them to load
...
How is this done? Each database instance holds a TCP connection to each of the database instances in the group. Whenever there's a write
...
We commonly talk about document replication, but we're actually replicating more then just documents. All the data inside a database is
...
deletes), attachments and revisions. The last inclusion deserves a bit of explanataion.
...
several times in short order&mdash;fast enough that it was modified several times before we were able to send the document to the other database
...
was used to write them and the speed of replication.
...
> a race condition. You'll learn more about how RavenDB handles such manners in the section about conflicts and change
> vectors.
...
Transactions in RavenDB are _important_. Relying on ACID transactions reduces a loss of the complexity that one has to deal with. However,
...
by a later transaction (the one from Listing 6.4). Since we replicate only the current state of the documents and, in this case, we 
configured RavenDB to send the smallest possible batch size, we'll replicate only the documents that were modified in Listing 6.3&mdash;not documents that were modified in the transaction for Listing 6.3 and then later modified by another transaction.
...
document that's newer than the document it has, it will update the document to match the revision. In effect, we'll send the entire 
transaction in Listing 6.3 as it was so that a client observing a second node can never see a partial transaction.

As you can see, revisions are a very powerful feature, and they're used in more scenarios then might initially be expected. The idea 
...
The easiest way to answer that question is to take a look at the clock. After all, "newer" is a function of time, and we can just check what time the document was modified. But that doesn't actually work when running in a distributed sytem. Each machine in the cluster may
...
are sometimes also called vector clocks) to tell when things hapened. A change vector is comprised of the node ID and a sequence number (the 
...
new database, called "ChangeVectorTrailRun" on two of the nodes in the cluster. Then we'll go into the database and create a document called `users/1`.
...
A conflict occurs when the same document is updated on two nodes independently from one another. This can happen because of a network split or because of several client updates that each talked to a different node faster than we could replicate the information. 

Let's consider the simplest case. Say we
have the document `users/1` whose current change vector is `A:7, B:5`, and we modify it on nodes A and B concurrently. On the database instance on node `B`, we'll have the change vector `A:8, B:5`, and on node B we'll have `A:3, B:8`. In this case, there's no way
...
consensus (which requires consulting a majority on every decision) or accept the potential for conflicts. For documents writes, RavenDB choses to 
accept conflicts as a tradeoff of always being able to accept writes.

The question is this&mdash;how are we going to handle such a case? Let's generate a conflict manually and see how this is handled. Here's how we'll do it:
...
In fact, you'll see _three_ versions of this document, as displayed in Figure 6.14. For developers, the screen shown may look very familiar. It's
...
RavenDB, of course, has no idea how to merge such a conflict. But we can teach it. TODO - figure out how this is exposed in the studio.
...
act independently) and how it affects our code. If there is a failure that knocks down half of the cluster, we'll not be able to do cluster operations. But 
...
In the next chapter, we are going to cover how to design your cluster, defining your topology for geo distributed environment and in general scale your RavenDB
cluster.