...
and open the command line terminal to the RavenDB folder. We want to reduce the amount of work we have to do per node
so open the `settings.json` file and ensure that it has the following properties:
...
>
> In the configuration we used so far, we used `ServerUrl` to specify what interface and port we'll be listening to.
...
you want it to resolve conflicts as they happen. By default, we resolve conflicts in favor of hte most 
...
how this look like in Figure 6.14.
