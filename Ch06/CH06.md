...
In Chapter 2, I introduced the compare exchange feature (`cmpxchg` from now on). This feature allows you to perform atomic compare and
swap operations at the cluster level. As you can imagine, this feature relies on the strong consistency available to the RavenDB 
cluster via the Raft consensus protocol. 

Consider the code in Listing 6.8^[The same code was already shown in Listing 2.20 from Chapter 2], which show how to use this feature
...
the usual sematics on how to handle this.
...
public string PullAvailableEngineer(IDocumentStore _store)
{
    while (true)
    {
        var get = new GetCompareExchangeValueOperation<List<string>>(key);
        ...
        {
            var del = new DeleteCompareExchangeValueOperation<List<string>>(
                key,
                getResult.Index);
...
public void PushEngineerAvailability(IDocumentStore _store, string engineer)
{
    while (true)
    {
        var get = new GetCompareExchangeValueOperation<List<string>>(key);
