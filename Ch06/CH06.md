...
Cluster consensus with strong leaders (Raft algorithm) provides strong consistency. The leaders ensure that, as long as a 
...
and open the command line terminal to the RavenDB folder.
...
The first thing we want to do is run an instance of RavenDB. Assuming you're using `PowerShell` as your command line terminal and 
have opened it to the RavenDB folder, please run the following command:
...
You can see in `127.0.0.2:8080` that the node is marked with a question mark, indicating an unknown node. That's because this is a new node that 
...
> client was restarted, we'll still remember where to look for our database. It's only a completely new client that needs to have the 
