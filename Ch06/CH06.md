...
Another difference between the two nodes can be seen at the bottom right, as shown in Figure 6.2. 
...
Click `Add Node to Cluster` and enter `http://127.0.0.2:8080`. The new node will show up as a cluster member, as shown in Figure 6.4.
...
nodes of its choice. This means we now have a five-node cluster with two databases, as you can see in Figure 6.9.

![The Spade and Pitchfork databases in our RavenDB cluster](./Ch06/img09.png)

Figure 6.9 shows the databases from the Studio in node `D`. So we have Pitchfork on three nodes and Spade on two. You can go ahead and
...
and you ought to read about proper deployment and security of your cluster in Chapter 13, before you 
...
The system administrator can configure such behavior globally, as shown in Figure 6.11, or the client can specify that using the conventions. 
...
* Modify the `users/1` document on node `A` so its name property will be "John," and save the document. You can see how this will look in Figure 6.15.  
...
* Bring up node `B` by executing:  
    ...
    --Logs.Path=Logs/B `  
    --DataDir=Data/B  
* Modify the `users/1` document on node `B` so its name property will be "Doe", and save the document. You can see how this will look in Figure 6.16.  
...
In fact, you'll see _two_ versions of this document, as displayed in Figure 6.17. For developers, the screen shown may look very familiar. It's
...
With a merge script such as the one in Listing 6.6, RavenDB can automatically resolve conflicts as they happen, with no service interruption. 
The cart in Listing 6.7 has been intelligently merged, so we don't have any duplicate products and we kept the higher number of milk 
...
for an operation to succeed. With 5 nodes in the cluster, you'll need 3, etc. See Table 7.1 in the next chapter for full details on this.
