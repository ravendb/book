...
Cluster consensus with strong leader (Raft algorithm) provides strong consistency. The leader ensures that, as long as a 
...
> [https://ravendb.net/download](https://ravendb.net/download) and download the download package for your platform. 
> The rest of this chapter assumes that you'll run all commands inside the uncompressed RavenDB directory.

The first thing we want to do is run an instance of RavenDB. I suggest running each RavenDB instance in its own terminal
window, since that make it easy to see the behavior of individual nodes:
...
This time we run the RavenDB instance bound to `127.0.0.2` (note the 2, not 1 in the end) and with the same `8080` port.
Logs goes to `Logs/B` and the data is stored in the `Data/B` directory. By utilizing the `127.*.*.*` local ip range we 
can easily run distint RavenDB instances on the same machine and easily tell them apart. 
...
is only set up to be held on node `A`. To spread the "Spade" database to node `B` you'll need to click on the "Manage group"
button on the "Spare" database, which will take you to a page allowing you to add a node to the database. Select node `B`
...
>
> For extra reliability, the client will also cache the topology on disk, so even if the document store was initailized with a single node
> which is down at the time that the  client was restarted, the client'll still remember where to look for our database. It's only a 
...
![A database group with 3 nodes, one of whom is down](./Ch06/img12.png)
...
In Figure 6.12 you can see that node `C` is down and the cluster has detected it and designated node `B` as the mentor node for node `C`.
When node `C` will come back again, it is node `B` that will send it all the changes that it missed, and it will only graduate to be a full
member in the cluster when node `B` deems it ready.
...
have the document `users/1` whose current change vector is `A:7, B:5`, and we modify it on nodes A and B concurrently. On the database instance on node `B`,
...
![The `users/1` document after modifications on node `B`](./Ch06/img14.png)

![The `users/1` document after modifications on node `A`](./Ch06/img14.png)
...
In fact, you'll see _three_ versions of this document, as displayed in Figure 6.15. For developers, the screen shown may look very familiar. It's
...
![Manual conflict resolution in the studio](./Ch06/img15.png)
