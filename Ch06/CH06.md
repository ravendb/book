...
This leader, in turn, is responsible for such things as monitoring the cluster health, selecting the preferred node that
...
they're direct connections between the various nodes that hold a particular database, and they form a multi-master mesh. 
...
and open the command line terminal at the RavenDB folder.
...
> The rest of this chapter assumes that you'll run all commands inside the unzipped RavenDB directory.
...
Those operations include creating a new database and monitoring the health of other nodes. But a lot of database-specific configurations 
(anything that impacts all instances of a database) go through the cluster as well. For example, we wouldn't be able to schedule 
...
very first thing that a client will do upon initialization is to query the `Urls` is defined and figure out what the actual nodes are
...
The easiest way to answer that question is to take a look at the clock. After all, "newer" is a function of time, and we can just check what time the document was modified. But that doesn't actually work when running in a distributed system. Each machine in the cluster may
...
A conflict occurs when the same document is updated on two nodes independently of one another. This can happen because of a network split or because of several client updates that each talked to a different node faster than we could replicate the information. 
