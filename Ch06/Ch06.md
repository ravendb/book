...
Instead of executing the indexes as part of the write transactions, we run them as a background task. You can see a hint of that in Listing 6.2. This code (or rather, its conceptual cousin) is always running in the background, and whenever there is a change in the documents, the indexing function is run on the new or updated documents.
...
It is just too costly to do so. 
