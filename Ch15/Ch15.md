...
Thigns seems to be fine: the average request time is low, and even the maximum isn't too bad. 
...
> of the split in requests and the relative costs associated.
...
memory (that goes through the .NET GC) and unmanaged memory (that is managed by RavenDB directly). Typically, these **(((these operations? these files?)))** will
...
RavenDB makes larger use of async operations internally to reduce the overall number of context switches. But the decision to prioritize more cores over faster cores is one you have to make based on your requirements.
...
This question is often academic in nature. **((I'm not sure what "This question is often academic in nature" means? That we can't really know?)))** We have tested RavenDB with a Raspberry Pi 3, which uses a quad-core 1.2GHz 
...
"High Priority" **(((Does this need to be in quotation marks/caps? Or can it just be high priority?)))** by definition. 
...
> depth in Chapter 6 **(((Should this have a link back to Chapter 6???**) with a multi-master architecture. 
>
...
**(((Note: you never explain what you mean by a "czar's" cluster.)))**
...
This **(((Is the "this" here a two-node cluster? If so, can we just say "A two-node cluster is sometimes...")))** is sometimes chosen as the selected topology when you want to manually control failover behavior for some reason.
...
> will **(((will or can?)))** replicate all its data to any of its sibling instances on the other nodes in the database group. 
>
...
catastrophe. On the other hand, at some point, we are safe enough and continuing replication is just burning through disk space. 
...
struggling to manage. Sure, each of those databases was quite small, 1-5GB in size, but when they **(((Is the "they" here the client? The database?)))**
needed to access all of them, the database **(((Should this be database or server?)))** was consuming a lot of resources and often failed to keep up with the 
demand. 
...
only ever a single write pending at any given time. This write always uses unbuffered writes and 
direct I/O to hit, making sure that the transaction is properly persisted. It's also in the hot path for a transaction 
commit, since we can't confirm that the transaction has actually been committed until the disk has acknowledged it.

The `Raven.voron` file is written to in a random manner, typically using memory-mapped I/O. Cccasionally a 
`fsync` is called on it. This **(((this file?)))** is not in the hot path of any operation, but if it's too slow, it can cause RavenDB
...
Chapter 13, all communication channels running in secured mode are using TLS 1.2 to secure the traffic. 
...
I/O is very important for RavenDB, and we'll go back to this topic in the monitor chapter as well **(((Should this list the specific chapter number?)))**. 
