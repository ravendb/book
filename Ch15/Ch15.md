...
We have tested RavenDB with a Raspberry Pi 3, a $25 computer, which uses a quad-core 1.2GHz ARM CPU and 1 GB of RAM.
On that machine, we were able to process a bit over 13,000 document reads per second. Those were simple document
...
direct I/O to ensure that the write by pass all caches and write directly to the underlying drive. This is important
...
any operation, but if it's too slow, it can cause RavenDB to use memory to hold the modified data in the 
