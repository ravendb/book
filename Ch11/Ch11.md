...
In the previous chapter, we looked at how we can query RavenDB, while letting the query optimizer handle the indexing for us. 
...
indexes work and behave, which is very useful for performance monitoring and troubleshooting issues. This chapter, however, will primarily focus on how you can define your own indexes, why you would want to do that and what options this provides you.
...
Indexes offer quite a bit of features and capabilities, it's a big topic to cover &mdash; but it's also
one that gives you a tremendous amount of power and flexibility.


### What are indexes?

Indexes allow RavenDB to answer questions about your documents without scanning the entire dataset each and every time. 
An index can be created by the query optimizer or by the user directly. The way an index works is by iterating over the documents and building a map 
between the terms that are indexed and the actual documents that contain them - a process that is called indexing. After the first indexing run, the index will keep that map current as updates and deletes happen in the database.

Listing 11.1 shows a simple way to construct an index. The code in Listing 11.1 has nothing to do with RavenDB but is provided so
...
The code in Listing 11.1 is meant to convey a sense of what's going on. We're given a list of users, and we iterate over the list,
...
two options shown in Listing 11.2.
...
In the first section in Listing 11.2, we generate the index and then use it multiple times. In the second section, we create the index 
for each query, dramatically increasing the overall cost of making the query.

The code in Listing 11.1 and Listing 11.2 is about as primitive an index as you can imagine. Real world indexes are quite a bit more
complex, but there's a surprising number of details that are actually unchanged between the toy index we have here and the real-world 
...
dictionary usage in Listing 11.1.

#### Indexes come in layers &mdash; and with an identity crisis

I'll be the first to admit that this can be quite confusing, but RavenDB actually has several different things called "index" internally. At the lowest level, we have Voron indexes, which is how RavenDB organizes the data in persistent storage. As a user, you
...
async nature of higher level indexes in RavenDB, these Voron indexes (sometimes also called storage indexes) guarantee full consistency to the reader. You'll rarely be able to see them in use or affect them in any way, but they're crucial for the well being and 
...
also stored in Voron, to ensure the index data is transactionally safe and can recover from an error such as an abrupt shutdown.
...
RavenDB uses C# and Linq to define indexes, and Figure 11.1 shows the simplest possible index: indexing the `FirstName` and `LastName`
from the `Employees` collection.

![A simple index over the `Employees` collection](./Ch11/img01.png)

An index is just a C# Linq expression on a collection that outputs the values we want to index. The index in Figure 11.1 isn't
...
> **You can create RavenDB indexes using C# Linq or JavaScript**
> 
> RavenDB supports the creation of indexes using both Linq syntax (C# based) and JavaScript syntax. You can do anything you want with indexes
> using either option, but you'll usually choose the option that you are most familiar and comfortable with. As far as RavenDB is concerned,
> there is no major difference in what language you use to write the index.
> 
> I'm mostly going to show off C# indexes, but I'll cover JavaScript indexes as well in this chapter, you can find more information about 
> indexes and how to write them in the online documentation. JavaScript indexes have a couple of major advantages over C# Linq indexes:
>
> * JavaScript indexes allow for much easier imperative logic. This is useful if you need to run non trivial logic in the index.
> * JavaScript indexes are interpreted, not compiled. That means that the database has far greater control over what you can do there.
>   Only DB Administrator can create a C# Linq index (compiled), but no special permissions are required to create a JavaScript index. 

Listing 11.3 shows how we can query this index. Instead of specifying that we'll use a collection, which will cause the query optimizer
to take over and select the best index to use, we're explicitly specifying that we want to use the "MyFirstIndex” index.
...
have to be the same. Consider Listing 11.4, which shows an updated definition for "MyFirstIndex". It indexes a computed value rather
than actual values from the document. Also consider Listing 11.5, which shows how to query the new index.
...
The result of Listing 11.4 and Listing 11.5 is quite interesting, so it's worth examining further. In Listing 11.4, we define an index
whose output is a computed field ("Name"), which in turn is the result of a concatenation of two values from the document. 
In Listing 11.5, you can see that we're querying over the index and finding a result, even though the source document never contained such a field. 
Listing 11.5 shows two ways to create the index. The C# Linq code and the JavaScript code. You can see that aside from the different ways to 
invoke them, they behave in exactly the same manner.
...
during the indexing process and then query over the result of the said computation. This is quite powerful because it allows you to 
do some pretty cool things. For example, consider Listing 11.6, which shows an index ("Orders/Total" in the sample dataset) that does
...
map("Orders", order =>{
    ...
    return {
        ...
        Total: total
        /* Could also use:
        Total: order.Lines.reduce((acc, l) => 
            acc + ( (l.Quantity * l.PricePerUnit) * (1-l.Discount) ),
            0)
        */
    };
})
...
In Listing 11.6, we've computed the total value of an order. The formula we used isn't too complex, but it's also not 
trivial. When comparing the C# code to the JavaScript code, we can see some interesting differences between them. 
The C# is a single expression, and it is very easy to do things like `Sum`. The JavaScript, on the other hand, we use
a loop. You can also use the `reduce` option, which is similar to how we do aggregation in the C# version, but I wanted
to make it clear that you can do imperative things inside the index. 

> **JavaScript, math and precision, oh my!**
>
> In JavaScript, *all* numbers are actually `double`. In other words, you don't have `int`, `long`, etc. All you have
> are double precision floating point numbers. For the most part, that shouldn't concern you, but when you use a 
> JavaScript index for numerical processing (as in the index above), you need to take that into account. 

The index (either option) in Listing 11.6 allows us to run a query such as the one in Listing 11.7.
...
The query in Listing 11.7 demonstrates a few key concepts. Once the indexing process is done, the 
...
happened at indexing time, and the cost of the query in Listing 11.7 is the cost of a seek to the relevant location in the index, plus the cost of sorting the results according to the indexed value. 
...
> shift the cost of the computation to indexing time as much as possible. Queries are far more frequent than updates, so this kind of cost-shifting makes a lot of sense. Of course, even so, if you have 
> several operations you want to do on a specific collection, you're better
...
> This means that for the same input, the index definition will always produce the same output. One of the reasons that RavenDB uses
...
>
> In particular, usage of the current date/time or `random`, as well as trying to access external resources, is not allowed. This lets RavenDB
> assume that identical inputs will produce identical outputs and is important for reindexing and updates. 
>
> When you are using JavaScript indexes, you need to make sure that you aren't doing something that relies on the current time or invoking
> something that isn't deterministic. RavenDB assumes that your indexes are pure functions and violating that assumption may cause hiccups
> in how RavenDB maintain the index. 
>
> The indexing function is called with the document as its only input, so unless you are trying to call `Math.random()` or `new Date()`, it 
> is actually quite hard to _not_ end up with a pure function. 
...
For example, let's assume we have the JSON document in Listing 11.8 and we query using `from Dogs where search(Name, 'Arava')`. 
...
additional processing, and the actual terms that will be indexed are shown in Listing 11.9.
...
see from the data we keep in the index in Listing 11.9 that we aren't storing the full document in the index. Instead, we only store the 
document ID.
...
> This means that even if the index itself hasn't caught up to changes, the data it pulls from the document store will have everything up to date.

Another important aspect of how queries work in RavenDB, which you can see in Listing 11.9, is that the `Name` field was not indexed as a single 
term. In other words, if we looked for `index.Name['Arava Eini']`, we wouldn't find anything in the index. This is because we search for the terms in the index. And during indexing, the terms were created by breaking the name to its constituents' parts and making all letters lowercase. At query time, we can apply the same
...
We already saw, in Figure 10.4 in the previous chapter, that you can pull the terms that are actually indexed from RavenDB and inspect them, 
...
Listing 11.6 serves as a good example for this feature. The documents don't have a `Total` property, but we can compute it during indexing
and query on it.

> **Security considerations**
> 
> It's worth noting that a static index using a C# Linq in compiled down to real binary code, which means you have a lot of power in your hands. An index can 
...
>
> JavaScript indexes are running inside an interpreter and have various limits placed upon them (limiting the recursion depth, a budget of instruction
> for processing a single document, etc). Like automatic indexes, JavaScript indexes can be defined by users of all permissions levels.
> 
...
> You can read more about the `Additional Sources` feature in the online documentation. With JavaScript indexes, you can add additional libraries
> such as `moment.js` or more complex routines for processing your documents.
...
Linq expression that composes the index definition. Listing 11.10 shows an example of filtering out employees without a manager for the
...
Only employees who have someone to report to will be included in this index. In the JavaScript version, you can see that we are filtering
the values we don't want by simply calling `return` with no value. There isn't usually a good reason to filter things during
...
where you can make such decisions on a per query basis. We'll see another use case for filtering the raw data in the index in a bit, when
...
from the results and load the documents from the document store. Since the typical query will return full documents back, that's usually what you'll 
want to do.

Sometimes, such as with the `Total` field in Listing 11.6, you want to compute a value during indexing and use it in your projection. By 
...
We can ask RavenDB to store the field. Go to `Indexes`, `List of Indexes`, and click the "Orders/Totals" index. This will take you to 
...
We can project the `Total` field from the query, as you can see in Listing 11.11.

```{caption="Projection of a stored computed field" .sql}
from index 'Orders/Totals'
where Total > 10.0
select Total, Company, OrderedAt
```

Listing 11.11 also shows that we can project a standard field, `Company`, without storing it. This works because if the value isn't stored in the index
...
Stored fields are used primarily to store the result of such computations. There's a small performance advantage in projecting everything
from the index. We don't need to do the document load, and in some very specific circumstances, that might be helpful. But document load is a _very_ 
...
see the content of the map functions in Listing 11.12.
...
The three map functions in Listing 11.12 each point to a different collection. RavenDB will use the information from all three map functions to 
...
even though the `Employees` collection has no such field. I'm going to skip defining JavaScript indexes along side the C# version unless there is
something interesting to point out regarding the difference between C# based index and a JavaScript one.

> **Null propagation in indexes**
>
> Looking at the index defined in Listing 11.12, what do you think will happen if we have a `Company` document that has a `null` value for
> it's `Contact` property? 
> 
> In either C# or JavaScript, running: `c.Contact.Name` when `c.Contact` is `null` will raise an error. But inside RavenDB, the behavior
> is a bit different. RavenDB will ignore the `null` value, as if you have written `c?.Contact?.Name`. In other words, you have
> automatic null propagation without really needing to think about it. 
...
that, it's usually best to project the data out into a common shape, as you can see in Listing 11.13.
...
The output of the query in Listing 11.13 can be seen in Figure 11.2. You can also remove the `select` clause for this query to see how the results will change when you get three different types of documents back from a single query.

![Common output shape for results from multimap index](./Ch11/img02.png)
...
The query in Listing 11.13 is nice, but it's awkward. We wouldn't want to ask our users to specify the full name of the person they're trying to 
...
results without the expense of scanning all of them. Listing 11.9 is a good example of how RavenDB breaks up the terms during indexing for quick 
lookups and speedy searches. But so far, we only looked at it with dynamic queries. How do I make use of the full text search capabilities of RavenDB 
...
terms. We've seen such an example in Listing 11.9, but let's consider the following string: `"The white knight is running to the princess's tower
...
Just changing strings to lowercase isn't that interesting, I'll admit, but analyzers can do much more. When you use `search()`, you'll
...
These are called stop words and are stripped from the terms the analyzers return because they add no semantic value to the search results.
...
English and most Latin-based languages, but sometimes you'll need more. In that case, you can look at the available analyzers and use them. A full 
list of the analyzers available by default with RavenDB can be found in the online documentation.
Because RavenDB uses Lucene behind the scenes, it's quite easy to find analyzers for most needs that can be readily used by RavenDB.
You can also define your own custom analyzers quite easily as well.

Let's look at how analyzers modify how RavenDB indexes documents. Figure 11.3 shows a sample document that we'll focus on.

![Sample document for playing with full text search](./Ch11/img03.png)

Go ahead and create this document, and then create an index named `test/search` with the index definition in Listing 11.14.
...
![Configuring the `test/search` index fields with different analyzers](./Ch11/img04.png)
...
on each of these fields. The results are shown in Figure 11.5.

![The indexed terms for the same values using different analyzers](./Ch11/img05.png)
...
different, yet the end result is the same. 
Consider the query:  
`from index 'test/search' as n  
where search(n.Search, "princess flower tower")`.
...
* `where lucene(n.Search, ' "princess tower" ')` &mdash;  
  Phrase query match (note the `"` in the query) because there's a `princess` followed by a `tower` in 
  the text, even though we don't have the `'` in the query.
* `where lucene(n.Search, ' "running princess" ')` &mdash;  
  Also a match because the word `running` is followed by a `princess` (with the stop words removed).
* `where lucene(n.Search, ' "running knight" ')` &mdash;  
...
* `where n.Search = "princess tower"` &mdash;  
  No match. There's no term `princess tower` for this field.
* `where n.Search = "dragon"` &mdash;  
  Match. There's a term `dragon` for this field.

This is really strange, isn't it? But take a closer look at Figure 11.5, and it will be clear what's going on. There's a term `dragon` there, and when
...
RavenDB doesn't require anything to match between the source document and its index entry. We saw that previously in Listing 11.14, when we indexed a 
...
Listing 11.14 showed an example of how we can index a single field multiple times. Listing 11.15 shows the index definition for `Companies/Search`, 
which allows for the reverse scenario, where we're searching over multiple fields in the document using a single field in the index entry.
...
The index definition in Listing 11.15 adds three different fields to the `Query` field on the index entry. When creating this index definition, you 
...
* `from index 'Companies/Search' where search(Query, "ALFKI")` &mdash;  
  Search companies using the external ID.
* `from index 'Companies/Search' where search(Query, "Alfreds")` &mdash;  
  Search companies by full text search on the company name.
* `from index 'Companies/Search' where search(Query, "Anders")` &mdash;  
...
![Marking a field as having the suggestions feature](./Ch11/img06.png)
...
The result of the query in Listing 11.16 can be seen in Figure 11.7. 

![Suggested alternatives for "Martin"](./Ch11/img07.png)
...
The query in Listing 11.17 will give `sommer` and `cramer` as the possible suggestions for `summer`. 
...
>
> The way "more like this" works beneath the surface is quite simple. You mark a field as full text search and define it to have a term vector. These two things
...
We'll start by defining the `Orders/Search` index, as shown in Listing 11.18.
...
Before we can start performing a "more like this" query, we need to configure a term vector for these fields, as shown in Figure 11.8. 

![Defining term vectors (for use with more like this) in the `Orders/Search` index](./Ch11/img08.png)
...
>
> You might have noticed in Figure 11.8 that we _didn't_ set the fields to use full text search. Why is that? The values we're going to
...
With all of the preparations complete, we can now actually make our first "more like this" query, which is shown in Listing 11.19.
...
The result of the query in Listing 11.19 is 65 results. It's the same as if we replaced the `morelikethis()` call with the equivalent query
...
I'm not going to cover all the options of `morelikethis()` here, but I wanted to point out that you have a great deal of control over exactly how RavenDB is going to match these documents. Take a look at Listing 11.20, where we want to apply the `morelikethis()` only on the `Address` field.

```{caption="Querying with morelikethis() on a specific field" .sql}
from index 'Orders/Search' 
where morelikethis(id() = 'orders/535-a', '{"Fields": ["Address"] }')
```

The results of the query in Listing 11.20 is just 15 similar orders: those that were sent to the same city and country. Other options allow you to 
...
As you can see in Figure 11.9, searching YouTube for "Dancing" is an interesting experience. How would I be able to choose from over 280
million different videos? 

![Faceted search in YouTube](./Ch11/img09.png)

Facets allow me to narrow down the search quite easily by exposing the inner structure of the data. There are two searches shown in Figure 11.9:
...
300,000 results. Figure 11.10 shows how eBay uses facets to help you narrow down the selection to just the right one.

![Facets can help a customer to narrow down to the exact product they want](./Ch11/img10.png)
...
figuring out what kind of questions they need to ask. The feedback from the numbers in Figure 11.10, in contrast to the match / no match 
indication in Figure 11.9 is another factor, giving the user the ability to guide their searches.
...
The results of the query shown in Listing 11.21 can be seen in Figure 11.11. The query itself is composed of three facets: a range facet on the 
...
![Faceted query results](./Ch11/img11.png)

The query in Listing 11.21 is simple since it has no `where` clause. This is where you'll typically start &mdash; just giving the user some indication
...
will then look like the one in Listing 11.22.
...
In Listing 11.22, we're querying over the same facets, but we're now also limiting it to just particular suppliers. The output of the `Price` facet
will change, as shown in Listing 11.23.

```{caption="`Price` facet output from the query in Listing 11.22" .json}
{
    "Name": "Price",
    "Values": [
        {
...
In Chapter 10, we discussed spatial queries and used them with automatic indexes. Using a static index gives 
...
`Companies/Spatial`, as shown in Listing 11.24.
...
With this index, we can now perform spatial queries. You can see such a query in Listing 11.25, querying for companies within a mile of
...
The query in Listing 11.25 has a single result: the White Clover Markets company. You can also use the `CreateSpatialField` to pass a `WKT`
...
`Spatial` toggle. The result should be similar to Figure 11.12. 

![Spatial field indexing options](./Ch11/img12.png)
...
terms. You should see something similar to Figure 11.13.

![Bounding box indexing behind the scenes](./Ch11/img13.png)
...
fields that you can see in Figure 11.13.
...
RavenDB will translate it to the same bounding box system. You can see a sample of this query translation in Listing 11.26.
...
save the index. Now, go to the index terms, and you'll find something similar to Figure 11.14.

![Geo hash indexing behind the scenes](./Ch11/img14.png)
...
![A map showing the top level of geohash](./Ch11/img15.png)

By looking at the map in Figure 11.15 and the terms from Figure 11.14, we can see that the prefix `6` covers most of South America. The
...
![A map showing multiple levels of geohash](./Ch11/img16.png)

Look at the spatial indexing options in Figure 11.12. You can see the `Max Tree Level` there, which determines the accuracy of the 
...
we do the reverse and find the matches. Note that with the geohash strategy, the geohash indexing creates the relevant shapes in a 
rough fashion, after which we check whether all the shapes inside the geohash match our actual query. 
...
![Coordinates terms when indexing using quad prefix tree](./Ch11/img17.png)
...
shapes can cause long indexing times. You'll usually not notice that, because indexing is async and doesn't hold up other tasks, but
...
In such cases, it might be better to move the spatial data to a dedicated collection containing just that, so only the documents with the
spatial data will be indexed if they change. This is typically not required, since even with spatial indexing, the indexing is
...
document whenever we updated the manager. That is...not so nice, even if RQL makes it easy, as you can see in the patching done in Listing 11.27.
...
The only thing that's of interest in Listing 11.27 is the passing of parameters to both the query and the `update` clause. Other than
that, it's a perfectly ordinary patch. However, having to do such things will likely result in the ghost of "you shoulda normalized that"
...
Another option is to ask RavenDB to handle this explicitly during the indexing process, as you can see in Listing 11.28.
...
// or JavaScript code
map("Employees", e => {
    var manage = load(e.ReportsTo, "Employees");
    return {
...
I've created Listing 11.28 as an index named `Employees/Search`. And I can query it as shown in Listing 11.29. 
...
As you can see, we're able to query the related information. But how does this work? In Listing 11.28, you can see the use of an unfamiliar
function: `LoadDocument(e.ReportsTo, "Employees")` or `load(e.ReportsTo, "Employees")`. 
This is the key for this feature. At indexing time, RavenDB will load the relevant
document and allow you to index its values. Note that we need to specify which collection we're loading the document from.
In both cases, the first argument can be a single document id or a collection of document ids. If you pass a collection of ids, 
`load` and `LoadDocument` will return an array of results. 
...
`Smith`. Then execute the query in Listing 11.29 again. You'll find no results. But if you run it on `ManagerLastName = 'Smith'`,
...
watch it for changes (in a similar way to how we watch the collection that we're indexing). The loaded documents can be from the collection we're already indexing (as is the case in Listing 11.28) or an unrelated collection.
...
> needs to scan and assess whether any references need to be reindexed.
...
The whole referenced document vs. referencing documents can be a bit confusing, so an example might be in order. In Listing 11.28,
...
Remember that the reindexing will happen on any change in the referenced document. In Listing 11.28, we are only using the
...
document and you do want a query based on data from related documents, `LoadDocument` is a really nice way to do so.
...
We can do that in RavenDB quite easily with a bit of pre-planning. Let's look at Listing 11.30, showing the `Employees/DynamicFields`
index and how RavenDB allows us to index dynamic fields. We'll first do that in C# only and then look at how this can be done with
JavaScript.
...
There are several things going on with the `Employees/DynamicFields` index in Listing 11.30. First, we used the `_` variable name as 
...
and whose value is the employee's `LastName`. This is an example of dynamic fields that are created explicitly. With the index
defined, we can now start querying it, as you can see in Listing 11.31.
...
the same fields; the `Employees/DynamicFields` shown in Listing 11.30 can accept any structure you want. Dynamic fields complete the 
schemaless nature of RavenDB and allow for complete freedom of operations.

The code in Listing 11.30 _works_, but it is one of the cases where we really feel the limits of the Linq syntax. A nicer way to
handle this is shown in Listing 11.31, using JavaScript, instead.

```{caption="`Employees/DynamicFields` index, using dynamic fields, in JavaScript" .js}
map("Employees", e => {
    var result = { 
        Name: e.FirstName + " " + e.LastName
    };
    result[e.FirstName] = e.LastName;
    for(var prop in e.Address) {
        result[prop] = e.Address[prop];
    }
    return result;
});
```

Listing 11.31 shows how JavaScript index is much clearer when we have to deal dynamic fields, because the semantics are much clearer
and more explicit. RavenDB allows you to return a `null` or `undefined` (in which case no indexing will take place), an object, 
in which case all the object properties will be indexed or an array of objects, where we'll index any of the objects independently
(allowing you to output multiple results for a single document).

RavenDB will scan only the first level properties. If you return a property whose value is an object, that object will be turned 
into a JSON string and then index as is. If the property value is an array, we'll treat each element in the array as a term for
the field (similar to how with full text search we emit multiple terms for a single string). 

> **Controlling indexing of dynamic fields**
>
> Previously in this chapter, we looked at defining the indexing mode, storage options and term vectors for fields. These required that
> we'll specify the field name in advance, which obviously doesn't work very well when you are working with dynamic fields. 
>
> There are two ways to handle this in RavenDB. First, you can define defaults for all these values that would apply to any
> field that hasn't been overridden. See the `Default field options` in the Studio or use the
> `Constants.Documents.Indexing.Fields.AllFields` field name to set those properties globally for the index.
>
> You can also define them on a case by case basis using `CreateField(name,value,CreateFieldOptions options)` in C# or by 
> embedding the options directly in the field value from JavaScript, like so:
>
> ```
> map("Employees", e => {
>     return { _ignored_: { 
...
> ```
>
> This allows you to have fine tuned control over the indexing options for dynamic values.
...
working with indexes explicitly by defining indexes using `Linq` expressions or `JavaScript` code.
These give us the ability to select the fields (and computed values) that we want to be able to index. 
...
and guess what the user actually meant to search for, and "more like this" can be used to find similar documents to the one the user is 
looking at.
...
RavenDB also supports spatial queries, allowing you to find documents based on their location on the globe. We looked at how such indexes
