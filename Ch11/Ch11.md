...
![Showing 606 mapped entries for `companies/1-A`](./Ch11/img06.png)
...
On the left in Figure 11.7, you can see that, unlike for `companies/1-A` in Figure 11.3 where we had a flat list, Figure 11.7
...
![Complex output from the index, showing aggregation inside the reduced result](./Ch11/img12.png)
...
![An artificial document created as a result of a MapReduce's output collection](./Ch11/img15.png)
...
We'll first add another entry for each of the existing map: `OrderTotal = 0`. And we'll add the same logic for the reduce 
...
> 
> * `D` &mdash; the total number of orders. In this database, after running the patch operation in Listing 11.2, the number is 83,830 documents.
