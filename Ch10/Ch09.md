...
The same is true for querying by document ID or IDs, as you can see in Listing 10.1. The query optimizer doesn't need an index to handle these queries. It can answer them directly. 
...
However, what happens when we start querying using the data itself? You can see the result in Figure 10.1. In particular, you'll note that 
RavenDB reports that this query used the `Auto/Employees/ByFirstName` index. 

![RQL query for employees named Andrew](./Ch10/img01.png)
...
You can see the index that was created as a result of running this query in Figure 10.2. 

![The merged auto index can answer queries by first name or last name or both](./Ch10/img02.png)
...
you can't just modify the query. You need to tell RavenDB it needs to do some computation during the indexing process. We'll see how that's done in Chapter 11.

Queries can also use more then a single field, as you can see in Listing 10.2.
...
Using the sample data set, this should give two results, as shown in Figure 10.3. In that figure, you can also see some of the options available to inspect
...
![Available operations for inspecting the index state](./Ch10/img03.png)
...
actually indexed, as illustrated in Figure 10.4.

![The indexed terms for the various fields in the index](./Ch10/img04.png)

Why is this so important? Even though Figure 10.4 doesn't show all the values, it shows enough to explain how RavenDB will actually process 
...
clauses, and you can see the result of each in Table 10.1.
...
To answer the query in Listing 10.2, RavenDB will find the matching documents for each of the clauses, as shown in Table 10.1. At that point, 
...
The standard query operations you would expect are here, of course, as well as a few more, as shown in Table 10.2.
...
These type of queries count as computation during a query and can't be expressed directly in RQL. Don't worry &mdash; you can still make such queries, but you need to 
use a static index to do that. This will be discussed in Chapter 11, which is dedicated just to this topic.
...
The answer is that a single value can't, but an array most certainly can. Consider the document shown in Figure 10.5, with an
...
![The `regions/1-A` document contains an array of `Territories`](./Ch10/img05.png)
...
range. `BETWEEN` is inclusive on the low and high ends. In other words, consider the query in Listing 10.3. 
...
The results of the query in Listing 10.3 will include employees hired in 1992, 1993 and 1994. We could have also written the same query 
with string matches, as shown in Listing 10.4. 
...
The query in Listing 10.4 will also match all the employees hired in 1992, 1993 and 1994. But why? It's because of a minor trick we use here. The actual date format used by RavenDB is `ISO 8601`, so technically speaking, the query in Listing 10.4 is supposed to look like this: 
...
On the other hand, you could have written the query as shown in Listing 10.5 and found only the results that match the value and the casing used.
...
terms) in a set of documents and find results without having an exact match. For example, examine Figure 10.6, where we're searching for a company that has the word 'stop' in its name.

![Full text search queries on companies' name](./Ch10/img06.png)
...
terms that are indexed are _not_ the names of the companies. Figure 10.7 shows a comparison of the two fields. 

![List of indexed terms for `Name` and `search(Name)` fields](./Ch10/img07.png)
...
The answer is in the way RavenDB indexes the data. Instead of indexing the `Name` field as a single value, RavenDB will break it into separate tokens, which you can see in Figure 10.7. This means that we can search for individual words inside the terms. We search not the full field but rather the indexed tokens, and from there, we get to the matching documents.
...
Listing 10.6 for the full query.
...
>
> Built-in methods (such as the ones listed in Table 10.2) are case insensitive, and you can call `where startsWith(Name, 'An')` or
...
Consider the query in Listing 10.7, which expresses a fairly complex conditional and ranking requirement using `Boost()`
...
The query in Listing 10.7 will select companies based in London, Paris or anywhere in Germany or Sweden. The effect of `Boost()` on the results is that a document matching on London would be given a boost factor in the ranking. This means that the query in Listing 10.6 will first get results for London, then Paris and then Germany and Sweden. 
...
An interesting issue with boosting is that it isn't quite as obvious as you may think. Consider the query in Listing 10.7. If we change the last clause to be `Address.Country IN ('Germany', 'France')`, we'll start getting Parisian companies first, even though the boost on London-based companies is higher. The reason for that is because the Parisian companies will have two matches to their names (both Paris and France) while the London companies will only have
...
The simplest query projection can be seen in Listing 10.8.
...
The query in Listing 10.8 will produce the results with three fields: `Name`, `Address.City` and `Country`. A single simple projection of fields and the use of aliases from this query is demonstrated in Figure 10.8. You can see that we didn't specify an alias for the `Address.City` field and that the
...
![A single document result in a projection query](./Ch10/img08.png)

In the case of the query in Listing 10.8, we're only projecting simple fields, but RQL is capable of much more. Listing 10.9 has a more complex projection example, projecting both objects and arrays.
...
A single projection from the results of the query in Listing 10.9 is shown in Figure 10.9. This query is a lot more fun. You can see that there's no
...
![Projecting array and complex object from a document](./Ch10/img09.png)
...
trivial of documents. With RQL, you aren't limited to simply selecting the flat list of properties from the document. You can also project a complex result with the object literal syntax. Let's look at a simple example of using object literals to query in Listing 10.10.
...
The result of the query in Listing 10.10 for document `orders/1-A` is shown in Listing 10.10. As you can see, we're able to project the data out not only using 
...
> 
> The query in Listing 10.10 is using `from Orders as o`, defining the alias `o` for the `Orders` collection. This is required when using the object literal
> syntax since we need to know the root object the expression starts from. 

```{caption="Result of projection from Listing 10.10" .json}
{
    "Country": "France",
...
The key to the object literal syntax is that this isn't a JSON expression; it's a JavaScript object literal, and any valid JavaScript expression is going to work. For example, let's take a look at Listing 10.12, which shows a slightly more complex example.
...
We'll look at one final example of the kind of projections you can make with the object literal syntax, mostly because I think it's a beautiful example of what you can do. Listing 10.13 shows a query that will get the two most expensive products and the total value of the order.
...
The object literal syntax is quite flexible, but it has a few limits. In particular, take a look at Listing 10.13 and how we compute the cost of a particular product by using the following formula: `l.PricePerUnit * l.Quantity`. However, I forgot to also include the discount that may
be applied here. The formula for the discount is simple. The new way to compute the price of a product is simply 
`l.PricePerUnit * l.Quantity * (1 -l.Discount)`. That's easy enough, but it repeats three times in Listing 10.13, making it a perfect example of a violation of the 
...
Listing 10.14 shows how we can use the function declaration to properly compute the product price while avoiding repetition. 
...
In Listing 10.14, we first declared the function `lineItemPrice`. This took the line item and computed the total amount you would pay for it. Once this was declared, you could then use the function inside the object literal.
...
I'm sure you can imagine the kind of queries that declaring functions make possible. You can tailor the results of the query specifically to what you want, 
and you can do all that work on the server side without having to send a lot of data over the wire. 
...
Listing 10.15 shows how we can project a method directly to the client, doing what's probably the world's most ridiculous localization effort.
...
The result of this query can be seen in Figure 10.10, where you can see that different documents have different shapes. I had a lot of fun writing the query in Listing 10.15, but I wouldn't want to have to deal with it in my code. It would probably be too confusing. 

![RQL query results for the projecting via a method call](./Ch10/img10.png)
...
> **Querying by ID**
>
> If you look at Listing 10.15, you'll see an interesting type of query. There, we're querying by document ID and specifying a projection. This may
...
>
> Querying by ID is handled differently. If the query optimizer can see that your query is using an ID, then instead of going through an index, it'll fetch
...
modeling, you shouldn't usually want to join. But RavenDB has ways to enable that scenario, and they're discussed in the next chapter. 

That said, it can be very useful to grab some data from related documents during the query. Consider a query to show the list of recent orders. We can query it using `from Orders where Company = 'companies/1-A'`, and the result is shown in Figure 10.11.

![The results of simple query on orders](./Ch10/img11.png)

As you can see in Figure 10.11, the output is the document. That document includes useful fields such as `Company` and `Employee`. This is great, but if we intend to show it to a user, showing `employees/6-A` is not considered a friendly act. We can ask RavenDB to include the related documents as
well, as you can see in Listing 10.16.
...
Figure 10.12 shows the output of this in the Studio, and you can see that we've gotten the company and the employees back from the query. We've already 
...
![Viewing included results in the Studio](./Ch10/img12.png)

Including related documents is very useful, but if we just intend to show the information to the user, sending the full documents back can be a waste. We can do better by using `load`. In Listing 10.17, you can see a small example of pulling data from multiple documents and returning that to the user.
...
`CompanyName: load(o.Company).Name` instead and gotten the same results. Load is also supported for collections, as you can see in Listing 10.18.
...
In Listing 10.18 we pull all the related products from the lines. Note that we indicate to RavenDB that the result is an array by using `products[]`,
...
RQL supports the `order by` clause, which allows you to dictate how results are sorted. Take a look at Listing 10.19 for an example. 
...
The query in Listing 10.19 reads like a typical SQL one, and that's by design. However, you need to be aware of a very important distinction. Consider
the query in Listing 10.20.
...
The query in Listing 10.20 is very nearly the same exact query as the one in Listing 10.19. However, the sort order is different. Why does this matter? It matters because, in both cases, we used the same index: "Auto/Employees/ByAddress.CountryAndFirstNameAndLastName," in this case. This is one of those
cases where you needed to have experienced the pain to understand why this is such an important detail.

Typically, databases will use an indexing format that allows the database to quickly answer specific `order by` clauses. 
Similar to the phone book we discussed earlier in the chapter, they can only be used for the purpose for which they were created. 
It's possible to use the phone book index to answer the query in Listing 10.20, but it isn't in Listing 10.19. 
RavenDB, however, doesn't use such single purpose indexes. It indexes each field independently. 
That means it's able to answer both queries (and any combination thereof) using a single index.
...
query pipeline corresponds to its place in the query as you type it. You can see that, in the queries in Listing 10.19 and 9.20, the `order by` comes after the
...
* `order by spatial.distance()` &mdash; useful for spatial queries which will be discussed later in this chapter.
...
`from Products order by PricePerUnit desc`. If you run it, you'll get some peculiar results, as shown in Figure 10.13.

![Unexpected ordering with numeric order by](./Ch10/img13.png)

The issue is that RavenDB doesn’t know the type of the PricePerUnit field, and therefore it defaults to lexical ordering, which means that 7 will show before 62.5. 
In order to get the actual descending results from our `order by` clause, we need to let RavenDB know what kind of sorting we want to do. 
An example of how to do just that can be seen in Listing 10.21.
...
The `as double` in Listing 10.21 instructs RavenDB to sort the results as doubles. You can also use `as long` to specify that the data should
...
Spatial searches allow you to search using geographical data. We'll explore them in more depth in Chapter 11, 
...
that information to query for nearby employees that can pick you up if you come through the Seattle airport. Listing 10.22 shows just how to tickle
...
The query in Listing 10.22 results in two matches: Nancy and Andrew. It's pretty simple to understand what the query is doing, and I wish most spatial
...
employees that live within that area. You can see the polygon on a map in Figure 10.14. 

![Polygon for the London Orbital](./Ch10/img14.png)

You can see how we query RavenDB using this polygon in Listing 10.23.
...
The actual polygon in Listing 10.23 is unlikely to be a very accurate representation of the London Orbital (I drew it by hand, and quite roughly,
...
operation using the basic building blocks that RavenDB provides. We'll touch more on spatial queries in Chapter 11, and they're covered in detail in the online documentation. 
...
underlying implementation is the same. Aggregation in RavenDB happens during indexing, not during query, and as such it is much cheaper than aggregation queries in other databases. 

Let's start by looking at the simple aggregation query in Listing 10.24. 
...
interesting implications for your system. When using an aggregation query you don't need to handle caching or have a scheduler to recompute results, all that is done for you. 

Let's see why this is so important. Consider the query in Listing 10.24. In a relational database, this kind of query is actually quite expensive. Even assuming we have an index on the `Company` field, the relational database will need to go over all the results and count them, filter those that don't have enough, order and sort them.
...
Indexing in RavenDB will always happen in the background. This has been mentioned a few times in this book, but it's important to call it out
...
completes the write operation. Instead, it writes the document data and completes the write operation as soon as the transaction is written to disk, scheduling 
...
You can also request that RavenDB waits until the index is up to date with the current changes by calling `WaitForNonStaleResults` on the 
...
user has a reasonable expectation to see the change they just made, and the query immediately after the save might be fast enough that it gets the index before it had a chance to complete indexing the new data.
...
Creating or modifying a document is not always followed by an immediate query for a list of items. In all of those cases, there's no real reason for you to wait until the indexes are done indexing. Your data is now on RavenDB, and the ACID nature of the database ensures that it's safely stored on disk.
...
Beyond the simple equality and range queries, we showed how you can use advanced features such as full text search and spatial queries. All of the queries that we looked at in this chapter used RavenDB's automatic indexes, and while their features are quite rich, 
...
is allowed, it _does_ add to the cost of the query, as we need to run the JavaScript code on the results. Usually this isn't an issue, but if you have 
a very complex logic or computation, that can impact the query performance. 

Queries in RavenDB can project data from other documents or include the whole document on the way back from the server, reducing the need to do another 
network round trip to the database. This can be done using `load`, which gives you access to a related document, or using `include`, which lets RavenDB
...
etc.), with RavenDB, you can just make the query. The database will take care of returning the results and update the final tally whenever 
