...
> using `LoadDocument` to refer to many different documents, that's going to generate good distribution of the data. Whenever a 
> referenced document is modified, we'll need to index just a few extra documents to catch up with the updates. 
...
Remember that the reindexing will happen on any changed in the referenced document. In Listing 10.28, we are only using the
...
RavenDB is schemaless. You can store any kind of data in any way you want, but that stops when it comes to querying. You do need
to know what you're querying on, right? Of course, you can query on fields in a dynamic fashion, and it will work because
the query optimizier will add the new field you just queried onto the index. But adding a field to an index requires us to reindex 
...
and whose value is the employee's `LastName`. This is here as an example of dynamic fields that are created explicitly. With the index
