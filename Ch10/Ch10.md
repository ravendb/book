...
In the next chapter, we'll talk about Map/Reduce indexes and in the one after that, we'll dive into the actual details of how indexes are implemented in RavenDB and the wealth of metrics that show how 
...
For example, using the sample data set, you want to search for employees using the name of their _manager_. In general, I dislike such
requirements. I would much rather have the user first search for the appropriate manager and then find all the employees that reports
to this manager. From many perspectives, from the purity of the model to the user experience you can provide, that is the better choice.

In particular, searching employees by a manager's name leads to confusion about which manager you are talking about if more than one
manager have the same name. Even so, sometimes this is something that you just have to do. The users may require it to behave in this manner 
because the old system did it in this way or maybe the model you use actually call out for exctly this need. So how _would_ you do it?

One way to handle this requirement is to store the related document field on the relating entity. In other words, store the manager's
name in the employee document. That _works_, but it will rightly raise eyebrows when you look at your model. This isn't a value that we
want to freeze in time, such as a product's price or its name in the `Orders` collection. A manager's name is their own and they are 
free to modify it as they wish. If we would store that name in the employee document, it would mean that we have to update the employee
document whenever we update the manager. That is... not so nice, even if RQL makes it easy, as you can see in Listing 10.27.
...
The only thing that is of interest in Listing 10.27 is the passing of parameters to both the query and the `update` clause. Other than
that, it is perfectly ordinary query. However, having to do such things will likely result in the Ghost of "You Shoulda Normailized That"
haunting you. In particular, while this can be a viable solution for certain things, it isn't elegent and it goes against the grain of
everything that is usually being thought in database design.
...
I've created Listing 10.28 as an index named `Employees/Search`. And I can query it with the query in Listing 10.29:
...
As you can see, we are able to query the related information, but how does this work? In Listing 10.28 you can see the use of an unfamliar
...
document and allow you to index its values. Note that we need to specify which collection we are loading the document from.
...
`Smith`. And the execute the query in Listing 10.29 again, you'll find no results. But if you'll run it on `ManagerLastName = 'Smith'`
you'll find the missing documents. 

In other words, when using `LoadDocument` in the index, you can index data from related document and it is _RavenDB_'s responsability to
keep such related data up to date. In fact, this is why we need to know which collection you are loading the document from, so we can
watch it for changes (in a similar way we watch the collection that we are indexing). The loaded documents can be from the collection we
are already indexing (as is the case in Listing 10.28) or an unrelated collection.

> **The cost of tracking related documents**
>
> The `LoadDocument` behavior doesn't come without costs. These come in two areas. First, during indexing, we need to load the 
> relevant document, which may require us to read a document that currently reside on the disk, skipping many of the performance
> optimizations (such as prefetching) that we apply when we need to index a batch of documents. This cost is usually not too much
> and is rarely an issue.
> 
> Of more cocnern is the fact that whenever any document in a collection that is referenced by `LoadDocument` is changed, the index
> need to scan that index as well to find whatever or not the index need to see if there have been any references to that document
> that now need to be re-indexed. This is usually the more expensive portion of this feature.
>
> It's worth noting that a primary factor in the cost of `LoadDocument` is the distribution of the referenced data. If an index is
> using `LoadDocument` to refer to many different documents, that is going to generate good distribution of the data. Whenever a 
> document that is being referenced is modified, we'll need to index just a few extra documents to catch up with the updates
> to the referenced document. 

When you consider using the `LoadDocument` feature, first consider whatever this is used to paper over modeling issues. In particular,
`LoadDocument` allow you to do "joins" during the indexing process. That can be very useful or lead you down a problematic road, 
depending on your usage. In particular if a large number of documents are referencing a single document (or a small set of them), 
then whenever that referenced document is changed, _all_ the documents referencing it will also need to be re-indexed. In other
...
These delays will only impact the specific index using `LoadDocument` and will have the affect of making it do more work, but other
...
The whole referenced document vs referencing documents can be a bit confusing, so an exmaple might be in order. In Listing 10.28
we have the `Employee` referencing the manager using the `ReportsTo` field. In other words, the referencing document is the 
`Employee` and the referenced is the manager (in this case, also a document in the `Employees` collection). The topology of the 
...
will be updated, the employees managed by this document will be reindexed. However, consider the case where _all_ employees are 
reporting to the same manager, and that manager's document is updated, we now need to index _everything_ that referenced it, which
can be a huge number of documents.

Remember that the reindexing will happen on _any_ changed in the referenced document. In Listing 10.28, we are only using the
...
`LoadDocument` is a very powerful feature and its deserve its place in your toolbox, but you should be aware of its limitations 
and costs. Unfortunately, it is used, all too often, as a means to avoid changing the model, which will usually just defer a problem,
...
RavenDB is schemaless. You can store any kind of data in any way you want, but it stops when it comes to querying. You do need
to know what you are querying on, right? Of course, you can query on fields in a dynamic fashion, and it will work, because
the query optimizier will add the new field you just queried on to the index. But adding a field to an index requires us to reindex 
it, and that isn't quite what we want. We want to be able to say: "This index will cover any of the fields that I'm interested in"
...
index and how RavenDB allows to index dynamic fields. 
...
There are several things going on with the `Employees/DynamicFields` index in Listing 10.30. First, we use the `_` variable name as 
...
We are also using the `CreateField` method twice. The first time we use it to index all the fields inside the `Address` object. 
...
The second time we call `CreateField` is much stranger. This create a completely dynamic field whose name is the employee's `FirstName`
and whose value is the employee's `LastName`. This is here as an example for dynamic fields that are created explicitly. With the index
...
It is still recommended that you'll mostly use static indexing, mostly because it is easier to reason about an work with. While RavenDB
doesn't actually care for the field names you have, it is usually easier if you'll use dynamic fields only when needed. In particular,
we have seen users that used dynamic fields to index every single field in their documents. That _works_, and sometimes you want to do 
this, but in most cases it was an issue of "we might need this in the future" and was rarely, if ever, used.
...
We started this chapter by discussing _what_ are indexes in RavenDB. We saw that, like an onion^[Or an Oger.], indexes come in layers.
There is the index definition, specifying what and how we should index, the index on disk and the actual indexing process. We started
...
all the way to the actual processing that happen for the data being indexed. We looked at how the values we index are broken into terms
according to the specified analyzer and then are stored in a way that allow quick retrieval of the associated docuemnts. 
We also saw that we can store information directly in the index, although that is reserved for special cases.

Once we got what indexes are, we started to look at how we can use static indexes in RavenDB. Multi map indexes allow us to index more
...
is a world into itself, but RavenDB contains everything you can wish for in this regard.

We looked at how full text search analysis process text and how it is exposed in the index terms in the studio. RavenDB allow you to utilize
full text search both using purpose built methods  (such as `StartsWith()`,`Search()`, etc) and a lower level interface exposed via the 
`Lucene()` method that allow you to interface directly with the Lucene engine.

A nice trick you can use in indexing is to merge multiple fields from the index into a single field in the index entry, which allow you to
...
like Google, and RavenDB make such behavior quite easy to implement.
...
Facets are a way to disect the result set you get from a query and gather additional information from it. Instead of forcing your users to
go through thousands or millions of results, you can have RavenDB slice & dice the results so the users can dig deeper and find exactly
what they are looking for. These last three features (Suggestions, More Like This and Facets) are primarily used when you need to directly
...
are defined, what you can do with them and dived into how they are actually implemented and the varying costs of the levels of accuracy that
you can get from spatial queries. 

We peeked into how RavenDB allows you to query related data, by calling `LoadDocument` at indexing time. This moves the responsability of 
...
Finally, we looked at how RavenDB allow you to define indexes on dynamic data, without having to have _any_ common structure between the 
indexed documents. This is useful for user generated data and when you are working on highly dynamic systems. Instead of the usual complexity
that is involved in such systems, with RavenDB you can just make everything work and the database will allow you to store, retrieve and query
the data in any way you want. 

In the next chapter, we are going to look into RavenDB's map/reduce indexes, what they can do for you and how they actually work.