...
we typically just use `__`, `___`, etc for ]. We are also using the `CreateField` method twice. The first time we use it to index all
the fields inside the `Address` object. In this way, we aren't explicitly listing them one at a time and if different documents
...
and whose value is the employee's `LastName`. This is here as an example for dynamic fields that are created explicitly. 
