...
> That means that even if the index itself hasn't caught up to changes, the data it returns will have everything up to date.
...
It is worth noting that a static index is just a Linq statement, which means that you have a lot of power in your hands. And index can 
