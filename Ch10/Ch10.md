...
> put into these fields (a city, a country or a product id) are all not terms that we wish to break up. In this case, using full text search
> on those fields would break them apart too much. For example, if we have a city named: "New York", that is the city name and we don't want
...
```{caption="More like this query limited to a single field"}
from index 'Orders/Search' 
...
We are now going to look at another facet of the RavenDB querying capabilities, facets and how we can use them to get all _sorts_ of information
