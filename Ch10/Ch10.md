...
If there's a query you only want to perform once, you're probably better off just scanning through the entire dataset since you'll do anyway when creating an index. But if you intend to query more than once, an index is a fine investment. Consider the 
...
happened at indexing time, and the cost of the query in Listing 10.7 is the cost of a seek to the relevant location in the index and plust the cost of sorting the results according to the indexed value. 
...
term. In other words, if we looked for `index.Name['Arava Eini']`, we wouldn't find anything in the index. This is because we search for the terms in the index. And during indexing, we broke the name to its constituents' parts and made all letters lowercase. At query time, we can apply the same
