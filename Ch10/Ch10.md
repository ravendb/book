...
than actual values from the document. Also consider Listing 10.5, which shows how to query the new index.
...
whose output is a computed field ("Name"), which in turn is the result of a concatenation of two values from the document. In Listing 10.5, you can see that we're querying
...
![Sample document for playing with full text search](./Ch10/img03.png)
...
into another 32 buckets, and so on. You can play with geohashing in an interactive manner using the following site:
...
document whenever we updated the manager. That is...not so nice, even if RQL makes it easy, as you can see in the patching done in Listing 10.27.
