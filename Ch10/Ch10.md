...
One way to handle this requirement is to store the document field on the related entity. In other words, store the manager's
...
> relevant document. That may require us to read a document that currently reside on the disk, skipping many of the performance
...
> needs to scan that index. It also assesses whether or not the index needs to see if there have been any references to that document
> that now need to be reindexed. This is usually the more expensive portion of this feature.
>
