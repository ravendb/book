...
In the next chapter, we'll dive into the actual details about how indexes are implemented in RavenDB and the wealth of metrics that show how 
...
Indexes are how RavenDB can answer questions about your documents without scanning the entire data set each and every time. 
...
between the terms that are indexed and the actual documents that contain them. After the first indexing run, the index will keep that map current as updates and deletes happen on the database.
...
If there's a query you only want to perform once, you're probably better off just scanning through the entire dataset since that's what creating an index will have to do anyway. But if you intend to query more than once, an index is a fine investment. Consider the 
...
I'll be the first to admit that this can be quite confusing, but RavenDB actually has several different things internally called "index." At the lowest level, we have Voron indexes, which is how RavenDB organizes the data on persistent storage. As a user, you
...
async nature of higher level indexes in RavenDB, these indexes (sometimes also called storage indexes) gurantee full consistency to the reader. You'll rarely be able to see them in use or affect them in any way, but they're crucial for the well being and 
...
also stored in Voron. But instead of being part of the storage engine, they're stored _inside_ the storage engine to ensure the index data is transactionally safe and can recover from an error such as an abrupt shutdown.
...
The good new about this naming mess is that you're rarely going to need to think about any of that. As far as the outside
...
to take over and select the best index to use, we're explicitly specifying that we want to use "MyFirstCollection" index.
...
That doesn't seem like such a good idea until you realize the shape of the index and the shape of the source document doesn't
...
than the actual values on the document. Also consider Listing 10.5, which shows how to query the new index.
...
trivial. What makes this interesting is that this allows us to run a query such as the one in Listing 10.7.
...
>
> In particular, usage of date time functions or `random`, as well as trying to access external resources, is not allowed. This allows RavenDB
> to assume that identical inputs will produce identical outputs and is important for reindexing and updates. 
...
simple, as it turns out. All RavenDB needs to do is simply run the udpated document through the indexing function again and index
...
The `search(Name, 'Arava')` will be then be translated into what's effectively an search on `index.Name['arava']` to get the proper
matches. 

This isn't how the index works _at all_. But it's a very good lie because it allows you to reason about what's going on and make intuative
...
term. In other words, if we looked for `index.Name['Arava Eini']`, we wouldn't find anything in the index. This is because we searched the
terms on the index. And during indexing, we broke the name to its constituents' parts and lowercased it. At query time, we can apply the same
...
querying on the output of the indexing function, and that may bear little resemblence to how your index looks. The `Total` field in 
Listing 10.6 serves as a good example for this feature. The documents do not have a `Total` property, but we can compute it during indexing
and query on it.
...
> a simple Linq expression and allows you to run any code whatsoever on the server. This is mostly meant to allow you to perform complex logic in the 
...
want to search defined on that index. It's better to have fewer and bigger indexes than smaller and more indexes.]
...
default, RavenDB will store only store enough information in the index to handle the query&mdash;not to get data out of the index. So as it stands,
...
This setting tells RavenDB that it needs to store the value itself (and not just the parts that it indexed) in such as way that we can later
retrieve it.
...
Stored fields are used primarily to store the result of such computations, but there's a small perfromance advantage in projecting everything
...
find. We'll typically want to do a smarter search...a_full text search_, one might even say.
...
The analyzer for full text search is defined on a field or fields on the index, and it determines how RavenDB will break apart the text into individual 
terms. We've seen such example in Listing 10.9, but let's consider the following string: `"The white knight is running to the princess's tower
to slay the dragon"`. How would full text search operate on such a string?

#### Full text search analysis

RavenDB will hand over this string to the analyzer and get a list of terms back. This is an extremely simplistic view of what's actually going on.
...
by using lowercase in the input string (with all the usual casing required for Unicode-aware programs, of course). It's important to note that 
...
With that done, you can click `Save`, and we're done. You can now go to the index `Terms` to see the different indexing methods that were used 
...
of breaking it up into individual terms, filtering out stop words and stripping the posessive "s" out. 
...
* `where lucene(n.Search, ' "running knight" ')`&mdash:not a match. There is no `running` followed by `knight` in the text.
...
cover but are quite interesting to read about.^[See the _Lucene in Action_ and _Managing Gigabytes_ books, recommened in the previous chapter.]
...
`searc()` or `lucene()`, we're perfoming more complex operations, which allows us to do more interesting queries.
...
system to recognize who they're talking about and take them to the right place. It may seems like a small thing, but these are the kind of things that
...
In the sample data, we have the `companies/8-A` document. It's for a company in Spain, whose owner name is: `Martín Sommer`. Note that diatric over 
...
them to simulate common errors. Those all go into an index that's queried during query. This does increase the cost of indexing, but the cost of 
...
that looks like this." In bug tracking, it might be finding a previous occurance of a bug. With a product catalog, that might be finding another item
...
> quickly. But while there's quite a lot tuning that you can do (see the online documentation for the full details), it's 
...
posts, emails and the like. It's somewhat of less use for static fields, but there are still quite a few interesting usecases for this feature.
...
Setting a field to use a term vector tells RavenDB that we need to store enough information about each index entry to not only be able to tell from a
...
we have, the higher the rank of a result. So this explains how we get more like this. We query on the matches, and
...
managable number. It also gives me additional information, such as the fact that there are no matches with the type "Show" in the results. You can see another
...
This section isn't going to be an exhuastive study of how spatial indexing works, nor will it dive too deeply into the actual implementation. The 
...
documentation.
What this section _is_ going to do is to give you a good idea of how RavenDB performs spatial queries&emdash;enough so that you'll be able to reason
...
In other words, with spatial queries using geohash (or quad), there are two stages to the query. First, do a rough match on the shapes. This is what you can see in the index terms. Second, have the actual spatial geomtery check to see if the shape matches the query.
...
the name quad). Quad buckets are always squares, while geohash buckets can be rectangular. This might make a difference if you're using heatmaps and want a more predicatable zoom in/out. 
...
dispaly spatial data, you'll probably want to select a mode that works best with whatever mapping component you're using. 
...
me all the schools in this district," that's something quite different, and you'll care about the exact shape that the query is 
processing.
