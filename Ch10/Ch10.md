...
terms on the index. And during indexing, we broke the name to its constituents' parts and made all letters lowercase. At query time, we can apply the same
...
`searc()` or `lucene()`, we're performing more complex operations, which allows us to do more interesting queries.
