...
the entire query a single string. The generic parameter that's used in the `RawQuery` method is 
...
> method. 
> 
> It's _strongly_ recommended that you only use parameters and you don't build querying using string
...
Consider the code in Listing 12.17, which uses `RavenQuery.Raw` to carefully poke a RQL snippet into
the Linq query. 
...
We already looked at this in [Chapter 10](#advanced-indexing), in the section about full text indexes, so
this shouldn't come as a great surprise. We need to mark the `Name` field as full text search field. But
...
> up by `IndexCreation` automatically. It just give us the maximum amount of flexibility in all aspects of the
> index.
...
> 
> In the locked mode, a new index definition would return successfully but will not actually 
