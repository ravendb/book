...
that are already in the index) of five minutes. An updated index can't just pick up from where the old index definition
left off. For example, we might have added new fields to the index, so we need to re-index all the relevant documents. But if 
...
scratch (which will impact queries), RavenDB keeps the old index around (including the indexing of new documents and answering
...
its work, it will be automatically replaced with the new version. Of course, you can also force an immediate replacement if
