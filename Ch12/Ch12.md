...
has a dedicated thread for all indexing work. This allows us to isolate any work being done to this thread and gives
...
window, you can click on the `Memory Stats` and see the memory utilization per thread.
...
> **Indexing in batches**
>
> RavenDB typically needs to balance throughput vs. freshness when it comes to indexing. The bigger the batch, the faster
...
> as many documents at once as it can. 
>
...
system level to hint at what should be done first. If you increase the index priority, RavenDB will update the indexing thread
...
server. This is true for the indexing creation, and when it's being run to update itself on each change to the data, we don't
...
> it indexes them all. What happens then?
> 
...
Let's assume we have an index deploy duration (from the time it's created to the time it's done indexing all the documents
that are already in the index) of five minutes. An updated index cannot just pick up from where the old index definition
