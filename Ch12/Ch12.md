...
that doesn't starts with `from index ...` is fair game for the query optimizer). The query optimizer will attempt to 
find an index that can answer the query being asked, but if it fails to find any appropriate indexes, it will go ahead and
create a new one.

One very important aspect of that is the query optimizer isn't going to create an index blindly. Instead of only 
...
3. Let's take a look at all the queries that were made against the same collection as the one that's now being queried and see what would be the optimal index to answer all of these queries. 
...
queries and figure out if they can use the existing indexes. If it can't, the optimizer will create a new index for them. All existing queries will
...
You can export that knowledge from the test machine and import it into the production cluster. The new indexes
...
the `Include Indexes` is selected and click the `Export Database` button. 
...
externalize a _lot_ of information about the indexing processes and make it available to you, mostly via the studio in `Indexes` and then the  `Indexing Performance` page. You can see a sample of what it looks when the 
system is indexing in Listing 12.8.

![Visually exploring details about the indexing actions (and their costs)](./Ch12/img08.png)

The timeline view in Figure 12.8 shows several indexes and the running at once. (Solid colors means the index batch is 
complete, stripes means this is an actively executing index) And you can hover over each of the steps to get more
information, such as the number of documents indexed or the indexing rate that's shown in Figure 12.8.
...
discussed (see Figure 12.3 for how this looks like in the studio) and notice that a particular indexing thread is 
...
is shown in Figure 12.9, where you can see the exact costs of suggestions during indexing.

![Drilling down into particular operations (such as "Suggestions") can provide insight into what's costly](./Ch12/img09.png)
...
the operator's attention when there are issues, as shown in Figure 12.10.
...
not using streaming. (Streaming queries were previous discussed in [Chapter 4](#advanced-client-api).) This can lead
...
it got was `null`) and the value of `HasManager` is going to be false because there's manager for `employees/2-A`.
...
In this way, a whole class of common issues is simply averted. On the other hand, the index _will_ contain an
...
Some kinds of errors don't really let us recover. Consider the code in Listing 12.22. The index itself is not 
...
going to fail. Let's see how RavenDB behaves in such a case. Figure 12.11 shows how this looks in the 
studio.
...
There are two very important details in that error message: we know what document caused this error
...
is common, it may be a while before your users' complaints of "I can't find that record" makes you 
