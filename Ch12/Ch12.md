...
amount of data using MapReduce. What we haven't talked about is how you'll work with indexes in a typical business
application.
...
button. Click the `View C# Classes` link, copy the code to a file call `Entities.cs` and save it in the `Northwind` app 
folder.  
...
As you can see, the major difference is that we're now querying on `Orders/Totals` index and we pass that as a string to the 
...
itself to provide use with a strongly typed answer for that. We'll recreate the `Orders/Totals` index in our C# code, as shown
...
projection in the `select`. As long as we're working on RQL directly, we don't really notice, but how do we deal with such
...
As you can see, even though Listing 12.9 has the `orderby` clauses on different locations and operating on different types, the
...
On the client side, a MapReduce index is very similar to a the simple indexes that we've already seen. The only difference is that
...
However, with a MapReduce index, we have two types. One is the type that the `Map` will operate on, just the same as we had before. But there's another type, which is the type that the `Reduce` is going to work on. As you probably expected, we also pass
...
class near the index using it). However, we're also using the inner class for this type as the generic argument for
the base type.
...
`IndexCreation.CreateIndexes(assmbly, store)` again. Go ahead and inspect the new index in the studio. Remember, the index name
in the RavenDB studio is `My/Products/Sales`. 

With the index on the server, we can now see how we can query a MapReduce index from the client side. This turn out to be 
...
> language. The idea is that from the client code, you'll gain the benefits of strongly typed languages such as IntelliSense, 
...
Within RavenDB, RQL queries give you the most flexibility and power. Any other API ends up translating to RQL, after all.
...
I mentioned briefly earlier that it is typical to deploy indexes using `IndexCreation.CreateIndexes` or its async equivalent
`IndexCreation.CreateIndexesAsync`. These methods take an assembly and the document store you are using and scan the assembly
for all the index classes, then they create all the indexes they found in the database.

During development, it is often best to call one of these methods in your application startup. This way, you can modify an 
index, run the application and the index defintion is automatically updated for you. It also works great when you pull changes
from another developer, you don't have to do anything to get the right environment setup.
...
nothing at all. Only when there are changes to the indexes will it actually take affect.

> **Locking indexes**
>
> Sometimes you need to make a change to your index definition on directly on your server. That is possible, of course, 
> but you have to be aware that if you are using `IndexCreation` to automatically generate your indexes, the next 
> time your application will start, it will reset the index definition to the original 
> 
> That can be somewhat annoying, because changing the index definition on the server can be a hotfix to solve 
> a problem or introduce a new behavior, and the index reset will just make it go away, apparantely randomly.
> 
...
> 
> In the locked mode, a new index defintion would return successfully, but will not actually 
...
> idea is that we don't want to your calls to `IndexCreation` by throwing an error.
>
> Note that this is _not_ a security measure, it is a way for the operations team to make a change in the index 
...
RavenDB will make sure that it is created in all the database's nodes. The same also applies for automatic indexes. If the 
...
instances, not just the one who processed this query. 
...
a node that have already created the indexes you are using to one that didn't accept any queries yet. All of the nodes in a 
database group will have the same indexes.

> **Failure modes and external replication**
>
> Index creation being a cluster operations means that it is reliable, it goes through the Raft protocol and a majority of
> the nodes must agree to accept it before it is acknowledged to the client. If, however, a majority of the nodes in the 
> cluster are not reachable, the index creation will fail. This apply to both manual and automatic index creation in the case
> of network partition or majority failure. Index creation is rare, though, so even if there is a failure of this magnitude
> it will not typically affect day to day operations.
> 
> External replication allows us to replicate data (documents, attachments, revisions, etc) to another node which may or
> may not be in the same cluster. This is often used as a separate hot spare, offsite backup, etc. It is important that
> external replication does _not_ also replicate indexes. Indexes are only sent as a cluster operation for the database
...
side of indexing, how they actually work. 

#### How indexes do their work?

This section is the equivalent of popping the hood on a car and examining the engine. For the most part, you shouldn't 
have to do that, but it can be helpful to understand what is actualy going on. 

An index in RavenDB is actually composed of:
...
* Dedicated index thread that does all the work for the index.

What is probably the most important from a user perspective is to understand how this all play together. An index in RavenDB
has a dedicated thread for all indexing work. This allows us to isolate any work that is being done to this thread and gives
the admin better accountability and control. In the studio, you can go to `Managed Server` and then click on `Advanced` and 
...
In Figure 12.3 you can see how much processing time is taken by the `Orders/ByCompany` indexing thread. In the same `Advanced`
windows, you can click on the `Memory Stats` and see the memory utilization per thread.

A dedicated thread per instance greatly simplify operational behaviors and allow us to apply several important optimizations.
It means that no index can interfere with any other index. A slow index can onl affect itself, instead of having a global
affect on the system. It simplify the code and algorithms required for indexing, because there is no need to write 
multi-threading safe code. 

> **Indexing in batches**
>
> RavenDB typically needs to balance throughput vs. freshness when it comes to indexing. The biggest the batch, the faster
> it takes to index each document. But we only see the updates to the index when we complete the batch. During initial
> creation, RavenDB typically favors bigger batches (as much as the available resources allow) and will attempt to index
> as many documents at once. 
>
> After the index completed indexing all the documents it covers, it will watch for any new or updated documents and index
> them as soon as possible, without waiting for more updates to come. The typical indexing latency (the time between a 
> document update and the time the index has committed the batch including this document) is measured in milliseconds on
> most systems.

This design decision also allows RavenDB to prioritize tasks more easily. RavenDB uses thread priorities at the operating 
system level to hint what should be done first. If you increase the index priority, RavenDB will update the indexing thread
priority at the operating system level. You can see how this is done in Figure 12.4.

![Changing the indexing priority will update's the indexing thread priority](./Ch12/img04.png)

By default, RavenDB prioritizes request processing over indexing, so indexing threads start with a lower priority than 
request processing threads. It means that requests completes faster and indexes get to run when there is capacity for 
them (with the usual starvation prevention mechanisms). 
...
will not overwhelme the system by taking too much CPU or saturating the I/O bandwidth we have available. 
...
server. This is true for the indexing creation and when it is being run to update itself on each change to the data, we don't
make distinctions between the two modes. 

> **What keeps the indexing thread up at night?**
>
> When you create a new index, RavenDB will spawn a new thread which will start indexing all the documents covered by the
> index. It will start going through the documents in batches, trying to index as many of them in one go as it can, until
> it got them all. What happens then?
> 
> The index then will go to sleep, waiting for a new or updated document in one of the collections that this index cares
> about. In other words, until there is such a document, the thread is not going to be runnable, isn't going to compete
> for CPU time and takes very few resources from the system.
>
> If the indexing thread detects that it has been idle for a while, it will actively work 
> to release any resources it currently holds and then go back to sleep until it is woken by a new document showing up.

The query optimizer's capability to create new indexes on the fly depends on making
sure that a new index on the fly depends on the new index not breaking things while it is being built. Because of this 
requirement, RavenDB is very careful about resource allocations to indexing. We talked about CPU and I/O priorities, but
there is also a memory budget that is applied. All in all, this has been tested in production for many years and it has
proven itself as an extremely valuable feature.
...
offered not just for new indexes, but also when updating existing ones.

#### Side by side

During development, you'll likely not really notice the indexing update times. The amount of data you have in a development
database is typically quite small and the machine is not usually too busy in handling production traffic. In production, 
the opposite is true. There is a _lot_ of data and your machines are already busy doing their normal routine. This means
that an index deploy can take a while.

Let's assume we have an index deploy duration (from the time it is created to the time it is done indexing all the documents
that are already in the index) that takes five minutes. A updated index cannot just pick up from where the old index definition
left. For example, we might have added new fields to the index, so we need to re-index all the relevant documents. But if 
we have a five minutes period in which we are busy indexing, what will happen to queries made to the index during that time
frame?
...
by changing the `Total` field computation and then save the document. Then immediately go to the indexes page and you should
see a sight similiar to what is shown in Figure 12.5.

![Updating an index keep the old definition alive until the new index is caught up.](./Ch12/img05.png)

Figure 12.5 shows an index midway through an update. But instead of deleting the old index and starting the indexing from 
scratch (which will impact queries), RavenDB keeps the old index around (including indexing of new documents and answering
queries) until the new version of the index has caught up and indexed everything. 

This way, you can minimize the effects of updating an index in production. Once the new version of the index has completed
its work, it will be atomatically replaced with the new version. Of course, you can also force an immediate replacement if
you reall need to (`Swap now` will do it). 

### Auto indexes and the query optimizer

We talked about the query optimizer creating indexes on the fly several times now, but now I want to shine a light on the 
kind of heuristics that the query optimizer uses and the logic that guides it. 

At the most basic level, the query optimizer analyzes all the queries that don't specify an explict index to use (antying
that doesn't starts with `from index ...` is a fair game for the query optimizer). The query optimizer will attempt to 
find an index that can answer the query being asked, but if it fail to find any appropriate index, it will go ahead and
create a new one.

One very important aspect of that is that the query optimizer isn't going to create an index blindly. Instead of only 
considering the current query when it is time to create a new index, the query optimizer is also going to weigh in all
the _history_ of the queries that were made against the database. 
...
2. There is no such index, we need to create one.
3. Let's take a look at all the queries that were made against the same collection as the one that is now being queries
   and see what would be the optimal index to answer all of these queries. 
4. Create the optimial index and wait for it to complete indexing.
5. Retire all the automatic indexes that have been created so far that are now covered by the new index.

The idea here is that RavenDB uses your queries as a learning opportunity to figure out more about the operational 
environment and the query optimizer is able to use that knowledge when it need to create a new index.

Over time, this means that we'll generate the optimal set of indexes to answer any query that doesn't use an explicit
index. Furthermore, it also means that operational changes, such as deploying of a new version of your application
with slightly different queries will be met with equanimity by RavenDB. The query optimizer will recognize the new 
queries, figure out if they can use the existing indexes or create a new one for them. All existing queries will
...
your application to production for RavenDB to start learning about the kind of queries that it is going to generate.
During the learning process, there might be several paths taken that you want to skip. 

You can run your application in a test environment, running it through it paces and making the application issue 
all its queries to your test RavenDB instance. That instance will apply the same logic and create the optimal set
of indexes to answer the kind of queries is saw. 

You can take export that knowledge from the test machine and import that into the production cluster. The new indexes
will be built, and by the time you are ready to actually deploy your application to production, all the work has 
...
the `Include Indexes` is selected and click on teh `Export Database` button. 

![Exporting just the indexes from our database.](./Ch12/img07.png)

You can then take the resulting file and import that into the production instance (`Settings` and then to `Import Database`)
...
When it comes the time to understand what is going on with your indexes, you won't face a black box. RavenDB tracks and
externalize a _lot_ of information about the indexing processes and make it available to you. Most of that is available
for you in the studio in `Indexes` and then `Indexing Performance` page. You can see a sample of what this looks when the 
system is indexing in Listing 12.8.

![Visually exploring deatils about the indexing actions (and their costs)](./Ch12/img08.png)

The timeline view in Figure 12.8 shows several indexes and the running at once (solid colors means the index batch is 
complete, stripes means that this is an actively executing index) and you can hover over each of the steps to get more
information, such as the amount of documents indexes or the indexing rate that is shown in Figure 12.8.

This graph can be very useful in investigating what exactly is going on inside RavenDB without having to look through 
a pile of log files. A good example of such a scenario is when you look at the threads details that we previously 
discussed (see Figure 12.3 for how this looks like in the studio) and notices that a particular indexing thread is 
using a lot of CPU time.

You can go into the `Indexing Performance` window and just look at what is taking so much time. For example, you may
be using the Suggestions feature, which can be fairly compute intensive with high update rates. An example of this
can be seen in Figure 12.9, where you can see the exact costs of suggestions during indexing.

![Drilling down into particular operations (such as Suggestions) can provide insight into what is costly](./Ch12/img09.png)

Figure 12.9 shows a fairly simple example, but the kind of details exposed in the timeline can give you a better idea
about what exactly is going on inside RavenDB. As part of ongoing efforts to be a database that is actively trying to 
help the operations team RavenDB is externalizing all such decisions explicitly. I encourage you to look at each of 
these boxes, the tooltips reveal a lot of what is going on, and looking at this kind of view should quickly give you 
a feeling about how much things _should_ cost, so you can recognize when things are out of whack if you are exploring
some issue.

Having a good idea about what is going on during indexing is just half the job. We also need to be able to monitor
the other side, what is going on when we query the database. RavenDB actively monitor such actions and will bring to 
the operator attention when there are issues that require their attention, as shown in Figure 12.10.

![RavenDB will generate operational alerts for slow queries and very large results sets.](./Ch12/img10.png)

Figure 12.10 shows the large result set alert, generated when a query returns a very large number of results while
not using streaming (streaming queries were previous discussed in [Chpater 4](#advanced-client-api)). This can lead
to higher memory utilization on both client and server and is considered a bad practice. RavenDB will alert you to 
this issue, provide the exact time and the query that caused it, so you can fix the problem.

In the same vein, very slow queries are also made explicitly visible to the operators, because they are something 
that they probably need to investigate. There are other operational conditions that RavenDB monitors and will 
bring to your attention, anything from slow disk I/O to running out of disk space to network latency issues, we'll
discuss alerts and monitoring RavenDB in much more depth in the next part of the book, so I'll keep it for there. 

### Error handling in indexing

Sometimes, your index run into an error. RavenDB actually goes to great length to avoid that. Property access 
inside the index will propogate nulls transitively. In other words, you can write the index shown in Listing 12.21
...
it got was `null`) and the value of `HasManager` is going to be false, because there is manager for `employees/2-A`.
However, just one line below we access the `manager` instance, which we know is `null`. 

Usually, such an operation will throw a `NullReferencException`. RavenDB, however, re-write all references so they 
use null propagation. The actual mechanism this is done is a bit complex, and out of scope for this topic, but you
...
In such a way, a whole class of common issues is simply averted. On the other hand, the index _will_ contain an
name for a manager for `employees/2-A`, it will be `" "`, because the space is always concatenated with the values
and `null` contatenated with a string is the string. 

Some kind of errors do not really let us recover. Consider the code in Listing 12.22. The index itself is not 
really interesting, but we have an `int.Parse` call there on the `PostalCode` property.
...
The `PostalCode` property in the sample data set is numeric for employees from the Seattle and alpha numeric for 
employees in London. This means that for about half of the documents in the relevant collection, this index is 
going to fail. Let's see how RavenDB behaves in such a case. Figure 12.11 shows how this looks like in the 
studio.

![Indexing errors and an errored index in the studio](./Ch12/img11.png)

We can see that the index as a whole is marked as errored. We'll ignore that for the moment and focus on the 
`Index Errors` page. If you'll click on it you'll see a list of the errors that happened during indexing. You 
...
There are two very important details in that error message. First, we know what document caused this error
and we know what this issue is. These details make it very easy to figure out what the problem is. Indeed,
looking at `employees/5-A` we can see that the value of the `PostalCode` property is `"SW1 8JR"`. Not really
something that `int.Parse` can really deal with.

So the indexing errors give us enough information to figure out what happened, that is great. But what about
...
Now things become much clearer. An index is allowed to fail processing _some_ documents. Because of the dynamic
nature of documents in RavenDB, you may get such failures. However, allowing such failures to go unattended is
dangerous. An error in indexing a document means that this particular document is now indexed. That may seem
like tautology, but it has important operational implications. If the document isn't indexed, you aren't going
to see it in the results, it is "gone". 

While the indexing error is quite intentionally very visible, if you are running in an unattended mode, which
is common, it may take a while until your users' complaints about "I can't find that record" will make you 
check the database. What would be worse is if you have some change in the application or behavior that can cause
_all_ new documents to fail to index. Because of that, an index is only allowed a certain failure rate and we'll
mark the entire indexed as errored when that happens.

An index in the error state cannot be queried, and will return an immediate error (similar to the error text 
above) with an explanation on what is going on. With an explicit error, it is much easier to figure out what
is wrong and then fix it. 

### Summary

We started this chapter by discussing indexes deployments. From the baseline of defining indexes using strongly
typed classes to the ease of use of `IndexCreation.CreateIndexes` to create all those indexes on the database.

We re-implmented many features and scenarios that we already encountered, but this time we did that from the 
client's code perspective. Building indexes using Linq queries is an interesting experience. We started from
simple indexes and map-reduce indexes with `AbstractIndexCreationTask<T>` and then multimap and multimap-reduce
...
toward more flexiblity with ever more complex queries. In both Linq queries and the strong typed indexes we 
talked about the fact that RavenDB is not actually aware of your client side types, nor does it really care
about them. 

All the work to make strongly typed indexes and queries on the client side is purely there so you'll have good
complier, intellisense and refactoring support inside your application. In the end, both queries and indexes
are turned into strings and sent to the server. 

We looked at how we can directly control the `IndexDefinition` that is sent to the server, giving us absolute
power to play with and modify any option that we wish to. This can be done by using the non generic
...
`RawQuery` (and remember to pass parameters only through `AddParameter`) to poking holes in Linq 
...
a reliable cluster operation, with a majority consensus using Raft) and what this means (not available for 
external replication, require a majority of the nodes to be reachable to create / modify an index). 
...
as well as define a memory budget to control how much RAM this index will use.
...
The process of updating an index defintion got particular attention, since this can be of critical importance
in production system. RavenDB updates indexes in a side-by-side manner. The old index is retain (and can even
index new updates) while the new index is being built. Once the building process is done, the old index is 
removed in favor on the new one in an atomic fashion.

We briefly looked at the query optimizer, not so much to understand what it is doing but to understand what
it _means_. The query optimizer routinely analyze all queries and is able to create indexes on the fly, but
the key aspect of that is that it uses that information to contiously optimize the set of indexes you have.
...
about how it index documents in an easy to consume manner, using the `Index Performance` page and it 
...
We closed the chapter with a discussion of error handling. How does RavenDB handles indexing errors, how
they are made visible to the operators and what kind of behavior you can expect from the system. RavenDB
will tolerate some level of errors from the index, but if there are too many indexing issues, it will 
decide that such is not acceptable and mark the whole index as failing, resulting in any query against
this index throwing an exception.

This chapter has marked the beginning of the end of theory and high level discussion and a move toward more
practical discussion on how to operate RavenDB. On the next part of the book, we are going to focus on 
