...
We re-implemented many features and scenarios that we already encountered, but this time we did that from the 
...
toward more flexibility with ever more complex queries. In both Linq queries and the strong typed indexes, we 
...
in production system. RavenDB updates indexes in a side-by-side manner. The old index is retained (and can even
...
We closed the chapter with a discussion of error handling. How does RavenDB handles indexing errors? How are they made visible to the operators, and what kind of behavior can you expect from the system? RavenDB
...
This chapter has marked the beginning of the end of theory and high-level discussion and a move toward a more
