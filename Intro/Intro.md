...
(high availability, load balancing, geo distribution of data and work, etc.). A single node can host multiple
...
datastore for an OLTP^[Online transaction processing, a fancy way to say a business application] applications(review - as we have the work "application" right before maybe it's not a good idea to use it again) should 
...
actually _use_ all of that experience and knowhow that we accured(review: what is this word?) to build a much better end result. 
...
Following on this _theoretical_(Didn't we said we are skipping the theory?) discussion we'll dive into the client API and explore the kind of advancced options
