...
lock contention, deadlock prevention, etc. With distibuted systems, you have all the usual problems of multiple threads
...
and can decide how to handle things from that point (**(((Is there a verb associated with the word error? Make an error, force an error?)))** error, try writing again with the new index, etc).
