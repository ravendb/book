...
., to a later part of this book, because I think that having a live version that you can play with will make it 
...
going to ignore everything related to actual production deployments in favor of getting you setup in as few steps 
as possible. Full discussion on how to deploy, secure and run RavenDB in product is available in 
the [Prouction Deployments](#production-deployments) chapter.
...
Probably the easiest way to get RavenDB is via Docker. If you already have Docker installed, all you need to
...
The most important piece of information you have here is the `Listening to: http://10.0.75.2:8080`, which tells you
the URL to put in your browser to access the newly installed instance.

### Running on Windows

To setup RavenDB on Windows you'll need to go to [https://ravendb.net/download](https://ravendb.net/download), 
...
To setup RavenDB on Linux you'll need to go to [https://ravendb.net/download](https://ravendb.net/download), 
...
You can do that by clicking on the `New database` button, name it `Northwind`, and accept all the defaults for
...
`sample data` feature in RavenDB to have some documents that we can work with. Go to `Create Sample Data` under 
`Tasks` in the left menu and press the `Create` button. 
...
On the left menu, select `Documents` and you'll see a view similar to Figure 2.3, showing the recently created 
...
Collections are very important to the way data is organized & optimized internally within RavenDB.Collections 
and the optimization opportunities that they provide are discussed at length in the [Bits on the Disk](#stroage-layout) 
chapter.
We'll frequently use collections to group similar documents together and apply some operation on them (subscribing 
...
result is shown in Figure 2.4. For the first time, we are looking at a real JSON document inside of RavenDB.

![An orders document](./Ch02/img04.png)

If you are familiar with working with non-relational databases, this is pretty obvious and not too exciting, but 
if you are mostly used to relational databases, there are several things to note here.

In RavenDB we are able to store arbitrarily complex data as a single unit. If you look closely at Figure 2.4 
you'll see that instead of just storing a few columns, we can store rich information and worki with nested 
...
This section will cover the basics of working with data within RavenDB Studio. If you are a developer, 
...
When you look at a particular document, you can edit the JSON, click on `Save`, and the document will be saved.
There isn't really much to it, to be honest. Creating new documents is a bit more interesting. Let us create a new
category document. 

Go to `Documents` in the left menu, click on `Categories` under `Collections` and then select 
`New document in current collection` as shown in Figure 2.5.

![New document in current collection](./Ch02/img05.png)

This will open the editor with an empty, new document that is based on one of the existing categories. Note
that the document id is set to `categories/`. Full in some values for the properties in the new documents and
save it. RavenDB wiil assign the document id automatically for you.
...
The first thing we'll learn to do is how to do bulk operations inside the Studio. Go to 
`Documents` on the left menu and click on the `Patch` menu item, select `Collection` in 
the `Type` drown down and then select `Categories` in the `Collection` drop down.
...
run a non-trivial transformation on the `categories` documents.  Using a patch script, we will add localization support 
 - the ability to store the category name & description in multiple languages.
...
Click on the `Test` button and see the results of running this operation a category document. You can also select which
specific document this test will be tested on. The before & after results of running this script on `categories/4` is 
shown in Figure 2.6.
...
It is important to mention that for performance reasons, such bulk operations can be composed of multiple, 
independent & concurrent transactions instead of spanning a single large transaction. Each such independent 
...
We've already used a document store to talk with a RavenDB server, but have you ever wondered what is its purpose? The 
document store is a main entry-point for whole client API, it holds Cluster URLs (so far we used only a single one, but 
in many cases our data can span across multiple nodes), the default database we will want to operate on and the API Key 
that will be used to authenticate ourselves. It's importance exceeds beyond connection management so let's take a 
closer look at it.

### The Document Store

The document store holds all the client-side configuration, including serialization configuration, failover behavior,
caching options and many more. In typical application, you shall have a single document store instance per application 
(singleton). Because of that, the document store is thread safe with initialization pattern that typically looks 
...
Listing 2.11 shows how we configure multiple nodes, setup security and select the appropriate database. We'll learn about how
...
need, which is why most of the client API parts are customizable.

Customizations can be applied by changing various settings and behaviors via `DocumentStore.Conventions` property. For example,
...
Here is how we can tell the client API that it should apply the `TypeName + Id` policy:

	documentStore.Conventions.FindIdentityProperty = 
		prop => prop.Name == prop.DeclaringType.Name + "Id";


Do not worry, we will not go over all of the available options since there are quite a few of them. Please refer to the 
online documentation to get latest list of available conventions with detailed description. It might be worth your time to go 
over and study them quickly, just to know what is available to you, even if for the most part they aren’t really something that 
you will touch all that often (or ever).
...
of e.g. default request timeouts, caching configuration or listeners. We will cover all of those later on, but for now let’s 
...
In development mode, you'll usually work with the `Raven/AnonymousUserAccessMode` setting set to `Admin`, which implies that 
any user will be automatically granted with administrative privilages. This reduces the number of things that you
have to do upfront. But as easy as that is for development, for production, we strongly recommend that this setting will be 
...
> requests with an error page explaining the situation with instructions how to fix the issue.
> 
> You can let RavenDB know that this is something that you actually want (if you are running on a secured local network).
> That requires an additional and explicit step to make sure that this is a conscious choice and not an admin 
> oversight.
...
it should Just Work. And the client API reflects that principle. If you look at the surface API for the session, here are the 
...
> sure to wrap that in a `using` statement or to ensure the session disposal. Not doing so can force us to do cleanup using
...
This will result in an dictionary with all three documents in it, retrieved in a single remote call from the server. If a 
...
document is independnt, isolated and coherent. 
...
We'll talk about modeling in a lot more depth in [Advanced client API](#advanced-client-api), when you have learned enough 
about the kind of environment that RavenDB offers to make sense of the choices we make.
...
those extensively in [Part III](#indexing). You already seen some basic queries in this chapter, so you seen how we can 
...
it doesn't have problems with table scans or slow queries. We'll touch on exactly why and the details about indexing in
[Part III](#indexing), but for now you can see that most queries will Just Work for you.
...
`Store` can also be used in optimistic concurrency scenarios, but we'll talk about this in more details in the
