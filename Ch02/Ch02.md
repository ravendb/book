...
Aside from getting the value by key, there is no other way to query **(((query for the values?)))**. Usually you already know what the compare-exchange
...
compare-exchange key in a document that you'll query and then use that **(((what's the "that" here?)))**. 
...
your RavenDB tests will use this class as a base class.^[Not strictly necessary, but this iss the easiest way to build tests.] 
...
to inspect, validate and modify the content of the in-memory database (while the test is _still running_). **(((This confuses me: did you say that you'll stop the current test?)))**
...
You can also generate the document ID yourself, which we typically call a semantic ID. Those **(((What is the "those" here referring to?)))** are identifiers that have meaning: 
...
the server). RavenDB makes use of metadata to pass **(((To pass as in, to skip over? Or to pass as in, pass on to someone else?)))** critical information such as the document collection, when it was last modifed, etc. 
