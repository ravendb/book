...
as possible. A full discussion on how to deploy, secure and run RavenDB in-product is available in 
...
based on the entity type of the document on the client. It's very similar to tables in a relational database, but 
unlike tables, there's no requirement that documents within the same collection will share the same structure or 
...
time and effort on the RavenDB Studio, and while I'll be the first to say that it's pretty, looking at a 
syntax highlighted text editor isn't really that impressive. So let's focus instead on what kinds of things RavenDB can do with the
data itself.
...
there is no such thing as schema in RavenDB. You are free to add or remove properties and values and modify the
