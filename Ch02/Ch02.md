...
[Part III](#indexing). You can also see that we can call `.ToString()` on the query to get the query text from 
...
document store is a main entry point for the whole client API. It holds the server URLs, for one. (So far we used only a single one, but 
...
have to do upfront. But as easy as that is for development, for production, you'll want to run in a secured fashion. 
...
> 
> By default, RavenDB will *refuse* to listen to anything but `localhost` in unsecured mode. 
> This is done for security reasons, to prevent admins from accidently exposing RavenDB without authentication over 
...
> 
> You can let RavenDB know this is something you actually want, if you're running on a secured and isolated network.
...
Once you have the metadata, you can modify it as you wish, as seen in Listing 2.19. The session tracks changes to
...
When the test reach this line, a magical thing will happen, as shown in Figure 2.15. The studio will open, and you'll be able to
