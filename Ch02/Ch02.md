...
> sure to wrap the variable in a `using` statement or else do something to ensure proper disposal. Not doing so can force you to 
> clean up using the finalizer thread, which can in turn increase the time it takes to release the acquired resources.
...
> This provides you with a test setup that's both very fast and that runs the exact same code as what you will run in production.
...
see and interact with everything that's going on inside RavenDB. One nice feature I like for complex cases is the
...
* HiLo&mdash;generates the identifier on the client by collaborating with the server to reserve identifier ranges that can be 
