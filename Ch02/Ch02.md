...
do is run the following command:
	...
	docker run -e UNSECURED_ACCESS_ALLOWED=PublicNetwork 
...
Start by adding the code in Listing 2.2 to the `Patch script` text box.
...
Create a new console application and run the command in Listing 2.3 on the console.

```{caption="Installing RavenDB Client NuGet package" .powershell}
dotnet add package RavenDB.Client -Version 4.*
```
...
Now that we have all those tasks, we want to start querying on the data. We'll get to how to query them from code in a bit, first
I want to show you how to query data from the studio. Go to `Indexes` and then `Query` in the studio and you'll see the query page. 
...
We'll learn all about querying RavenDB in [Part III](#indexing). For now, let's concentrate on getting results, which means looking
at how we can query RavenDB from code. Let's say I want to know what kind of tasks I have for
the next couple of days. In order to get that information, I can use the query in Listing 2.9.
...
public class DocumentStoreHolder
{
	...
	private static IDocumentStore CreateDocumentStore()
	{
      ...
      Certificate = new X509Certificate2("tasks-client.pfx"),
			Database = "Tasks",
		};
...
human-readable document identifier that make it easy to locate a document based on a user provided value (the email). While 
...
client, it will generate `people/128-B`, `people/129-B`, etc. These are different documents.  Using shorthand to refere to
...
RavenDB is meant to be run in a cluster. You can run it in a single node mode, but the most common (and recommended) 
deployment option is with a cluster. You already saw some of the impact that this has on the design of RavenDB.
...
RavenDB uses several strategies to do that, discussed in Part II of this book. At this point, I want to introduce
one of the tools that RavenDB provides specifically in order to allow users to manage distributed state correctly.

If you worked with multi-threaded applications, you are familiar with many of the same challenges. Different threads
can be doing different things at the same time. They may be acting on stale information or modifying shared state.
Typically such systems use locks to coordinate the work between threads. That lead to a whole separate issue of 
lock contention, deadlock prevention, etc. With distibuted systems, you have all the usual problem of multiple threads
with the added comlication that you may be operating in a partial failure state. Some of the nodes may not be able 
to talk to other nodes (but can talk to _some_).

RavenDB offers a simple primitive to handle such scenario, the compare exchange feature. A very common primitive
with multi-thread solutions is the notion of an atomic compare-and-exchange operation. From code, this will be
`Interlocked.CompareExchange` when using C#. Because this is so useful, this is supported at the hardware level with
the `CMPXCHG` assembly instruction. In a similar way, RavenDB offers a distributed compare exchange feature. 

Let's take a look at Listing 2.20, for a small sample of how this looks like in code.
...
at large. This operation compare the existing index for `names/john` with the expected index (in this case, `0`, meaning
...
and can decide how to handle things from that point (error, try writing again with the new index, etc).
...
For example, in Listing 2.20 we ensure a unique username for each user in a way that is resilient to failures, network
partitions, etc. 
...
We'll talk more about compare exchange values in Chapter 7. For now, it is good to remember that they are there and can help
...
Aside from getting the value by key, there is no other way to query them. Usually you already know what the compare exchange
key will be (as in the case of creating a new user name and checking it isn't already taken). Alternatively you can store the 
compare exchange key in a document that you'll query and then use that. 
If you know what the name of the compare exchange value, you can use it directly in your queries, as shown in Listing 2.22.
...
this feature again in Chapter 7. This feature relies on some of the low level details of RavenDB distributed flow, and it 
...
When the test reads this line, a magical thing will happen, as shown in Figure 2.15. The studio will open, and you'll be able to
