...
Now that we have a running node, the next stage is to create a new database on this node. 
...
easy to understand. If you _aren't_ familiar with LINQ, I strongly recommend learning it (Review: even if I'm a nodejs dev? Better to explain it for them here as it will help also for new c# devs). From the consumer side, it's
...
var conString = 
	...
	"ApiKey=tasks.myapp.example/MKGBO59mhw3y8DIP0yqLSKdM(TODO: We don't have ApiKey anymore.)";
...
In development mode, you'll most commonly work with the `Security.Authentication.Enabled` setting set to `false`, which implies that 
any connection will be automatically granted server administrator privileges. This reduces the number of things that you
have to do upfront. But as easy as that is for development, for production, we strongly recommend that this setting be 
changed to `true`.^[Note that this is a server-wide option, affecting all databases.] After doing so, all access 
to the server is restricted to authenticated users only. 

> **Caution:** unsecured network-accessible databases are bad for you.
> 
> By default, RavenDB will *refuse* to listen to anything but `localhost` if the `Security.Authentication.Enabled` is set to
> `false`. This is done for security reasons, to prevent admins from accidently exposing RavenDB without authentication over 
...
RavenDB uses `X509` client certificates authentication and securing the communication layer. The good thing about certificates 
...
authentication, and typically (`X509` client certificate) are granted on a per application / role basis.
...
> the database. It's common to see a web application making tens of calls to the database to service a single request, usually
> for no good reason.  
...
The client API is pretty smart about it, if you try to load a document that was alread loaded (directly or via `Include`) the 
...
Having to go to the database twice is pity because the server already knows the value of the `AssignedTo` property, 
...
(Review: Put this in a WARNING! box)As powerful as the `Include` feature is, one of _the_ most common issues we run into with RavenDB is people using 
...
We _could_ just name(give them ID, not name - identify those documents) those documents `transactions/1234`, `transactions/1235`, etc. It would work, but there are better ways. 
...
You might recall that I mentioned that RavenDB doesn't require documents within a given collection to be named(not the right terminology - identify?) with the 
...
> RavenDB contains several ways to handle them (discussed in ["RavenDB Clusters"](#clustering-setup) chapter). 
...
ends with the pipe symbol (`|`). Go into the RavenDB Studio and create a new document. Name(terminology: give it the tryouts| ID or identify it as) it `tryouts|` and then click on
...
they're most familiar with. But updating an identity happens in a _separate transaction_ than the current one. In other words, if we try to save a document with the `invoices/` ID and the transaction failed, the identity value is still 
...
protocol, which we'll cover in ["RavenDB Clusters"](#clustering-setup).] 
...
fit fewer of them inside a single database page(what is database page? maybe "page in the tree" is better), and we'll need to do a bit more work to find the right document. The same
...
* The hilo algorithm&mdash;generates the identifier on the client by collaborating with the server to reserve identifier ranges that can be 
...
You can also generate the document ID yourself, which we'll typically call semantic Ids. Those are identifiers that have meaning: 
...
the server). RavenDB makes use of metadata to pass critical information as the document collection, its Id (You didn't mentioned this before, so why now...), etc. 
