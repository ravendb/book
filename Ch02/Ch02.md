...
> **Running RavenDB in the cloud**
>
> In this book, I'm assuming that you are running RavenDB on premise, on machines that you control. This is 
> so I would be able to provide you with the maximum amount of information about how RavenDB operates behind
> the scenes.
>
> You can also go to [cloud.ravendb.net](https://cloud.ravendb.net) and setup a RavenDB instance or a cluster
> in a couple of minutes. There is a _free_ tier that you can use which will allow you to run RavenDB without
> having to get a credit card.
>
> When running in the cloud, you are always going to be running in a secured mode, which will necessitate
> changing the code samples in the book to include the client certificate to access your cloud instance. 
> You can find the full details in the online documentation.
...
to work with a RavenDB cluster in Chapter 7. We still have a lot to cover 
...
`Load` can also be used to read more than a single document at a time. For example, if I wanted three documents, I could use:
    ...
    Dictionary<string, ToDoTask> tasks = session.Load<ToDoTask>(new[]{
        "ToDoTasks/1-A",
        "ToDoTasks/2-A",
        "ToDoTasks/3-A"
    });
...
mean for working with the data? Let's say that, given an task ID, we want to print the details of the corresponding task, including the name of the person assigned to it. Listing 2.15 shows the obvious way to do this.
...
but that feature is exposed differently (via the `declare function` mode, which we'll cover in Chapter 10).
...
> RavenDB contains several ways to handle them (this is discussed in Chapter 7 in more detail). 
...
protocol, which covered in Chapter 7.] 
...
We'll talk more about compare-exchange values in Chapter 7. For now, it's good to remember that they're there and can help
...
proper production deployments. Check Chapter 16 for details on those.] And we learned how to set up a new 
