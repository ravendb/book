...
Put the following code in the `Patch script` text box:
...
Here is what our `ToDoTask` class looks like:
...
RavenDB, reminding us that we need to pick up a bottle of milk from the store tomorrow. Here is what the code to do
that looks like:
...
completed. Here is the code to do just that:
...
Let us exapnd on that and create a few more tasks, to see how that works:
...
the next couple of days. In order to get that information, I can use the following query:
...
Let us look at an aggregation query, giving us the results of all the tasks per day:
...
If you are familiar with Linq, there isn't much to say about this code. It works, and it is very obvious and easy to 
understand. If you _aren't_ familiar with Linq, I strongly recommend learning it. From the consumer side, it is very
beautiful. As someone who had to implement querying using Linq a few times, it is utterly atrocious, but lucky for
