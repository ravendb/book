...
The profilers are wonderful tools when you are working on relational databases, but they didn't actually solve the problems that I saw over and over again. It was not easy to pinpoint where the application was doing silly things. In essence, that meant that we had just moved the problem; no longer having to deal with the common errors, we now had to deal with a much more complex ones. How do we get all of this complex data to the user, manipulate it properly and save it back in a consistent, performant and maintainable way.
