...
Docker will now get the latest RavenDB version and spin a new container to host it. Note that we run it in 
...
The most important piece of information you have here is `Listening to: http://10.0.75.2:8080`, which tells you
the URL you'd put in your browser to access the newly installed instance.
...
but the details on the state of a newly minted node are a bit complex and covered in more detailed in 
["Your First RavenDB Cluster."](#clustering-intro) chapter] 
Now that we have a running node, the next stage it to create a new database on this node. 

You can do that by clicking the `New database` button, naming it `Northwind` and accepting all the defaults in
the dialog. We'll discuss what all of those mean in [Chapter 19](#prod-deploy). Click the `Create` button, and 
that's pretty much it. Your new database is ready. You can see what this looks like in Figure 2.2.
...
holds similar documents (though it doesn't have to). These documents are most often based on the entity type of the document in your  
code. It's very similar to tables in a relational database, but unlike tables, there's no requirement that documents within the same  
collection will share the same structure or have any sort of schema.

Collections are very important to the way data is organized and optimized internally within RavenDB. Collections 
and the optimization opportunities that they provide are discussed at length in the ["Bits on the Disk"](#storage-layout) 
chapter. We'll frequently use collections to group similar documents together and apply an operation to them (subscribing 
...
object graph can be stored in a single document. Modeling will be further discussed in [Chapter 3](#modeling), 
...
that the document Id is set to `categories/`. Fill in some values for the properties in the new documents and
...
`Documents` on the left menu and click the `Patch` menu item. Select `Collection` in 
the `Type` dropdown and then select `Categories` in the `Collection` dropdown.
You'll be presented with the screen shown in Figure 2.5.

![Patching the categories](./Ch02/img06.png)

Patching allows you to write a JavaScript function that can modify the matching documents. To try this out, let's
run a non-trivial transformation on the `categories` documents. Using a patch script, we'll add localization support&mdash;
the ability to store the category name and description in multiple languages.

Start by writing the code in Listing 2.1 in the `Patch script` text box.
...
specific document this test will be tested on. The before-and-after results of running this script on `categories/4` are 
shown in Figure 2.6.
...
RavenDB Studio. As the index, select `dynamic/Companies`, and in the query text box, write `Address.Country: UK`. 
...
wrote the query.) We'll discuss indexes and querying at length in [Part III](#indexing). 

This section was meant to show you the basics of using the RavenDB Studio, not to be a thorough walkthrough of 
all the options. This overview is mostly here so you're familiar with the basics of using the studio and can see the 
results of what we're doing in code in the _next_ section, where we get to actual coding.
...
Create a new console application and run the command in Listing 2.1 on the `Package Manager Console`.
...
This will set up the latest client version for RavenDB 4.0 on the project. The next step is to add a namespace
reference by adding `using Raven.Client.Documents;` to the top of the `Program.cs` file. 

And now we're ready to start working with the RavenDB client API. The first thing we need to do is to set up access
to the RavenDB cluster that we're talking to. This is done by creating an instance of `DocumentStore` and 
configuring it as shown in Listing 2.3.
...
The document store is the starting location for all RavenDB work, but the real workhorse we typically use is
the session. The session is what will hold our entities, talk with the server and, in general, act as the 
frontman to the RavenDB cluster.
...
Listing 2.4 shows what a class that will be saved as a RavenDB document looks like. 
...
You'll notice several interesting things, even in this very small sample. We loaded the document and modified it, and then
...
The document session implements the `Unit of Work` and `Unit of Work` design patterns. This makes it much easier to 
...
Let's expand on that and create a few more tasks. You can see how this works in Listing 2.7.
...
Now that we have all those tasks, we want to start querying on the data. Let's say I want to know what kind of tasks I have for
...
Running the code in Listing 2.8 gives the following output:
...
Let's look at an aggregation query. The code in Listing 2.9 gives us the results of all the tasks per day.
...
If you're familiar with LINQ, there isn't much to say about the code in Listing 2.9. It works, and it's obvious and 
easy to understand. If you _aren't_ familiar with LINQ, I strongly recommend learning it. From the consumer side, it's
quite beautiful. Now, if you were to implement querying using LINQ, it's utterly atrocious&mdash;take it from someone who's done it a 
few times. But lucky for you, that isn't _your_ problem. It's ours.
...
document store is a main entry point for the whole client API. It holds cluster URLs, for one. (So far we used only a single one, but 
...
API key that will be used to authenticate ourselves. Its importance goes beyond connection management, so let's take a 
closer look at it.
...
looks like the code in Listing 2.10.
...
explicit thread safety issues. And you can configure the document store as you see fit. The rest of the code has access to the
...
to work with a RavenDB cluster in the "[Your First RavenDB Cluster](#clustering-intro)" chapter. We still have a lot to cover 
on the document store without getting to clusters, though.


#### Conventions

The RavenDB client API, just like the rest of RavenDB, aims to _just work_. To that end, it's based on the notion 
...
by default, the RavenDB Client API will use a property named `Id` (case sensitive) to store the document Id. But there are users
...
online documentation to get latest list of available conventions, with a detailed description. It might be worth your time to go 
...
of, like default request timeouts, caching configuration and listeners. We'll cover all of those later on. But for now, let’s 
...
In development mode, you'll most commonly work with the `Raven/AnonymousUserAccessMode` setting set to `Admin`, which implies that 
any user will be automatically granted administrative privileges. This reduces the number of things that you
have to do upfront. But as easy as that is for development, for production, we strongly recommend that this setting be 
changed to `None`.^[Note that this is a server-wide option, affecting all databases.] After doing so, all access 
to the server is restricted to authenticated users only.

> **Caution:** unsecured network-accessible databases are bad for you.
> 
> By default, RavenDB will *refuse* to listen to anything but `localhost` if the `Raven/AnonymousUserAccessMode` is set to
...
> 
> You can let RavenDB know this is something you actually want, if you're running on a secured local network.
> It requires an additional and explicit step to make sure this is your conscious choice and not an admin 
> oversight.

RavenDB uses API keys for authentication. The good thing about API keys is that they're _not_ users. They're not tied to a
...
A much deeper discussion of authentication, managing API keys and security in general can be found in the 
["Securing Your Ravens"](#security) chapter.

### The document session

The session (also called "document session," but we usually shorten it to just "session") is the primary way your code
interacts with RavenDB. If you're familiar with NHibernate or Entity Framework, you should feel right at home. The RavenDB
...
We've already gone over the basics in `Your first RavenDB program`, so you should be familiar with basic CRUD operations using the 
...
> sure to wrap that in a `using` statement or to ensure the session disposal. Not doing so can force you to do cleanup using
...
Even though we called `Load<ToDoTask>("ToDoTasks/1")` twice, there's only a single remote call to the server and only a single
...
document we tried to load wasn'tt found on the server, the dictionary will contain null for that document Id.
...
> the database. It's common to see an application making tens of calls to the database, usually for no good reason. In 
> RavenDB, we've done several things to mitigate that problem. The most important among them is to allocate a budget for
...
>  once. In the same manner, we also allow you to batch _read_ calls. We'll discuss it in the next chapter, in the "Lazy" section.

Just like when you attempt to load an existing document that was already loaded, the session will also remember that it couldn't 
load that document. Even if asked again, it will immediately return null rather than attempt to load the document again.
...
extensively in the ["Advanced Client API"](#advanced-client-api) chapter, but the gist of modeling in RavenDB is that each 
document is independent, isolated and coherent. 
...
RavenDB is transactional, and we only send the request to the server on `SaveChanges`. So why could we print
...
This code works, but it's inefficient. We're making _two_ calls to the server here, once to fetch the task and another to
...
and it could send the document that matches the value of that property at the same time it's sending us the task. Includes, which 
handles this in one step, is a favorite feature of mine because I still remember how excited I was when we finally figured out how to do 
this in a clean fashion. Look at Listing 2.16 to see how it works, and compare it to Listing 2.15.
...
in the session and don't have to go to the server to fetch it.
...
but that features is exposed differently (via transformers, which we'll cover in ["Advanced Client API"](#advanced-client-api)).
...
We'll talk about modeling in a lot more depth in ["Advanced Client API"](#advanced-client-api), when you've learned enough 
...
those extensively in [Part III](#indexing). You've already seen some basic queries in this chapter, so you know how we can 
...
[Part III](#indexing), but for now you can see that most queries will _just work_ for you.
...
`Store` can also be used in optimistic concurrency scenarios, but we'll talk about this in more detail in the
["Advanced Client API"](#advanced-client-api) chapter.
...
>
> A common problem we see with people using the RavenDB client API is that they frequently start by defining their own 
> data access layer, usually named `IRepository` or something similar.
>
> This is generally a bad idea. We've only started to scratch the surface of the RavenDB client API, and you can already see 
...
>
> The RavenDB client API is meant to be easy to use and high level enough that you'll not need to wrap it for convenience’s sake.
> If all you're doing is forwarding calls back and forth, there's little point in doing so.
>
...
We covered the basics of working with the RavenDB client API in this section, but that was mostly mechanics. We'll dive deeper into using RavenDB in the next chapter, where we'll also learn how it's all put together.

### Going below the session

"Ogres are like onions," said Shrek. In a way, so is the RavenDB client API. At the top, and what you'll usually interact
with, you'll find the document store and the document session. They, in turn, are built on top of the notion of `Operations` 
...
from the server. You won't typically be using any of that directly, only through the session. Occasions to use an `Operation` 
...
The reason that they're exposed to the user is that the RavenDB API, at all levels, is built with the notion of layers. The
...
UTF8 string up to 512 bytes, although getting to those sizes is extremely rare. You've already seen document Ids used in
...
see in Figure 2.11. Amusingly enough, trying to include a raw emoji character broke the build for this book.

![Unicode gave us Emojis, and the birdie document](./Ch02/img11.png)

While going full-on emoji for document names might be going too far^[Although, when you think about it, there's a huge 
...
So RavenDB document Ids are Unicode strings up to 512 bytes in length, which must be globally unique in the scope of the 
...
> hard finding the differnce is, I believe the point is made. `Guid`s are not friendly, and we want to avoid having to deal with 
...
The `people/oscar@arava.example` example, which uses an email address in the document Id, is a common technique to generate a human-
readable document Id and to force uniqueness of a certain value (the uniqueness of the email address). The second 
example uses a unique key that's defined in another system. This is common if you're integrating with existing systems
...
We _could_ just name those documents `transactions/1234`, `transactions/1235`, etc. It would work, but there are better ways. 

We're going to store the transaction information on a per-day basis, using names that embed both the owner account and the 
...
> documents with the same Id by accident.^[If the user explicitly specified the document Id, there's nothing that RavenDB
...
> RavenDB contains several ways to handle them (discussed in ["Clustering in the Deep End"](#clustering-deep-dive) chapter). 
...
node is operating independently (indeed, a network failure might mean that we aren't _able_ to talk other nodes). In order
...
server to cooperate to get to the end result. This is not an issue if you're using any of the RavenDB client APIs, but if
...
character with a slash (`/`) and append a numeric value (the only guarantee you have about this value is that it's always
...
> RavenDB will not check for that and will _overwrite_ the existing document. That's partly why we're putting all those
...
All the Id generation strategies we've outlined so far have one problem: they don't give you any promise with regards to the 
...
a document with the Id `tryouts/1`. Doing so again will generate `tryouts/2`, and so on.

> **Invoices and other tax annoyances**
> 
> For the most part, unless you're using semantics Ids (covered earlier in this chapter), you shouldn't care what your
...
they're most familiar with. But updating an identity happens in a _separate transaction_ than the current one. In other words, if we try to save a document with the name `invoices/` and the transaction failed, the identity value is still 
incremented. So even though identity generated consecutive numbers, it might still skip Ids if a transaction has been 
rolled back. 
...
protocol, which we'll cover in ["Clustering in the Deep End"](#clustering-deep-dive).] 
...
There are also performance differences between the various methods that I want to talk about.
...
> My strong recommendation is that you use whatever document Id generation that best matches what _you_ need, and only consider
...
RavenDB keeps track of the document Ids by storing them inside a B+Tree. If the keys are very big, it will mean that RavenDB can
fit fewer of them inside a single database page, and we'll need to do a bit more work to find the right document. The same
...
`people/100`). But with the exception of when we add a digit to the number^[Rolling from `99` to `100` or from `999` to `1000`], values 
...
The server-side method using the pipe symbol (`|`) generates the best values in terms of suitability for storage. They're a 
...
Semantics Ids (`people/oscar@arava.example` or `accounts/591-192/txs/2017-05-17`) tend to be unsorted, and sometimes that can 
...
it without careful consideration and analysis. Identity requires network roundtrips to generate the next value, and it will 
...
The metadata is also a JSON format, just like the document data itself. RavenDB reserves for its own use metadata property names that 
start with `@` , but you're free to use anything else. By convention, users' custom metadata properties 
use a Pascal case naming convention. In other words, we separate words with a dash, and the first letter of each word is
...
* The etag&mdash;stored in the `@etag` metadata property and discussed at length in the next chapter.
...
> then create a new document with the same name in a different collection, but that tends to be confusing, so it's best
...
This chapter is quite long, but I can't really complete the basics without discussing testing. When you build an application, 
...
state setup before you start and they are isolated from one another. 

Listing 2.20 shows the code for a simple test that saves and loads data from RavenDB. 
...
There are just two interesting things in the code in Listing 2.20. The code inherits from the `RavenTestDriver<RavenExecLocator>`
...
your RavenDB tests will use this class as a base class.^[Not strictly necessary, but that's the easiest way to build tests.] 
...
The code in Listing 2.20 is using `xunit` for testing, but there's no dependency on the testing framework from 
...
> using in-memory-only node binding to `localhost` and a dynamic port. Each call to the `GetDocumentStore` 
...
> This provides you with a test setup that's both very fast and runs the exact same code as that in the real production.
...
When the test reads this line, a magical thing will happen, as shown in Figure 2.13. The studio will open, and you'll be able to
see and interact with everything that's going on inside RavenDB. One really nice feature that I like is in complex cases. It's
the ability to just export the entire database to a file, which lets me import it into another system for later analysis.
...
your machine.^[The steps outlined in this chapter are meant to be quick and hassle-free, not be an examination of proper production 
deployments. Check [Chapter 19](#prod-deploy) for details on those.] And we learned how to set up a new database and played a bit with 
...
to navigate it well enough that it's useful. We'll cover more about working with the studio in the 
rest of the book, but the details are covered extensively in the online documentation and unlikely 
...
We dove deeper and discussed the architecture of the RavenDB client, the use of `Document Store` and the `Document Session` to
access the cluster and a specific database, respectively. As a reminder, the document store is the singleton access point to a
...
* HiLo&mdash;generates the identifier on the client-side by collaborating with the server to reserve identifier ranges that can be 
         exclusively generated by a particular client.
* Server-Side&mdash;generates the identifier on the server-side, optimized for very large tasks, and allows each server to generate
         human-readable, unique identifiers independently of one another.
* Identity&mdash;generates a consecutive numeric value using a consensus of the entire cluster. Typically the slowest method to use
         and only useful if you _really_ need to generate consecutive Ids for some reason.

You can also generate the document Id yourself, which we'll typically call semantic Ids. Those are Ids that have meaning: 
...
workflow steps, etc.) without impacting its structure. You can modify such metadata seamlessly on the client-side (and access it on
the server). RavenDB makes use of metadata to pass critical information, the document collection, its Id, etc. 

Our last but certainly not least topic was discussion of testing your applications and how the RavenDB test driver allows you
...
In this chapter, we starting building the foundation of your RavenDB knowledge. In the next one, we'll start building on that 
