...
I'll cover cloud deployments in <<cloud-n-edge>>.
====
...
<3> Forward port 8080 from the container to port 8080 in the host machine
<4> The docker image to run: `ravendb/ravendb`
...
You can use `tar -xf RavenDB-8.0-linux-x64.tar.bz2` to decompress the archive. 
...
using a cloud instance, be sure to note the differences (running with secured setup, mostly). You can skip ahead to <<cloud-n-edge>> to
...
What we have right now is a RavenDB node that is a self-contained cluster.footnote:[Actually, that's not exactly the case, 
...
You can do that by clicking `Databases` button on the left and then `New Database` button, naming the new database `Northwind` and 
selecting `Quick Create` to accept all the defaults. We'll discuss what all of those mean later in this book. You can see the create database dialog in <<create-db-dialog>>.

[[create-db-dialog]]
.Create database dialog
image::./Ch02/img02.png[]

Click the `Quick Create` button, and that's pretty much it. Your new database is ready. 
...
to changes, indexing, querying, ETL, applying AI operations, etc.).
...
One thing that may not be obvious is that while the Studio generates an empty document based on existing ones, you're free 
to add or remove properties and values and modify the document's structure however you like. RavenDB is typically run in schema-less
mode, although it offers schema validation for enforcing data consistency. This flexibility in document structure makes it much easier to
evolve your data model and handle more complex data.
...
transaction processes a set of documents with full ACID properties (while the patch operation as a whole does not).
...
We explored the `Northwind` sample data earlier, but for our code exercises, we want a clean slate. Go back to
the Studio, click `Databases` on the left, then `New Database`, name it `Southsand` and click `Quick Create`. 
Starting with an empty database means we control every document in it — the IDs will be predictable, 
and we won't have to worry about confusing our own data with the sample dataset.
...
machine — and that we are going to be using the `Southsand` database we just created. The document store is the starting location for 
...
the time, you're working with your entities. For the purpose of this chapter, we'll work with products 
for our online store. 

<<product-class>> shows what a class that will be saved as a RavenDB document looks like. 
...
This is about as simple as you can get, but we're only starting, so that's good. Let's create a new product in
RavenDB. The code to do this is shown in <<creating-new-product>>.

[[creating-new-product]]
.Saving a new product to RavenDB
[source,csharp]
----
...
We opened a new session and created a new `Product`. We then stored the product in the session and called 
`SaveChanges` to save all the changes in the session to the server. You can see the results of this in <<creating-new-product-result>>.

[[creating-new-product-result]]
.The newly created product document in the Studio
image::./Ch02/img10.png[]

The price of Chai has gone up, so we need to update it. <<updating-product-price>> shows the code required to handle updates in RavenDB.
...
using (var session = store.OpenSession())
{
    ...
    product.PricePerUnit = 20.00m;
    session.SaveChanges();
}
----

Several interesting things can be noticed even in <<updating-product-price>>. We loaded the document and modified it, and then
we called `SaveChanges`. We didn't need to call `Store` again. Because the `product` instance was loaded via the session,
it was also tracked by the session, and any changes made to it would be sent back to the server when `SaveChanges`
was called. Conversely, if the `PricePerUnit` property already had the same value, the RavenDB client would detect that
and do nothing since the state of the server and the client match.

[NOTE]
.Asynchronous vs synchronous APIs
====
The RavenDB Client is natively asynchronous (it is using async to implement the synchronous API). In C#, you can choose which
API to use by calling `OpenSession()` or `OpenAsyncSession()`. The async session has methods such as `LoadAsync`, `StoreAsync`,
`SaveChangesAsync`, etc. We'll cover the async API later in this chapter.
====

The document session implements the `Unit of Work` footnote:[Unit of Work pattern: Maintains a list of objects affected by a business transaction and  coordinates the writing out of changes and the resolution of concurrency problems. See https://martinfowler.com/eaaCatalog/unitOfWork.html] and `Identity Map` footnote:[Identity Map pattenr: Ensures that each object gets loaded only once by keeping every loaded object in a map. Looks up objects using the map when referring to them.
...
Let's expand on that and create a few more products. You can see how this works in <<creating-multiple-products>>.
...
using (var session = store.OpenSession())
{
    var products = new[] 
    {
        ("Chang", 19.00m, 17, "Beverages"),
        ("Aniseed Syrup", 10.00m, 13, "Condiments"),
        ("Pavlova", 17.45m, 29, "Confections"),
        ("Tofu", 23.25m, 35, "Produce"),
        ("Manjimup Dried Apples", 53.00m, 20, "Produce")
    };
...
<<creating-multiple-products-result>> shows the end result of all this playing around we've done. We're creating five new products and saving them in 
the same `SaveChanges` call, so they will be saved as a single transactional unit.

[[creating-multiple-products-result]]
.All the current product documents
image::./Ch02/img11.png[]

#### Querying RavenDB

Now that we have all these products, we want to start querying the data. Before we get to querying these products from code, 
...
Let us find all the products that cost more than 15, we can do that using the following query: `from Products where PricePerUnit > 15`.
You can see the results of this in <<querying-products-studio>>.

[[querying-products-studio]]
.Querying for products in the Studio
...
at how we can query RavenDB from code. Let's say I want to find the most affordable products. 
I can use the query in <<querying-products-code>>.

[[querying-products-code]]
.Querying products using LINQ
[source,csharp]
----
using (var session = store.OpenSession())
{
    var affordableProducts =
        ...
        where p.PricePerUnit <= 25
        orderby p.PricePerUnit
        select p;
    Console.WriteLine(affordableProducts.ToString()) ;

    foreach (var product in affordableProducts)
    {
        Console.WriteLine($"{product.Id} - {product.Name} - {product.PricePerUnit:C}");
    }
}
----

Running the code in <<querying-products-code>> will result in the output shown in <<querying-products-output>>.
  
[[querying-products-output]]
.Querying products output
[source,plaintext]
----
from Products where PricePerUnit <= $p0 
    order by PricePerUnit

Products/3-A - Aniseed Syrup - $10.00
Products/4-A - Pavlova - $17.45
Products/1-A - Chai - $18.00
Products/2-A - Chang - $19.00
Products/5-A - Tofu - $23.25
----

The query in <<querying-products-code>> shows us using LINQ to perform queries against RavenDB with very little 
...
Let's look at an aggregation query. The code in <<aggregation-query>> gives us the count of products per category.

[[aggregation-query]]
.Aggregation query on products
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    Console.WriteLine(productsByCategory.ToString());

    foreach (var pbc in productsByCategory)
    {
        Console.WriteLine($"{pbc.Category} - {pbc.Count}");
    }
}
----
...
So far, we've explored the RavenDB API a bit, saved documents, updated a product and queried the products in various
...
public class DocumentStoreHolder
{
    ...
    private static IDocumentStore CreateDocumentStore()
    {
        var documentStore = new DocumentStore
        {
            ...
            Certificate = new X509Certificate2("southsand.pfx"),
            Database = "Southsand",
        };
...
The code in <<type-name-plus-id>> is how you can tell the client API to apply the `TypeName + Id` policy to find the ID property
for your entities.

[[type-name-plus-id]]
.Customizing how the RavenDB client selects which property is used as the ID
[source,csharp]
----
...
[source,csharp]
----
var t1 = session.Load<Product>("Products/1-A");
var t2 = session.Load<Product>("Products/1-A");
 
Assert.True(Object.ReferenceEquals(t1, t2));
----

Even though we called `Load<Product>("Products/1-A")` twice, there's only a single remote call to the server and only a single
instance of the `Product` class. Whenever you load a document, it's added to an internal dictionary that the session
...
[source,csharp]
----
Dictionary<string, Product> products = session.Load<Product>(
    "Products/1-A",
    "Products/2-A",
    "Products/3-A"
);
...
once. In the same manner, we also allow you to batch _read_ calls. We'll discuss the `Lazy` feature in more depth in <<advanced-client-api>>.
****
...
transaction. But we haven't actually worked with anything more complex than a `Product`. That's pretty limiting, in terms
of the amount of complexity we can express. <<employee-order-model>> lets us add the notion of employees who handle orders to the model.

[[employee-order-model]]
.Employees and Orders model in RavenDB
[source,csharp]
----
...
From looking at the model in <<employee-order-model>>, we can learn a few interesting tidbits. First, we can see that each class stands on
its own. We don't have an `Employee` object on `Order` or an `Orders` collection on `Employee`. We'll learn about modeling more 
...
In the case of an `Order`, I can look at my orders, create new ones or mark them as shipped without having to look at the 
`Employee` document. This is quite a shift from working with relational databases, where traversing between rows and tables is 
very common and frequently required. 


[NOTE]
.Modeling data in the age of Large Languages Models
====
The reason we model documents to be  independent, isolated, and coherent is that a single document holds all
the information it needs to be understood. It turns out that this approach is incredibly beneficial for AI models. 
Because the documents are self-contained, an AI can process them as they are, without needing to pull data from other 
sources to make sense of them. This simplifies the data for the model and makes it easier for the AI to work with.
====

Let's see how we can create a new order and assign it to an employee. <<creating-new-employee>> shows an interesting feature of 
...
using (var session = store.OpenSession())
{
    ...
    session.Store(employee);            <1>
    Console.WriteLine(employee.Id);     <2>
    session.SaveChanges();              <3>
}
----
<1> Storing the `Employee` entity in the session (and generating an ID for it)
<2> Printing the (newly generated) ID of the `Employee` entity - _before_ we save that to the database
<3> Actually going to the database server and saving the new entity

RavenDB is transactional, and we only send the request to the server on `SaveChanges`. So how could we print
the `employee.Id` property before we called `SaveChanges`? Later in this chapter, we'll cover document identifiers and how they're 
generated, but the basic idea is that the moment we returned from `Store`, the RavenDB client 
ensured that we had a valid ID to use with this document. As you can see with <<create-employee-and-order>>, this can be quite
important when you're creating two documents at the same time, with references between them.

[[create-employee-and-order]]
.Creating a new employee and an order assigned to them at the same time
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    var order = new Order
    {
        ...
        RequireAt = DateTime.Today.AddDays(1),
...
mean for working with the data? Let's say that we want to print the details of a particular order, including the name of the 
company that placed it. <<loading-two-documents>> shows the obvious way to do this.

[[loading-two-documents]]
.Displaying the details of an order (and the company that placed it)
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    Console.WriteLine(
        $"{order.Id} - due {order.RequireAt:d} for {company.Name}");

    // will print 2
    Console.WriteLine(session.Advanced.NumberOfRequests); <1>
}
----
<1> The code prints how many requests we made to the server (one to load the order, and one to load the company)

This code works, but it's inefficient. We're making _two_ calls to the server here, one to fetch the order and another to
fetch the company. This is part of the budgeting and awareness program RavenDB has, aimed at reducing the number 
of remote calls and speeding up your applications.

.Error handling
****
<<loading-two-documents>> really bugged me when I wrote it, mostly because there's a lot of error handling that isn't being done: 
the order ID being empty, the order document not existing, the company reference being null... you get the drift. I just
...
Having to go to the database twice is a pity because the server already knows the value of the `Company` property, 
and it could send the document that matches the value of that property at the same time it's sending us the order. RavenDB's 
`Includes` functionality, which handles this in one step, is a favorite feature of mine because I still remember how excited
I was when we finally figured out how to do this in a clean fashion. Look at <<loading-two-documents-with-include>> to see how it
works, and compare it to <<loading-two-documents>>.

[[loading-two-documents-with-include]]
.Order and company - single roundtrip
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    Order order = session
              .Include<Order>(x => x.Company)                       <1>
              .Load(orderId);
    
    Company company = session.Load<Company>(order.Company);         <2>

    Console.WriteLine(
      $"{order.Id} - due {order.RequireAt:d} for {company.Name}");

    // will print 1
    Console.WriteLine(session.Advanced.NumberOfRequests);           <3>
}
----
<1> Including the document referred to by the `Company` property
<2> Loading the related document (no remote call required)
<3> Printing the number of remote calls (only one, since we included the referenced value)

The only difference between the two code listings is that in <<loading-two-documents-with-include>> we're calling to `Include` 
before the `Load`. The `Include` method gives instructions to RavenDB: when it loads the document, it should look at the 
`Company` property. If there's a document with the document ID that's stored in the `Company` property, it should send 
it to the client immediately.

However, we didn't change the type of the `order` variable. It remains an `Order`. So what exactly did this `Include` method 
do here? What happened is that the session got a reply from the server, saw that there are included documents, and put them
in its `Identity Map`. When we request the `Company` instance referenced by this order, we already have that information
...
Using multiple `Include`s on the same operation, however, is just fine. Let's load an order, and with it we'll include both the
company that placed the order and the employee who handled it. This can be done using the following snippet:

    Order order = session.Include<Order>(x => x.Company)
                         .Include(x => x.Employee)
                         .Load(orderId);

Now I can load both the `Company` and the `Employee`, and there's still only a single round trip to the server. 
...
document ID. The following are various ways to delete a document:
    ...
    session.Delete(order); // delete by instance

And:

    session.Delete("Orders/1-A"); // delete by ID
...
changes that were made to it.footnote:[You can call `session.Advanced.Refresh` if you want to force 
the session to update the state of the document from the server.]
...
* Because a generic interface doesn't expose the relevant (and useful) features of RavenDB, you're stuck with using the lowest
  ...
  allow you to switch between data store implementations.footnote:[The 1% case where it will help is the realm of demo apps with 
...
behaviors and functionality,footnote:[`Unit of Work` won't work. Neither will change tracking, optimistic concurrency you'll have
...
[[working-with-async-session]]
.Working with the async session
[source,csharp]
----
...
using (var session = documentStore.OpenAsyncSession())
{
    ...
    var results = await ordersByEmployee.ToListAsync();

    foreach (var item in results)
    {
        Console.WriteLine($"{item.Employee} - {item.Count}");
    }

}
----

<<working-with-async-session>> shows a few examples of working with the async session. For the rest of the book, we'll use both 
the async and synchronous sessions to showcase features and behavior of RavenDB. 
...
directly usually arise when you're writing some sort of management code, such as <<creating-database-using-operations>>, 
which creates a new database on the cluster.

[[creating-database-using-operations]]
.Creating a database named 'Orders' using Operation
[source,csharp]
----
...
this chapter — `employees/1-A`, `Orders/4-A` and the like. Using a `Guid` like `92260D13-A032-4BCC-9D18-10749898AE1C`
...
see in <<document-with-emoji>>. Amusingly enough, trying to include a raw emoji character broke the build for this book.

[[document-with-emoji]]
.Unicode gave us Emojis, and the birdie document
image::./Ch02/img13.png[]

While going full-on emoji for document identifiers might be going too far footnote:[Although, when you think about it, there's a huge
...
****
Usually, we strongly recommend to have document IDs that are human-readable (`Orders/123-A`, 
`companies/alfreds-futterkiste@example.com`). We often use identifiers for many purposes. Debugging and troubleshooting are not the least
of those. 

A simple way to generate IDs is to just generate a new `Guid`, such as `92260D13-A032-4BBC-9D18-10749898AE1C`. 
But if you've ever had to read a `Guid` over the phone, keep track of multiple `Guid` values in a log file or just didn't realize 
that the `Guid` in this paragraph and the one at the start of this section aren't, in fact, the same Guid...

If you're anything like me, you went ahead and compared the two `Guid` values to see if they actually didn't match. Given how 
hard finding the difference is, I believe the point is made. `Guid` values are not friendly, and we want to avoid having to deal with 
...
identifier that's of some meaningful value. For example, `companies/alfreds-futterkiste@example.com` or `products/EXT-591-192` are two document IDs 
that the developer can choose. <<saving-new-company-with-external-id>> shows how you can provide an external identifier 
when creating documents.

[[saving-new-company-with-external-id]]
.Saving a new company with an externally defined document ID
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    session.Store(company, "companies/alfreds-futterkiste@example.com");
    session.SaveChanges();
}
----

The `companies/alfreds-futterkiste@example.com` example, which uses an email address in the document identifier, is a common technique to generate a 
...
the `products/EXT-591-192` example uses a unique key that's defined in another system. This is common if you're integrating with 
existing systems or have an external feed of data into your database.

#### Nested document identifiers

A special case of external document naming is when we want to handle nested documents. Let's consider an e-commerce system that
needs to track companies and their orders over time. We have our company document `companies/91-A`, but we 
also have all the orders placed by this company that we need to track. 
...
practical to hold all the orders directly inside the company document. So we need to put the orders in separate documents. 
We _could_ identify those documents using `orders/1234-A`, `orders/1235-A`, etc. It would work, but there are better ways. 

We're going to store the order information on a per-month basis, using identifiers that embed both the company and the 
time period: `companies/91-A/orders/2017-05`. This document holds all the orders for 
company `91-A` for May 2017. 
...
of other documents. In other words, as far as RavenDB is concerned, the only thing that `companies/91-A` and 
`companies/91-A/orders/2017-05` have in common is that they're both documents.
...
documents with the same ID.footnote:[If the user explicitly specified the document ID, there's nothing that RavenDB
...
RavenDB contains several ways to handle them (this is discussed in <<ravendb-clusters>> in more detail). 
****
...
smoothly, and the only impact is that you'll have a few documents with similar IDs.

.Dispose the document store to return unused range back to the system
**** 
When you take a range out using the `hilo` algorithm, it is marked as yours exclusively. If you aren't using the entire
range, it will be lost when another client is using it. That can be annoying if you care about gaps in the numbers.

When you dispose the document store, RavenDB will attempt to return any unused range back to the system. Note that this is
done simply for cosmetic reasons, to avoid the simplest scenario of running a process to save a single document and consuming
the entire range. With `hilo`, there is no actual attempt to _ensure_ that there would only be sequenctial values. 
****
...
and then click on the `Save` button. The generated document ID should look something like <<server-side-gen-id>>. 
 
[[server-side-gen-id]]
.Server side generated document ID (tryouts/0000000000000000012-A)
image::./Ch02/img14.png[]
...
If you manually created a document with an ID like `tryouts/0000000000000000015-A` and RavenDB is about to create such an ID,
RavenDB will detect the problem and move to the next number. However, given that you may also have documents with IDs like `tryouts/0000000000000000016-A`
we don't want to burn cycles trying to find a free number. So RavenDB will handle the problem by appending a GUID to the end of the ID.

The end result will look like: `tryouts/0000000000000000016-E7GEsXQbsUuDxjSxW5+l/g`. In other words, everything will _work_, but it 
is likely to be quite surprising to see such an ID in your system. It's easier all around to just avoid creating documents with IDs that
match the pattern of server-side generated IDs.
****
...
The id generated by the `/` prefix contains the node id as well, since it is possible for two nodes to generate the same id
on two nodes independently. For higher consistency, we need to turn to cluster-wide approaches, see the next section.
...
process of generating a new identity requires the nodes to coordinate with one another.footnote:[This is done using the Raft 
consensus protocol, which covered in <<ravendb-clusters>>.] 
...
.Don't generate similar IDs manually - part II
****
Just like with server-side ID generation, you need to be careful not to create documents that match the pattern of identity
generated IDs. If you create a document with the ID `tryouts/15` and the next identity value is `15`, RavenDB will detect the conflict
and _abort the transaction_ with a concurrency error. Unlike server-side ID generation, we can't just move to the next number, since that would
break the promise of consecutive IDs, so we have to fail the entire transaction.
****
...
RavenDB also has the notion of *cluster-wide transactions*, which we'll cover in the <<ravendb-clusters>>, ensuring that you
can both generate identifiers _and_ persist documents in an atomic fashion. 
...
pack less of them in a given space.footnote:[RavenDB is using B+Tree for on disk storage, and uses pages of 8KB in size. Bigger 
document IDs means that we can fit less entries in each page, and need to traverse down the tree, requiring us to do a bit more 
work to find the right document. The same is true for saving unsorted document IDs, which can cause page splits and increase the 
depth of the tree. In nearly all cases, that doesn't really matter.] 

The `hilo` algorithm generates document IDs that are lexically sortable, up to a degree (`people/2-A` is sorted after 
`people/100-A`). But with the exception of when we add a digit to the number footnote:[Rolling from `99` to `100` or from 
`999` to `1000`.], values are nicely sorted. This means that for the most part we get nice trees and very efficient searches. 
It also generates the most human-readable values.
...
Semantic IDs (`companies/vinset-alcools@chevalier.example` or `companies/91-A/orders/2017-05`) tend to be unsorted, and sometimes that can 
...
code in <<modifying-metadata>>.

[[modifying-metadata]]
.Modifying the metadata of a document
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    metadata["Last-Modified-By"] = employee.LastName;
    session.SaveChanges();
}
----
...
Once you have the metadata, you can modify it as you wish, as seen in <<modifying-metadata>>. The session tracks changes to
...
Let's take a look at <<using-compare-exchange>>, for a small sample of what this looks like in code.

[[using-compare-exchange]]
.Using compare exchange to validate unique username in a distributed system
[source,csharp]
----
...
The code in <<using-compare-exchange>> uses `PutCompareExchangeValueOperation` to submit a compare-exchange operation to the cluster
...
[NOTE]
.Cluster-wide operations & regular transactions
====
In <<using-compare-exchange>>, we create a `names/john` compare exhcnage value that is tied to `users/1-A`. What happens if we
fail immediately after the creation of the compare exchange value, but before we saved the `users/1-A`? We may have inconsistent state.
RavenDB offers cluster-wide transactions for this scenario, see <<using-cluster-wide-tx>> for more details.
====
...
For example, in <<using-compare-exchange>>, we ensure a unique username for each user using a method that is resilient to failures, network
partitions, etc. 

You can see how this is exposed in the Studio in <<viewing-compare-exchnage-values>>.

[[viewing-compare-exchnage-values]]
.Viewing the compare exchange values in the Studio
image::./Ch02/img15.png[]

We'll talk more about compare-exchange values in <<ravendb-clusters>>. For now, it's good to remember that they're there and can help
...
compare-exchange value using the code in <<reading-compare-exchange-values>>. Aside from checking the current value of the key, you get the 
current index, which you can then use in the next call to `PutCompareExchangeValueOperation`. 

[[reading-compare-exchange-values]]
.Reading an existing compare exchange value by name
[source,csharp]
----
...
If you know the name of the compare-exchange value, you can use it directly in your queries, as shown in <<querying-compare-exchange>>.

[[querying-compare-exchange]]
.Querying for documents using cmpxchg() values
[source,sql]
----
from Users 
...
The query in <<querying-compare-exchange>> will find a document whose ID is located in the `names/john` compare-exchange value. We'll discuss
this feature again in <<ravendb-clusters>>. This feature relies on some of the low-level details of RavenDB distributed flow, and it 
will make more sense once we have gone over that. 

### Document extensions: beyond JSON documents

RavenDB is primarily a document database, storing JSON documents and letting you query and operate on them. That isn't the sum total
of its capabilities. In this section, we'll briefly touch on those additional capabilites. As this section name suggest, RavenDB groups
those additional options under the document's extension moniker. Let's explore what options RavenDB offers beyond JSON documents:

* **Attachments** - raw binary data that is being stored by RavenDB as-is, also known as BLOB (binary large objects). 
  The easiest way to think about those is as attachments to email. RavenDB stores the attachemnts and allow you to download them
  afterward and index their contents. Examples: profile picture on a user profile, invoice PDF on an order form, etc.
* **Counters** - an 64 bits integer that counts (up or down). The reason this exists is that the counter allows concurrent and 
  distributed operations to seamlessly merge without conflicts. Examples: visits counter on a page, current occupancy in a building, etc.
* **Time Series** - allows to track a value (or set of values) that changes over time. For example, tracking your heartrate throughout
  the day, a car's GPS location, etc. You not only track the current value, but also all the historical ones and are able to efficeintly
  execute aggregation operations on the values (what is my average heartrate on an hourly basis for the past week).

We'll cover all of those in depth in <<documents-extensions>>.

### Cluster-wide transactions

RavenDB is always transactional. But there are two types of transactions that you can use, single-node transacctions and cluster-wide transactions.
We'll learn more about them in <<ravendb-clusters>>, but <<using-cluster-wide-tx>> is a simple example of using cluster-wide transactions to do the same work as we
did in <<using-compare-exchange>>.

[[using-cluster-wide-tx]]
.Using cluster-wide transaction to ensure a unique username in a distributed environment
[source,csharp]
----
using (var session = store.OpenSession(new SessionOptions {
           TransactionMode = TransactionMode.ClusterWide        <1>
}))
{
    var user = new User("john", "john@example.com");
    session.Store(user);                                        <2>
    session.Advanced.ClusterTransaction
        .CreateCompareExchangeValue("names/john", user.Id);     <3>
    session.SaveChanges();                                      <4>
}
----
<1> Open the session with cluster-wide transaction mode.
<2> Store an entity as we normally would (to go through the cluster).
<3> Create a new compare exchange value (and ensure it _is_ new) in the same transaction as storing the `user`.
<4> Save both the compare exchange and the document store to the database in a cluster-wide transaction.

The main difference we have here is the use of `ClusterWide` as the `TransactionMode` when we open the session. What this code does is store a new `User`
document in the session (and thus getting its ID). We then create a new compare exchange value off the session's cluster transaction and then we call to
`SaveChanges`. 

RavenDB ensures that the creation of the `names/john` compare exchange value and the saving of the new `User` document will happen in an atomic fashion.
Two concurrent operations trying to also create a user named `john` is _guranteed_ to have one of them fail with a `ConcurrencyException` (and reverting
all operations sent in the `SaveChanges` call) and one will succeed. We'll learn more about cluster-wide and how it works in <<ravendb-clusters>>.
...
In order to aid in testing, RavenDB provides the `Raven.TestDriver` NuGet package. The same capability exists for other platforms
as well, check the online documentation for the full details.  Using the test driver, you can get an 
...
<<basic-crud-test>> shows the code for a simple test that saves and loads data from RavenDB. 

[[basic-crud-test]]
.Basic CRUD test using RavenDB Test Driver
[source,csharp]
----
public class BasicCrud : RavenTestDriver            <1>
{
    [Fact]
    public void CanSaveAndLoad()
    {
        using (var store = GetDocumentStore())      <2>
        {
            string id;
            using (var session = store.OpenSession())
            {
            ...
            using (var session = store.OpenSession())
            {
                ...
                Assert.Equal("Côte de Blaye", product.Name);
                Assert.Equal("categories/1-A", product.Category);
            }
        }
    }
}
----
<1> Inherits from the `RavenTestDriver` to enable using RavenDB directly in your tests.
<2> Use `GetDocumentStore` to get a live document store instance managed by the testing infrastructure.

There are two interesting things happening in the code in <<basic-crud-test>>. The code inherits from the 
`RavenTestDriver` class, and it uses the `GetDocumentStore` method to get an instance of the document store.
Let's break apart what's going on.

The `RavenTestDriver` class is the base test driver, which is responsible for setting up and tearing down databases. All 
your RavenDB tests will use this class as a base class.footnote:[Not strictly necessary, but this is the easiest way to build tests.] 
Most importantly, from your point of view, is that the `RavenTestDriver` class provides the `GetDocumentStore` method, which 
...
The code in <<basic-crud-test>> is using `xunit` for testing, but there's no dependency on the testing framework from 
...
line between the two sessions in the code in <<basic-crud-test>> and then run the test:

    WaitForUserToContinueTheTest(store);

When the test reaches this line, a magical thing will happen, as shown in <<peeking-into-running-test>>. 
The Studio will open, and you'll be able to see and interact with everything that's going on inside RavenDB.
One nice feature I like for complex cases is the ability to just export the entire database to a file, which
lets me import it into another system later on for further analysis.

[[peeking-into-running-test]]
.Peeking into running test instance mid-test
image::./Ch02/img16.png[]

At this time, the rest of the test is suspended, waiting for you to confirm you're done peeking inside. You can do that 
by clicking the button shown in <<continue-the-test>>, located at the bottom of the screen, after which your test will 
resume normally and (hopefully) turn green.

[[continue-the-test]]
.Press this to continue the test
image:./Ch02/img17.png[]
...
your machine.footnote:[The steps outlined in this chapter are meant to be quick and hassle-free, rather than an examination of
proper production deployments. Check Chapter 15 for details on those.]
And we learned how to set up a new database and played a bit with the provided sample database.
