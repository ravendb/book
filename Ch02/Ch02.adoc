...
using (var session = store.OpenSession())
{
    ...
    session.Store(person);          <1>
    Console.WriteLine(person.Id);   <2>
    session.SaveChanges();          <3>
}
----
<1> Storing the `Person` entity in the session (and generating an ID for it)
<2> Printing the (newly generated) ID of the `Person` entity - _before_ we save that to the database
...
ensured that we had a valid ID to use with this document. As you can see with <<create-person-and-task>>, this can be quite
important when you're creating two documents at the same time, with references between them.

[[create-person-and-task]]
.Creating a new person and assigning him a task at the same time
[source,csharp]
----
using (var session = store.OpenSession())
{
...
person assigned to it. <<loading-two-documents>> shows the obvious way to do this.

[[loading-two-documents]]
.Displaying the details of a task (and its assigned person)
[source,csharp]
----
using (var session = store.OpenSession())
{
...
----
<1> The code prints how many requests we made to the server (one to load the task, and one to load the assigned person)

This code works, but it's inefficient. We're making _two_ calls to the server here, one to fetch the task and another to
fetch the assigned user. This is part of the budgeting and awareness program RavenDB has, aimed at reducing the number 
...
.Task and assigned person - single roundtrip
[source,csharp]
----
using (var session = store.OpenSession())
{
    ...
    ToDoTask task = session
              .Include<ToDoTask>(x => x.AssignedTo)             <1>
              .Load(taskId);
    
    Person assignedTo = session.Load<Person>(task.AssignedTo);  <2>
...
----
<1> Including the document referred to by the `AssignedTo` property
...
`AssignedTo` property. If there's a document with the document ID that's stored in the `AssignedTo` property, it should send 
...
that the developer can choose. <<saving-new-person-with-external-id>> shows how you can provide an external identifier 
when creating documents.

[[saving-new-person-with-external-id]]
.Saving a new person with an externally defined document ID
[source,csharp]
----
using (var session = store.OpenSession())
{
...
time of the transactions: `accounts/591-192/txs/2017-05-27`. This document holds all the transactions for the `591-192` 
account for May 27th, 2017. 
