...
fetch the assigned user. The last line of <<loading-two-documents>> prints how many requests we made to the server. 
This is part of the budgeting and awareness program RavenDB has, aimed at reducing the number of remote calls and speeding up your applications.
...
}))
{
    ...
    session.Advanced.ClusterTransaction
        .CreateCompareExchangeValue("names/john", user.Id);
    session.SaveChanges();
}
----
...
[source,csharp]
----
public class BasicCrud : RavenTestDriver
{
    public class Play
    {
        public string Id { get; set; }
