...
of the order. Lastly, it calls to `loadToOrders` with the final tally for the `Orders` table. RavenDB will actually call into the relational database and update it only when the script completes.
