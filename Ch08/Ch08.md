...
because you don't just set up the replication and RavenDB takes care of everything else. But there are other modes for distributing data in your systems.
...
Consider a microservice architecture and the customer benefits service. This service decides what kind of benefits the customer has. It can be anything
...
Another example is the reporting database. RavenDB is a wonderful database for OTLP scenarios, but for reporting, your organization likely already has a 
...
> **What about security?**
>
> We'll cover security in depth in the ["Securing your Ravens"](#security) chapter, but given that we've shown a connection string, I wanted
...
It's very important to remember that ETL is quite different from replication. When a RavenDB node performs ETL to another node, it's not replicating the data&mdash;it's _writing_ it. In other words, there are no conflicts and no attempt to handle such. Instead, we'll always _overwrite_ whatever exists on the other side. 
...
> that the ETL source can just update it to the latest version it has. That means if you made any modifications to the ETL'ed data, they will be lost. Instead of 
...
Sometimes you don't want to send a full document in the ETL process. Sometimes you want to filter them or modify their shape. This matters, since ETL processes compose a part of your public interface. Instead of just sending your documents to a remote destination willy nilly, you'll typically only
...
A good solution is to limit yourself to just the data from that particular document. That makes it easy to ensure that whenever the document is changed, the ETL process will reflect these changes completely on the other side. 
...
When you have a complex system, composed on more than a single application, it's considered bad form to just go peek inside
...
One way to create such a boundary is to mandate that any time an application needs some data from another application, it will go there.
...
Any support ticket opened by a user will require a call from the help desk service to the user's management service for details and updates.
...
Taking down the user's management service for maintenance will require taking down everything else that needs to call to it. A better alternative
...
user's management service is down, it doesn't impact operations for the help desk service. 
...
help desk database contains a section that's updated by the user's management service. In this manner, the ownership of the data is retained
by the user's management service, but the responsibility for maintaining it and keeping it up is that of the help desk service.

If the user's management service is taken down for maintenance, it has no impact on the help desk service, which can resume operations normally.
...
Beyond using ETL for disseminating documents between services, there are a few other scenarios in which they can be useful. A typical usage scenario for 
...
parties. We'll discuss authorization in detail in the ["Securing your Ravens"](#security) chapter, but for now, know that RavenDB allows you to define permissions at the
database level.

Sometimes, you need to apply such permissions on a per-collection or a per-document level. In some cases, it's per field or even depends on the exact
...
Another example is data aggregation. One use case we've seen many times is the embedding of RavenDB inside a larger application. In this manner, each 
...
use an ETL process to update the central server in the store, telling it about the new sales that were rang on the machine. You can also imagine the same architecture
...
You probably already have reporting infrastructure in your organization. Instead of having to build one from scratch for use with your data inside
...
We'll start with a simple ETL process for `Employees` as a way to explore the SQL ETL feature. Before we can start, we need to have created data on Microsoft SQL
...
> RavenDB is running under has permissions to the database in question. In this case, it's running as if it were in debug mode under my own
...
> 
> Usually you don't care about them; it's an internal implementation detail. For ETL, what you need to know is that a deletion of a document will trigger
...
processes between RavenDB instances, you get failover on both the source destinations, with each cluster able to route the ETL process to the right location
