...
This will generate `Territories:Asia` as the query and will find all the relevant employees. We can do this because at indexing time, we can feed the `Territories` array into the index and perform a highly efficent search in the index to find all the employees that have Asia in their territories. 
...
>
> Many advanced operations are exposed via extentions methods from the `Raven.Client.Linq` namespace, and if
...
And the resulting query would be `@in<Address.Country>:(Spain,Denmark,Switzerland)`. Note that this is using a RavenDB specific extension to the Lucene query syntax to provide an efficent search mechanism for potentially large lists.
