...
We can then run through the results and cast them to the right types. That _works_, but it is ugly and quite uncomfortable. We can do better, by asking RavenDB to give us a common shape back. That can be if the types share a common ancestor, but a lot of the time, that isn't the case.

Luckily for us, usually when we query a multi map index, we don't actually care for the actual documents matches. We need to show something to the user. So we need just enough information so the user can decide what to do with this. That being the case, we already have a common shape that all the results of the multi map index share, the shape that is actually being indexed.

The multi map index enforce that all map functions will have the same output, and we can use that when the time comes to query the index.
