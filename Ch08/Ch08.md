...
As you can imagine, such a system still require a lot of work. In particular, even though we have a clear boundary with a remote call from 
one application to another, we have temporal coupling between the applications. We can never take down the users application for maintenance
without also taking down the helpdesk application.
