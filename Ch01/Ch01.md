...
A common example for normalization is addresses. Instead of storing a customer's address on every order that he has, we'll simple store the address id in the order, and we have saved ourselves the need to update the adress in multiple locations.
