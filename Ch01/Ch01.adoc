...
its own. I spent the better part of two decades (since 2008) making this vision into reality. 

Today, RavenDB is a multi-model, high-performance, non-relational, distributed database for online transactions processing,
...
RavenDB is a *multi-model* database. It primarily deals with documents, but it also supports binary data (BLOB), counters 
and time-series data. When talking about "documents" in the context of RavenDB, we're talking about structured information in 
the form of self-contained data (as opposed to Word or Excel documents). A document is usually in JSON or XML format. RavenDB is,
essentially, a database for storing and working with JSON data. You can also store binary data and a few other types, but 
you'll primarily use RavenDB for JSON documents.
...
### Fascinating perspective about Boring Database
...
that after reading this book, you'll understand the lengths we went to in order to make RavenDB boring to use, reliable to deploy and a joy to work with.

This book isn't meant to provide documentation; RavenDB has quite a bit of documentation that tells you exactly how to use it. While 
informative, documentation tends to be dry and it hardly makes for good reading (or for an interesting time writing it). A blog post, on the other hand, tells
...
I'm a developer at heart. That means that one of my favorite activities is writing code. Next in the list is writing about writing code.
Writing documentation, on the other hand, is so far down the list of my favorite activities that one could say it isn't even on the list. I
do like writing blog posts, and I've been maintaining an active blog for close to fifteen years.footnote:[You can find it at http://ayende.com/blog] 
...
On the technical side, I want to thank Adi Avivi, Anders Wåglund, Arkadiusz Paliński, Arkadiusz Palinski, Aviv Rahmany,
Bartosz Piekarski, Dan Bishop, Damian Olszewski, Danielle Greenberg, Djordje Djukic, Egor Shamanaev, Elemar Junior, Efrat Shenhar, 
Federico Lois, Fitzchak Yitzchaki, Golan Nahum, Gracjan Sadowicz, Grisha Kotler, Grzegorz Lachowski, Idan Ben Shalom, Iftah Ben Zaken, 
Igal Merhavia, Jay Lokhande, Jonathan Matheus, Judah Himango, Karmel, Karmel Indych, Lev Skuditsky, Liel Nagar, Lukas Bickel, Maciej Aszyk, 
Maksym Smoliński, Marcin Lewandowski, Mateusz Bartosik, Maxim Buryak, Michael Yarichuk, Michał Leśniak, Paweł Pekról, Rafał Kwiatkowski,
Shahar Hikri, Shiran Shalom, Stav Lagziel, Szymon Kulec, Tal Weiss and Tomasz Opalach.

The editors, who had the harsh task of turning raw text into a legible book. Erik Dietrich, Laura Lattimer,
Katherine Mechling, Amanda Muledy and above all Danielle Greenberg. All the errors you find were inserted by myself after the last 
...
My name in Oren Eini and over 18 years ago, I got frustrated working with relational databases. At the time, I was
...
At the time, building RavenDB felt not like I was creating something from scratch, but like I was merely letting out something
...
book, which I _highly_ recommend. We tried to get a lot of things right from the get go, and with close to two decades in production
...
Finally, we'll go over the care and feeding of indexes in RavenDB: how you can create, deploy, monitor and manage them yourself. 
We'll talk about how the RavenDB query optimizer interacts with your indexes and how to move them between environments. 

#### Part VI - AI & Large Language Models Integration

Focus: Developers and architects

This part discusses RavenDB's integration with AI and Large Language Models, starting with how vector search allows for advanced semantic queries.
We'll explore embedding generation, which creates vectors from your data, enabling this type of search. Then we'll cover how you can apply Generative AI
directly from within the database to enhance, summarize, and make decisions on your data.

You'll also learn how to build AI agents that leverage RavenDB's capabilities to become intelligent, production-ready systems with a very short development
time. We'll dive into features like conversation memory and summarization, and see how you can expose RavenDB data and queries directly to the model. 
You'll also learn how to let the model securely invoke actions within your code.

The AI integration in RavenDB is designed to let you use the advanced features of AI models while protecting you from the risks of giving a potentially
 untrustworthy model full control. We'll discuss RavenDB's security approach, which allows you to manage the AI models and their access to your systems safely.
...
We'll discover how you can dig into RavenDB and see exactly what is going on inside the engine. We'll go over the kinds of 
self-optimizations RavenDB routinely applies and how you can take advantage of them.
...
We'll spend a whole chapter discussing backups and restores. RavenDB supports several options for backing up your data: offsite hot spares, 
full binary snapshots of your data and highly compressed backups meant for long term storage.
...
full background on and greater understanding of how RavenDB works. I'm not covering the nitty-gritty details of every API call and 
what parameters should be passed to it.

In many cases, I have elected to discuss a feature, give one or two examples of its use and where it's best utilized and leave 
the reader with the task of reading up on the full details in the documentation.
