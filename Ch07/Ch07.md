...
An funway to experiment with this is to create some work in your cluster (a few external replications or subscriptions would do just fine) and then close one 
of the nodes.
...
The supervisor is also free to demote all the database instances in a group, if it detect that they all failed or are unreachable, until only a single one remains.
As the promotable nodes begin to come up again the supervisor will let them catch up to the single remaining full member and then promote them once they finish catching up.
We'll cover the supervisor with a lot more detail in the ["Prouction Deployments"](#production-deployments) chapter. 
