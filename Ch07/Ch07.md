...
When node B comes back up, node A will send it all the updates that it missed. However, we're in somewhat of a pickle if node A is also failing at this point (and the operations team is now 
...
> needs. 
> We'll cover the working of the supervisor in a lot more detail in the ["Production Deployments"](#production-deployments) chapter. 
...
the supervisor notices that node E is down. It moves the node to the rehab program and hopes that it will recover. However, if it is down for too long (the default value is 15 minutes),
...
> RavenDB will _not_ start moving databases back to node E on its own and will instead require the operations team to instruct it to do so. Why is that?
>
