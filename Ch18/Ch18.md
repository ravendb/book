...
again **(((Should this be, "in order for the databases on this node to become available again?)))**.
...
Forming a new cluster is a one-way option. After the new node has been moved from **(((to?)))** passive mode, you cannot add it back to the 
...
recommended mode) **(((the "which is not the recommended mode" here makes it sound like "just updating the DNS" is the recommended mode.)))**. There are a few things you need to do to ensure that the transition is handled properly, starting
...
a new node to the cluster. The new **(((Is this actually a new node, or is it the original node C that's now on a different machine?)))** will be re-added (still called node C). RavenDB will remember the node and perform
all the necessary hookups to make sure that the newly added node returns to its rightful place in the cluster.
...
In short order, the cluster will make sure that node C is backed up, has all of its relevant data, is fully up
...
more powerful nodes. Each of the nodes is exactly the same **(((it's strange to say that the nodes are exactly the same when you just said that some are newer/more powerful than others?)))**; the only difference is the role they are 
assigned. Any watcher node can become a member node and vice versa. 
...
that doesn't hold. In a three-node cluster, if you lose two of the nodes, the cluster cannot recover from this **(((Here, you say "the cluster cannot recover" followed by the cluster recovering. I think it's that the cluster cannot recover while two fo the nodes are out, correct? So maybe: "if you lose two of the nodes, your cluster goes down until one of the failed nodes recovers"?)))**. When one
...
three out of five nodes in a cluster. **(((Why does losing three out of five nodes in particular make this an issue?)))**

> **Do _not_ pull the emergency handbrake**
>
> This **(((What's the "this"? The emergency handbrake?)))**, like everything else under the Admin JS Console, is provided with an important caveat. This **(((again, what's the this?)))** is modifying the
> internal structure of RavenDB in a very invasive way. It isn't meant to be something that you use only in the most
> dire of emergencies.
> 
> You need to verify that this **(((??))*** is the only thing that is running on the system while this is going on and execute the 
> sequence of steps exactly as specified. Otherwise, you might see bad system behavior or unexpected results. 

Losing three nodes means that you can't have a majority. You must cause one of the nodes to secede from the cluster. You
can't have two nodes secede **(((Should this be: "But when you cause a node to secede, it will form its own single-node cluster"?)))** since each will create its own single-node cluster. 
...
distinguish between them because the process **(((Which process?)))** is transparent. We can look at the databases from any node without needing to
specify which node we are talking to for each database from the client side **(((does "from the client side" mean "from the client's perspective"?)))**.
...
for you) to the less easy (when you do everything yourself). We'll discuss the easy way to do things in just a bit, but right now I want to focus on 
the manual mode.
...
no means all of them, mind) it can be faster to do things directly. It is **(((What is? The manual process?)))** also a real exposure of how RavenDB is actually
...
The database instance managed at the node level is, to use a very crude resolution **(((definition?)))**, just the set of files that make up the
...
>
> It is _not safe_ to generally **(((To generally what much?)))** much about in the RavenDB directory. That is true for users in general as well as other things like antiviruses, file system indexing, file monitoring, etc. RavenDB has firm ideas about how it interacts with the disk. Interfering with its processes can cause issues.
>
...
attach it to another with relative ease. In this case, moving the data manually **(((Does that make sense, rather than the vaguer "this"?)))** might be worth this hassle. Otherwise, just use RavenDB's built-in replication
for this scenario.
...
that if the configuration isn't set, the first thing you should do is to set the `Read balance behavior` to `Round Robin` and wait a bit. You'll see
...
of system resources. If at all possible, try doing this sort of thing **(((instead of "this sort of thing," can we just say "scaling"?)))** ahead of time or during idle periods.
...
instance. At this point, I suggest running whatever system tests you have **(((running your load test procedure and encouraging users?)))** run your load testing procedure and encourage users 
...
isn't going to be using the new indexes, there will be very little impact while they are being built. Once the indexes are done,
...
hosts a RavenDB server to record its sales. Figure 18.6 shows the geographic distriubtion of the stores.

![Gary's Shoes locations (and the locations of the servers)](./Ch18/img06.png)

To properly manage this chain of stores, we need to be able to look at data across all stores. One way of doing is to set up external replication from each store location to a central server. This way, all the data is aggregated into
a single location. In most cases, this would be the natural thing to do. In fact, you would probably want two-way 
replication of most of the data so you could figure out where a specific shoe model is located in nearby stores by just looking
...
the sales replicated. We just want some aggregated data. But we want this data aggregated across all stores, not just at one individual store. Here is how we can handle this. We'll define an index that would aggregate the sales across the 
