...
This chapter mostly consist of walkthroughs of particular tasks that you might need to 
...
more powerful nodes. The nodes in RavenDB are all homogenous; the only difference is the role they are assigned. 
...
> The hard reset will modify the internal structure of RavenDB in a very invasive way. It isn't meant to be something that 
> you use only in the most dire of emergencies.
> 
> Before calling `HardResetToNewCluster()` you should verify that there is no other usage of the system. The kind of operation
> should be done in isolation from outside interference, otherwise, you might see bad system behavior or unexpected results. 

In order to recover from permenantly losing three out of five nodes in the cluster, you must cause one of the nodes to 
...
because each of them will be its own _separated_ cluster. We need to take a slightly more complex series of steps.
...
distinguish between them because RavenDB let you work at the database group or database instance level transparently. 
...
are handled automatically and you ususally don't care what database instance you are talking to for a particular operation. All of
that is handled for you behind the scenes by RavenDB.
...
allows any node in the cluster to tell a client where to find the database, regardless if this particular nodes hosts an instance
...
between nodes is also a real exposure of how RavenDB is actually managing databases internally and may be helpful in other places.
...
>
> It is _not safe_ to generally muck about in the RavenDB directory. That is true for users in general as well as other things like 
...
> the behavior of the system. Of course, this is what you want, why am I mentioning this? 
>
> Because this also change externally observed behavior. A write to one server followed by a load balanced read from another
...
> before setting it. 
>
> Under the gun, you may accept the potential for (what is likely to be very rare) issue like that, just until you can come
> back from the precipice. In general, you want to set this only with full understanding of the impact this will have on 
> your system. We discussed this setting in more details in Chapter 6.
...
the static indexes you made are backward compatible, otherwise queries may fail once the indexes complete and the side-by-side index
