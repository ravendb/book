    ...
    public class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            using (var session = documentStore.OpenSession())
            {
                ...
                var searchResult = program.Search(new SearchQuery
                {
                    Query = "Chef Alice",
                    Facets =
                    {
                        new FacetSearchResult{SearchClause = "Supplier: suppliers/2"}
                    }
                });
                var serializeObject = JsonConvert.SerializeObject(searchResult);
            }
