...
manually fail over to it while you replicate the data back to the original cluster **(((Does this make sense: replicating back to the cluster vs. to the nodes/to the database?)))**. 

> **Restoring databases can take time**
> 
> The potential that you could delete a database or lose a whole cluster is the perfect reason to have a backup. **(((Here, "seems like" makes it sound as though these aren't actually good reasons to have backups. "It _seems_ like it's a good reason, but _actually_ it's not." Does the way I reworded the sentence make sense?)))**
...
> of the disk itself. But the actual restore time can be higher. Plus, the I/O costs aren't the only costs to consider. Most
...
> Alternatively, you may decide that if those issues do come up, the 20+ minutes it will take to restore from backup is acceptable.
...
option gives you the smallest possible size for your data and makes it easier and cheaper to store it. On the other hand, when 
...
> this scenario. In short, you should be using RavenDB's own backup system, not relying on the file system for that.
...
An incremental backup is defined as the set of changes that have happened since the last backup/snapshot. Regardless of whether you
use a backup or a snapshot, an incremental backup always uses gzipped JSON (RavenDB doesn't do incremental snapshots). **(((I'm having trouble with the previous sentence. "Regardless of whether you use backup/snapshot" seems to imply they're going to follow the same process, but then it seems like incremental backups and incremental snapshots follow very different processes?)))** The
...
consistency of changes for the database. Such operations **(((Which operations? All that's mentioned in the prior sentence is the "consensus algorithm"; there's no reference to any operations.)))** include identities, creating indexes, etc.
...
> incremental backups). If you have a _lot_ of identities (very rare) or many compare-exchange **(((For consistency, should this be "compare-exchange" or "Compare Exchange"?)))** values (more common), you might
...
these tasks. That's what you want when restoring a down node, but it's something to note if you're restoring a backup on
...
For full backups, this **(((What is the "this" referring to, here?)))** doesn't matter. Any node will perform the backup from its current state as usual. If the cluster decides that
a different node will execute the backup, they **(((Is the "they" just the one node performing the backup, or all the nodes?)))** are clearly marked and timestamped. From an operational perspective, there is no
...
* When the cluster moves the ownership of a backup from node to node, the new owner will first run a full backup even if only an 
  ...
  When a new node is assigned the backup task, it will check to see if it has done a full backup within the recent defined duration. If it has,
  then it will only run an incremental backup (from the last full backup for this node).
...
> When you define a backup schedule for RavenDB, _always_ use the server local time. This can be 
...
Figure 17.5 shows how the studio makes clear where each task will run. And in Figure 17.6, you can see how
...
a database from a backup. RavenDB attempts to make the restore process simple and obvious. To start the restore process, go to the `Databases` view in the Studio **(((Should studio be un-capitalized?)))** and click on the cheveron **(((This is a pretty uncommon word. Do you mean, v-shaped button?")))** near the `New database` button, then select 
...
You can see that in this case, we have a full **(((Is it true that the first is a full snapshot? Just wanted to check.)))** snapshot from node C from April 22 at 2 a.m. and an incremental snapshot from the same
...
> **Backup locations**
> 
> RavenDB doesn't care if all your databases go to the same location; it uses the database name as part of a backup folder
...
In particular, you need to account for the fact that you must have the backup media available on the machine to which you want to restore **(((Does the previous sentence make sense?)))** . Consider a backup strategy that backs up to a local folder (retained one week), Amazon S3 (retained one year) and 
Amazon Glacier (retained forever). 

Assuming that you want to restore from a recent backup, you can expect pretty speedy restore times. I have tested this on an
