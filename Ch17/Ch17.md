...
> depending on how much there is to index and what is the initial state **(((The initial state of what? The data? The backup?)))**. This is done to give you access to the documents and to enable you to start defining a replication factor for the new database as soon as possible.
...
First, you need a file path: a way to expose the backups to RavenDB. **(((The phrase "this seems obvious" made it sound like the next bit of the sentence wasn't going to be obvious. But it didn't seem counterintuitive, so I deleted it. Does the previous sentence work on its own?)))**. This is easy enough when you are backing up to a local path in the first place, but what happens if you have backed
...
* Ensure that on all the nodes in the cluster you want to restore on, you have a way to access the data. You could do this by copying it to each machine, having it shared on the network or using mount tools such as `S3FS-FUSE` and the like. 
  If you are using some sort of shared option (network share or **(((Is "or" right here?)))** all nodes reading directly from the cloud) you have to 
...
* Add the database **(((Which database is this? The one on the original/first node?)))** to all the other relevant nodes.
...
database, it **(((What does the "it' stand for?)))** will be sent to all the nodes in the database group, which will open the database at the specified path and
...
When resoting an unencrypted snapshot, you cannot provide an encryption key. You must first restore the snapshot and then export/import the data to a separate encrypted database. 
...
The main is backup vs. snapshot. A snapshot is a full binary clone of the database at a particular point 
...
of the backup process between nodes if there are failures and gracefully recovering from it **(((What's the "it" here? The nodes?)))**.
