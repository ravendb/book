...
Closing this chapter, we look at the Changes() API, which allow us to register for notifications from RavenDB whenever a document changes (as well as a host of other stuff) and result transformers, which allow to run a server side transformation of the data before is it sent to the client.
