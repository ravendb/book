...
its criteria to arrive. When that happens, the server will immediately send that document to the subscription. In other words, not only did we get traditional batch
...
You can create the subscription through the RavenDB studio. In the same database that we used in the previous chapter, go to `Settings` and then `Tasks`, and you'll 
...
by another client. As you can see, most times the `Run` method will return (if you manually disposed the subscription). But it will also throw 
...
JavaScript expression (see Figure 5.4) and send it to the server to be evaluated on each document in turn. Any matching document will be sent to the client for 
processing.
...
There's a lot going on in Listing 5.5, including a commented out example of what _not_ to do. Even though we removed the code that actually opens the 
...
a `return`, `return false` or `return null` to be an indication that we want to skip this document, and it won't send the document to the client. On the other
...
type you use for `Open` must be the same. Scenarios like the one outlined in Listings 5.10 and 5.11 make this flexibility very useful.
