...
large amount of data. Implementing those kind of processes with the OLTP mode is possible, but it isn't really easy to do.
...
In the studio, if you'll go to the edit subscription page, you'll see the option to test a subscription. If you click on that, you'll see a result
...
However, there are few reasons a subscription _will_ exit. Problems with the network will simply cause it to retry or failover to another node, for
...
when there's no way for it to recover. We'll cover more on this in the subpscription deployment section later on in this chapter. 
...
After all this buildup, the actual code in Listing 5.3 is pretty boring. The lambda we sent get a batch instance, which has a list of `Items` that are contained
in this batch. And on each of those items, we have a `Result` property that contains the actual document instance that we  sent from the server. 
...
In Listing 5.5, the concurrency exception is an expected error. We can just ignore it and skip processing this document. There's a bit of a 
trickery involved here. Because the document has changed, the subscription will get it again anyway, so we'll skip sending an email about this call now. But 
...
we will only set the `Survey` field on it. In Listing 5.6, we're also sending the survey email _before_ we set the `Survey` flag, so a failure
...
>   ready for failure on that end and have a meaningful strategy for handling it. Your options are to either propogate the error up the chain,
...
> solved. 
>
> In practice, RavenDB had support for distributed transactions with multiple resources up until version 3.x, when we deprecated this feature.  
...
> success. A distributed transaction coordinator basically requires all praticipants in the transaction to promise that, if it tells them to
...
> coordinators will retry such transactions (manually or automatically) and recover from trasients errors. But the problem with the line of thinking that deems this an issue
...
citizen and not go to the server any more times than it needs to. This is also a good chance to demonstrate a real usecase for using `Defer` in production code. The use of `Defer` means two things: one, we don't need to worry about the number of calls and, two, we've handled concurrency. 
...
> to keep that all in the document metadata, and we'll typically introduce a separate document to maintain the state of the subscription. In it, we're
> tracking support calls, and `SupportCalls/238-B` will have a `SupportCalls/238-B/EscalationState` document that will contain the relevant
...
If this was all we could do with the subscription script, it would have been very useful in reducing the amount of data that's sent over the wire. But 
...
term of overall performance. Even with the optimization we implemented in Listing 5.7, there's still another remote call to do. We can do better.
