...
Working with subscriptions is divided into two distinct operations. First we need to create the subscription and then open it. Subscriptions aren't like
...
You can create the subscription through the RavenDB studio. In the same database as we used in the previous chapter, go to `Settings`, `Tasks` and you'll 
see the task selection dialog shown in Figure 5.1. Ignore the rest of the tasks and select `Subscription`. You'll see the subscription edit form, now name the task "CustomersSubscription" and select the `Customers` collection. This is it, you can save the subscription. The result of this is shown in Figure 5.2.

![Database tasks selection dialog](./Ch05/img01.png)

![The newly created customers subscription](./Ch05/img02.png)

You can also do the same through code, as can seen in Listing 5.1, where you can see how we create a subscription for `Customer` documents. 
...
Regardless of if you create the subscription manually or via code, the result is a named subscription. But the question is, what exactly _is_ that?
In the studio, if you'll go to the edit subscription page, you'll see the option to test a subscription and if you'll click on that, you'll see a result
similar to Figure 5.3.

![Testing the customers subscription](./Ch05/img03.png)

Those are our customers documents, but what does it means? A subscription is a way to go through all documents in a collection. On its own, it isn't really
...
and inspect things. The point is that RavenDB will send those details to your code, as you can see in Listing 5.2, where we open the subscription and 
...
There isn't much in Listing 5.2. We open a subscription using the subscription name, then we `Run` the subscription, I redacted the actual batch processing
for now, we'll see what it is in the next section. After calling `Run` we wait on the returned task, why do we do that?
...
example. And if it processed all the data that the database had to offer, it will simply sit there and wait until it has something more to give it. However,
...
Next, we might have been kick off the subscription for some reason. 
The admin may have deleted the subscription or database, the credentials we use are invalid, or the subscription connection has been taken over
...
The actual batch processing of the documents is done by the lambda that is passed to the `Run` call  on a background thread. In Listing 5.2 we used the  
the subscription name we previously created. This is typical to how we are using subscriptions. The subscription id should be persistent, usually either
hard coded or configured becuase it is used to represent the state of the subscription and what doucments have already been processed by that 
particular subscription.

You can set your own subscription id during the `Create` call, which give you well known subscription name to use, or you can ask RavenDB to choose one for 
you. Note that even if you use your own subscription name, it still needs to be created before you can call `Open` on it. 
...
JavaScript expression (see Figure 5.4) and send it to the server, to be evaluated on each doucment in turn. Any matching document will be sent for the client for 
processing.
...
but also the change vecotr and id for this document. This ensures that when we call `SaveChanges`, if the document has changed in the meantime on the server
...
> change vector, as we discussed in the previous chapter. However, reading from one node and writing to another can lead to some funny situation where the
> node that you are subscribed to and reading from is up, but the node you are trying to write to is not. That can cause an error, subscription batch failure
> and unnecessary work. It is generally better to write back to the same node that we are reading from.
>
...
> created will operate against the same node that you are reading from and thus allow us to reason about the state of the system with fewer variables. 
...
> described^[That particular lecture was over a decade ago, but I still vividly remember it, it was _that_ good.] the problem far better than I 
> could:
>
...
>
> Any time that you need to run non trivial code you need to consider how you'll debug it. The subscriptions RavenDB offer gives you a _lot_ of power
> and they can carry the load of significant portions of your business logic. Because of that, it is very important that you'll be able to test and play
> with them as you develop them.
>
> For that reason, RavenDB has the subscription test feature. This feature will apply the subscription logic on top of the database and allow you to see
> the results of your filter or transformation scripts. We have already see a taste of that in Figure 5.4, but the more complex your scripts are, the more
> useful this feature become.
>
> There is one item to consider here, though. Subscription test work just like a subscription. It matches all the documents in the particular collection
> you are operating on and apply the script to each and every one of them, giving you those that matched successfully. However, if your script filters 
> a lot of the matches and you have a lot of data, it may take the subscription test a while to find enough documents that will match your subscription
> filter.
> 
> You can reduce that time by selecting a document that it start the scanning process from, so the test doesn't have to scan all the documents in the 
> collection. This is one of the options in `Send Documents From`. While you'll typically use "Beginning of Time" as the default, it can save some time
> by positioning the subscription directly on the first of the documents you want to process.
...
Systems typically accure additional behavior over time. As business processes mature and evolve so does the demand on our systems. The way we talked about
subscription so far is mainly as a core part of the application itself. Something that the developers would write, build and manage. But that doesn't have
to be the case. In fact, it is very common that subscriptions are used as "aftermarket" entry point for modifying, extending and supporting the system.

Consider the case of wanting to send all new users a welcome email after they registered. We _could_ do that as part of the application itself, but given
...
using the Python client API in Listing 5.20 to perform this task (I could have used node.js, Ruby, etc. I'm using Python as the example here because it 
is a great scripting language which I'm very fond of).
...
The code is very similar to the way we would do things in C#. We open a document store, open the subscription and pass a method to process the document 
...
or a horrible one, mind.

> **Subscription in a dynamic language**
> 
> A major advantage of writing such scripts in a dynamic language is that the low overhead and  friction.
...
> your subscriptions code in step with the rest of the code. 

Subscriptions, even if they are written in another platform, are still considered to be a part of your system. RavenDB is meant to be _an_ application
database, not a shared database between a multidudes of applications using the same data. Even if there are additional processes going on, they should
be (eventually, at least) all owned by the application itself.

The beauty in using a scripting langauge for those kind of tasks is that they allow you to just setup a subscription without any sort of application, 
...
One thing that _isn't_ on Listing 5.20 is error handling. In previous examples in this chapter I spent some time discussion error handling and any
subscription scripts you have most certainly do need to have something there. But a conscientious admin will already setup a watchdog for this, report
errors and raise alerts accordingly. Any production system is made of a lot of moving parts, and it is very likely that the infrastructure to mangage 
such scripts, retstart them if there is an error and alert if they haven't recovered is already in place. 

I'm mentioning it because it is important that it _would_ be in place. The nasty side of this kind of approach is that it is easy for what will turn
...
somewhere, and it will continue to do so until a reboot will happen, at which case stuff breaks, and no one really know what. 
