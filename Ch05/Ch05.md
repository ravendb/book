...
Those are our the documents in the `Customer` collection, but what does this mean? A subscription is a way to go through all documents that match a query. 
...
The actual batch processing of the documents is done by the lambda that's passed to the `Run` call on a background thread. In Listing 5.2, 
we used the subscription name we previously 
created. This is typical to how we're using subscriptions. The subscription ID should be persistent, usually stored in configuration or loaded from the database.
...
Figure 5.4 shows the newly created subscription on the server side. You can see that the filter we provided on the cilent side was turned into a server 
side script that decided whatever a particular `SupportCall` is a match for this subscription.

![This subscription filters support calls on the server side](./Ch05/img04.png)
...
subscription (this is identical to Listing 5.4). For each item in the batch, we'll create a new session, load the customer for
...
the client API was able to turn that into a query that was sent to the server. Listing 5.4 was a pretty simple expression, and the code
...
> the results of your filter or transformation scripts. We've already gotten a taste of that in Figure 5.3, but the more complex your queries are, the more
...
What we're doing in Listing 5.10 is filtering the support calls. If the call has less then 10 votes, we'll just filter it. 
...
to the sleep time until a certain maximum (such as 1 minutes). In many cases, this is enough to handle transient errors without having to get an admin
...
You may decide that you want certain operations to be handled on one workder node^[I'm using the term worker node here to refer to machines that are running business processes, 
...
on any other worker nodes. This way, a particular subscription has a preferred workder node that it will run on, but if that one fails, it will run on another.
In other words, each worker node will run only its own preferred subscriptions unless there's a failure. 

When your deployment contains mutliple worker nodes that are all processing subscriptions, you need to be even more aware of the fact that you're running in a 
...
nature of Ravendb in Chpater 6 in depth.
...
The good news is that we can utilize another RavenDB feature, revisions (dicusssed in Chapter 4), to allow us to see all the changes that 
