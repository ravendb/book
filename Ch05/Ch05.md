...
polling and remembering the last items that you read. RavenDB's subscription also includes error handling, recovery, retries and on-the-fly updates. 
...
Those are our the documents in the `Customer` collection, but what does this mean? A subscription is a way to go through all documents in a collection. On its own, it isn't really
...
by another client. As you can see, most times the `Run` method will return only if you manually disposed the subscription. For most other cases, it will throw 
...
will be sent. It's the acknolwedgement of the processing of the batch that makes this process reliable. Until the client confirms receipt of the batch,
the server will not move forward and send the next one. 
...
We've registered for subscriptions on support calls that have more than 10 votes and over 25 comments, and we add a flag to record whether or not we've sent the
...
full query (see Figure 5.4) and send it to the server to be evaluated on each document in turn. Any matching document will be sent to the client for 
processing.
...
>
> The code in Listing 5.5 uses `batch.OpenSession` instead of `store.OpenSession`. Why is that? Well, this is where I need to skip ahead a bit and explain some 
...
> change vector, as we discussed in the previous chapter. However, reading from one node and writing to another can lead to subtle concurrency issues. Those are
> handled by RavenDB, but can be surprising to the users. It's generally better to write back to the same node you're reading from.
>
...
In Listing 5.5, the concurrency exception is an expected error. We can just ignore it and skip processing this document. There's a bit of trickery involved here. Because the document has changed, the subscription will get it again anyway, so we'll skip sending an email about this call for now. But 
...
the client API was able to turn that into a query that was sent to the server. Listing 5.4 was a pretty simple expression, but the code
...
the Linq expression and go directly to the query. In Listing 5.8, we're going to see how we can subscribe to all the support calls that require 
special attention.
...
of a support call, we want to give the call some special attention via this subscription. We do that by simply scanning through the array of `Comments` and checking
...
> the results of your filter or transformation scripts. We've already gotten a taste of that in Figure 5.4, but the more complex your queries are, the more
...
What we're doing in Listing 5.10 is filtering the support calls. If the call has less then 10 votes, we'll just filter it. However, we aren't limited to just
filtering the full document, we can also return an object of our own. That object can be built by us and contain just what we want to send directly to the
...
there are actually more options available to us that we haven't gotten around to yet. Consider Listing 5.6. There, we get the support call and immediately
...
terms of overall performance. Even with the optimization we implemented in Listing 5.7, there's still a remote call to do. We can do better.
...
When processing the output of this subscription, we can directly process the results without making any other remote calls, even to load the associated document. In Listing 5.13, you can see how we'd process such a subscription.
...
we call to `Run`, we need to wait on the resulting task. If an error is raised during document processing, the subscription will close and that 
...
All other exceptions would typically be raised from within the batch processing code. In that case, we use the `errorTimings` queue to check if we 
...
You may decide that you want certain operations to be handled on one node and configure those subscriptions to `TakeOver` on that node and `WaitForFree`
on any other node. This way, a particular subscription has a preferred node that it will run on, but if that one fails, it will run on another. In other words, each machine will run only its own preferred subscriptions unless there's a failure. 
...
The good news is that we can utilize another RavenDB feature, revisions (we looked at it in ["Advanced Client API"](#advanced-client-api)), to allow us to see all the changes that 
were made to a document. 

The first step to exploring this feature is to enable revisions on the collection you want. In this case, it's the `Customer` collection. Set the minimum retention
...
The beauty in using a scripting language for those kind of tasks is that they allow you to simply set up a subscription without any sort of application, 
...
errors and raising alerts accordingly. Any production system is made of a lot of moving parts. It's very likely that the infrastructure to manage 
...
Finally, we looked at how we can manage subscriptions in the RavenDB studio, looking at subscription states, editing them and, in general, verifying that
