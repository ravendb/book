...
previous chapters, but with distributed database we also need to take into account eventual consistency and conflicts between machines. We'll talk in 
[Chapter 5](#clustering-setup) about such topics in depth.
...
The good news is that we can utilize another RavenDB feature, Versioning (we looked at it in [Chapter 4](#client-api)) to allow us to see all the changes that 
...
> topic in [Chapter 7](#integrations).
