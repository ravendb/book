...
> 
> In Listing 5.1, we have used the `Eagerly.ExecuteAllPendingLazyOperations` method to force all the lazy values to execute. But that is merely a convienance method. We could have done the same by just calling `lazyOrder.Value`. 
>
...
Because of that, the `BatchSize` option also control how many documents we can have waiting to be sent. The queued documents will be up to 150% of the size of the `BatchSize`. Assuming the `BatchSize` is set to 512, then the maximum number of pending documents will be 768. At this point, if you try to `Store` an additional document, you'll be blocked until another batch has been processed and space on the queue is freed.
...
Whenever a serializer is created by RavenDB, this event is called and allow you to define the serializer's settings. You can see an example of that in Listing 5.13.
...
```
The idea in Listing 5.13 is to have a `Money` object that holds both the amount and the currency, but to serialize it to JSON as a string string property. So a `Money` object reprensenting 10 US Dollars would be serialized to the following string: "10 USD".
...
Result transformers are pretty cool, being able to select which properties to project can save a lot in bandwidth. But what make them a truely awesome feature is the ability to reference other documents. Listing 5.16 shows a much better version of the transformer.
