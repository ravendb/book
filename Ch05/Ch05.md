...
Then it will execute them one at a time. Since they represent changes to the document, that is safe to do. So one of the main usecases for patching is to concurrently update documents. That said, note that if you have a _lot_ of patch requests to the same document, eventually the internal queue RavenDB uses will be full and patch requests will be rejected. 

So far, we have talked mostly about patching a single document. And I've been careful to give you all the reasons why you _shouldn't_ be using this feature. So why is it there in the first place? The answer is in the next section. It is usually a bad idea to work with patching on a single document, but when we are working with sets, it is pretty much the way to go.

## Set based operations

Unlike a relational database, RavenDB
[[NEXT]]
