...
The answer is simple. Any time a RavenDB server talks to a node, it will identify itself using the server 
...
We've talked so far about certificates and trust within a single cluster. We also need to talk about how certificates and authentication work when you're running multiple clusters that need to communicate securely amongst one another. Whenever a RavenDB server 
...
maps certificates internally to security clearances and permissions for specific databases, and that the server's
