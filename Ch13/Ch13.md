...
A client trust the server because the server's certificate is signed by a trusted party. For the server to trust a client 
certificate, that certificate needs to be _explicitly_ registered in the cluster. RavenDB does not delegate trust to a 3rd party
...
and deploying it directly. In this case, we'll be using the setup wizard to get things going. This simplify the process of setting
...
> * On Linux, using port 443 usually requires root priveleges. You can either use a higher port (such as 8443) or use 
...
The automated setup is nice. I'm really happy about how it saves a lot of time and effort. It's a feature aimed primarily at developers running a secured system locally. It is also useful for small applications that don't get a lot of operator attention.
...
This service is provided (free of charge) as a courtesy for RavenDB users. For mission critical systems, 
...
address on this node." For production systems, you are encouraged to run RavenDB on your own DNS and using your own certificates.
...
> If a user has permission to create a static index, they have the ability to write code that does _anything_ at all.
>
...
RavenDB makes no attempt to accomodate all of them. Instead, we provide the following options:
