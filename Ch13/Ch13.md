...
We'll go through exactly how set up RavenDB securely in the next section. But first, I want to focus on the general theory and what exactly
is going on. **(((Oren: can we delete this paragraph? The next section is so close, that it seems to make more sense to me to just go into: "Client certificates are easy to get...")))**
...
complex to get ahold of because you're dealing with issues of trust. In many organizations, access to certificates is tightly controlled
(and right so). 
...
and deploying it directly. The first time you start a RavenDB instance, you'll be directed to the setup wizard where
...
> 
> Enter `Let's Encrypt`. Its mission is to bring HTTPS everywhere by providing free, trusted, fully automated certificiates to all. The only requirement of `Let's Encrypt` is that you must prove that you own the domain
...
> `Provide Your Own Certificate` option. `Let's Encrypt` is available to make it easier for you to set up RavenDB in a secured manner, should you choose to use it.
>  
...
> any external access.
>
> To alleviate much of the complexity of such a deployment, RavenDB takes it upon itself to handle all of that **(((Does "all of that" refer to certificate generation or domain management?)))**. The RavenDB team is
...
>
> If there are any issues during the process, RavenDB will alert the operations team such that you have plenty of time to resolve it. It will also retry on regular basis to avoid being blocked by any transient errors. 
...
This service is provided as a courtesy for RavenDB users, but for mission critical systems, we recommend that your operations team
...
RavenDB explictly does not offer a read only access mode. If you want a user to be able to access your data without being able to modify it, set up external replication (or even RavenDB ETL, 
...
as other nodes, the new node intrinsically knows that it can trust the other side. This greatly simplifies the process of 
setting up a secured cluster.

You _can_ run RavenDB a cluster with a different certificate for each node, but you'll need to register all the certificates
...
We've talked so far about certificates and trust within a single cluster. We also need to talk about how certificates and authentication work when you're running mulitple clusters that need to communicate securely amongst one another. Whenever a RavenDB server 
...
RavenDB makes no attempt to accomodate all of them. Instead, we offer a choice. **(((After indicating that there's a choice, can you immediately state what the choice is? Otherwise, it seems like you're only talking about one option (the certificate being located in the server's folder).)))**
...
also need to be able to keep track of who has connected to the system and when. RavenDB supports the process of access audits at the level of 
database connection.
...
done for performance and managability reasons; otherwise, you'd have extremely large and unwieldy audit logs. With HTTP 1.1, a single TCP connection is used for many different requests. The only items
...
don't need security (for example, if you're working locally on a development machine), you can choose to run in unsecured mode.
...
and small applications. If you're setting up everything yourself, you'll use the same setup process;
