...
> things in a meaningful way. A much better way to state such a request would be to use a similar format to that in Table 16.1. 
> 
...
> |        200 | 99.99% |               350 | 
> 
> Table 16.1: SLA table allowing for max response time for requests under different loads
>
> Table 16.1 is actional. It tells us that under a load of 100 requests per second, we should complete 
...
>
> If you are running in the cloud, you should take a look at [cloud.ravendb.net](https://cloud.ravendb.net), which can
> provide you with database as a service option. The details of this chapter are still important, even on the cloud,
> but you can utilize the RavenDB Cloud offering to remove a lot of the decision making and cost of running RavenDB.
>
> RavenDB Cloud allows you to select which cloud provider and region you want to run on and then allows you to chose
> the right size for your needs. You will also be able to scale your cluster instances up and down as needed, add 
> additional compute / RAM or storage space, without any downtime and in mere minutes.
>
> When running on RavenDB Cloud, tasks such as updating the underlying system, keeping up with RavenDB updates, and
> in general all devops tasks are handled by the cloud platform, requiring no attention from you or your team.
...
I want to refer you back to Table 16.1. Before you can make any decisions, you need to know what kind of requirements 
...
Figure 16.1 shows a couple of graphs from a production system. This isn't peak load, but it isn't idle either.^[All
...
![Load and resource graphs from the RavenDB dashboard on production system](./Ch16/img01.png)
...
well, as you can see in Figure 16.2. (In the Studio go to `Manage Server` and then `Traffic Watch`).

![Request latencies tracked using the RavenDB Traffic Watch feature](./Ch16/img02.png)
...
clearly seen in Figure 16.1, and that can help you figure out what kind of system you want to run this on.
...
> what kind of resource usage they're going to need. In Figure 16.1, you can see that RavenDB divides requests into
...
example of this topology in Figure 16.3.

![A single node option hosting multiple databases](./Ch16/img03.png)
...
replication factor of two or three. Figure 16.4 shows a three-node cluster where the 'Helpdesk' and 'Users' databases 
have a replication factor of three, while 'Fulfillment', 'Ledger' and 'Orders' databases have a replication factor of two.

![A three-node cluster with each database residing on two or three nodes.](./Ch16/img04.png)

In the three-node cluster mode shown in Figure 16.4, we know that we can lose any node in the cluster and have no issue continuing 
...
>
> The topologies shown in Figure 16.4 and Figure 16.5 showcase a deployment method that ensures that as long as
...
> depth in Chapter 7) with a multi-master architecture. 
>
...
Another classic setup is the five-node cluster, as shown in Figure 16.5. Each node in the cluster contains three databases
...
![A five node cluster with each database residing on three nodes.](./Ch16/img05.png)

There are many cases where you can live with a lower redundancy level than the one shown in Figure 16.5. Having a five-node cluster with each of the databases having a replication factor of two is also an option. Of course, in this mode, 
...
You can see how this topology looks like in Figure 16.6.

![A large cluster divided into full members and watchers](./Ch16/img06.png)
...
can be seen in Figure 16.7. You have a primary server that does all the work and a secondary server that 
replicates the data. This is also called active/passive topology.

![A primary server replicating to a secondary](./Ch16/img07.png)
...
Figure 16.8 and on the Studio under `Settings` and then `Manage Database Group`. 

![Showing the priority order for clients to access nodes in a database group](./Ch16/img08.png)

As you can see in Figure 16.8, the admin can control this order. This is designed not just to comply with
...
in Chapter 8. Using external replication, you can control exactly where and how the data will travel. You aren't limited
...
in another RavenDB cluster (not a single node) is also very important. Consider the topology in Figure 16.9.

![Two clusters in different data centers connected via external replication](./Ch16/img09.png)

In Figure 16.9, you have two independent clusters deployed to different parts of the world. These clusters are 
...
is that you can distribute the load among all the instances you have. You can see how that works in Figure 16.10.

![The database group topology and task distribution among the nodes](./Ch16/img10.png)
...
Using a database's data directory as the base, RavenDB uses the format shown in Listing 16.1.
...
Chapter 14, all communication channels used when RavenDB is running in a secured mode are using TLS 1.2 to secure 
the traffic. 
...
automatic certificate generation if you're using RavenDB's Let's Encrypt integration (see Chapter 14). 
...
that many file descriptors.^[See Listing 16.1 to get a good idea of the number of files RavenDB will 
