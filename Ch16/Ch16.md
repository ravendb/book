...
As part of its a database, I/O _matters_ to RavenDB. In particular, I/O performance matters. A lot of problems in production can be tied directly to issues with I/O. And these issues typically come with one (or both) of the following:
...
number one hot path (do you mean "clue" here?) for any issues with slow writes.
