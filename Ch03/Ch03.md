...
RavenDB doesn't impose any additional costs on storing / loading documents by default, so you get to use a very fast database 
...
* You can generate the document ID independently of the collection used.
...
* You can perform searches using the document ID as a prefix (and even using glob like searches).
...
>
> A common bad practice is to integrate between different applications using a shared database. This has been known to be a bad
...
and we'll start with the most obvious one. In this model, we don't have documents, we have _a_ document, just a single one per child. That is 
...
So if the default approach is to embed data, when _wouldn't_ we want to do that? There are a few cases, primarily when the data doesn't
...
will be almost as good (we discuss them in the ["Advanced client API"](#advanced-client-api) chapter) and save us the need to split our data. 
...
Another scenario that might require you to split a document is if you have a real need to have concurrent activities on a document. This is
pretty rare in general, since a document is the unit of concurrency in RavenDB, you'll model your documents so they'll have a single reason
...
for tracking so they can see what kind of steps a particular order goes through and may take action at various points in the workflow. Such
...
this order. This make it an explicit actuion in your domain, instead of just tacking it onto the existing objects.
...
Reference data is very common, it can be anything from a list of states to the tax rates, localized text and to dispaly on the screen. The 
...
more detail in ??XYZ_XYZ??, but RavenDB has builtin revisions capability, which can be _very_ useful when you are talking about reference
...
employee's record, we'll have a referecnce to `contracts/hourly/1234-A`, which is the current version of the contract, but when issuing
...
A common modeling technique is to keep track of some cached global state inside our entities. The most trivial example I can think about is 
...
aggregation. We dedicate a whole chapter ([Map/Reduce](#map-reduce)) to discussing this topic, so I won't go into the details here, but the impact
...
That lead to a set of interesting problems regarding concurrency. What happens if two requests are trying to modify the same document at
...
modifications one at a time, and the last one would win (there is no way to control which one that would be)^[While only the last operation
would be visible, it is important to note that _both_ operations will execute. That means that in certain cases, it will leave marks. In 
particular, if you have revisions enabled for this collection, there are going to be two revisions for this document. We'll discuss revisions
in the next chapter].

> **Change Vectors in RavenDB**
> 
> RavenDB make extensive use of the notion of change vecotrs. A change vector is composed of a list of node ids and etags. A node id uniquely
> identify a node, while an etag is a 64 bits number that is incremented on every operation in a database. Each
> time a document is modified is it associated with the current etag value and a new change vector is generated. 
>
> This change vector uniquely
> mark the specific version of the document globally and is used for optimisitic concurrency control, for
> various internal operations and caching. 
> We'll discuss change vectors in detail in the ["Clustering Setup"](#clustering-setup) chapter.
...
is generating the ID, it _knows_ that it is meant to create a new document, and it sets the expected change vector to be empty, 
so if the document already exist, and error will be generated.
...
In either case, the RavenDB Client API will send the expected change vector to the server, which will compare it to the current change vector.
If the current change vector does not match the expected change vector, a `ConcurrencyException` will be thrown, aborting the entire transaction.

The first two options uses the change vector that the server supllied when the document was loaded, in other words, they would error if the 
...
The ability to specify the change vector on a specific entity is quite important, because that allow you to perform _offline_ optimistic concurrency
checks. You can render a web page to the user, and along the document data there, include the change vector that you got when you loaded that 
...
document with the change vector as it was when we rendered the page to the user, and then call `SaveChanges`. If the document has been modified by
anyone in the meantime, the operation will fail.

In this manner, you don't need to keep the session around, you just need to pass the change vector back and forth. As you can see, the change vector
is quite important for concurrency, but it plays another important role with caching.
...
`ETag` header. In the case of loading a single document, that `ETag` header is also the document change vector, but if we are querying or loading
multiple documents that `ETag` header will contain a hashed value. 
...
Earlier in this chapter we looked at various types of relationships between objects and documents. From embedded value inside a larger
...
be something that we need to keep^[A signed PDF invoice is pretty common, and you are required to keep it for tax purposes, and cannot just
...
RavenDB's answer to that need is attachments. Attachments are binary data that can be attached to documents. An attachment is always attached to a
document, have a unique name within the scope of the parent document and have a binray data. Attachments are
...
Healthcare, for example] we need to be able to show all changes that happened to a document. RavenDB support that using the revisions
feature.
...
only specific collections and only keep track of the last N revisions and to a minimum amount of time, but it is very common to just say "track everything", since
...
In RavenDB, all operations that operate on a document or an attachment using their ID (put, modify, delete) are always consistent and run in an
...
>
> Waiting for indexing or waiting for replication is something that you have to explcitly ask for (either during the write or when you query
> the index), because it costs, but what _are_ those
...
> it may take longer, because the write will have to wait for the index to catch up not just with our write, but with all the writes that 
...
> if we are holding hundreds of threads open, but additional waits for the request is something we want to avoid.
>
...
We started by looking at RavenDB as a simple key/value store. Just throw data in (such as session information, shopping cart, etc) and access it by well known
ID. That has the advantage of giving us very fast operations, but without hiding the data behind an opaque barrier. We can also use the expiration feature
...
size for a document in RavenDB. We looked at concurrency control and chanve vectors which are useful for optimistic concurrency and for caching and why we should avoid caching aggregated data in our model (`NumberOfOrders`).

We looked at handling binary data with attachments, auditing and change tracking using the revisions feature and learned that we can expire (we repeating what we said earlier) documents natively
in RavenDB. Reference handling at indexing and query time was briefly covered (we'll cover it in depth in the [Glorious Indexing](#advanced-indexing)) as it is important for how you model your documents.
...
such as `accounts/1234/txs/2017-05` or having meanigful document IDs such as `config/states` helps a lot in setting out the structure for your model.
