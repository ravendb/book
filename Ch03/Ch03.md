...
that a document might very well be composed of additional datum. For example, when working with an order document, the invoice PDF might
...
all the expired documents automatically. This is a relatively small feature, but it's nice to be able to implement features such as
...
it works, but it should be noted that the expiration is a soft one. In other words, if you speciifed an expiration time, the document might 
...
By default, when you save a document into RavenDB, we'll aknowledge the write when the data has been saved on one node in a durable 
...
right there in the main pathway for updating data, which explains why they can so severely degrade performance. When desiging our indexing
...
> your cluster is going over a public internet connection.
>
> For indexing, this is usually a short amount of time since the indexes are updated quickly. If there's a new index,
> it may take longer because the write will have to wait for the index to catch up not just with out write but also with all the writes that 
...
> that gives us a budget of 1/10 of a millsecond to process a request. Inside RavenDB, requests are handled in an async manner, so it isn't as
...
From the trivial model of key/value store, we moved to considering real document models. By looking at how documents in the real world&mdash;phyiscal ones&mdash;are 
modeled, we made it much easier to explain document models to the business. Reducing the impedenace mismatch beteween the
...
more advanced modeling techniques, such as how to handle references and configuration data and how to deal with temporal infromation and  hierarchical
structures.
...
A repeating theme was the the use of semantic IDs to give you better control over documents and their meaning&mdash;not so much for RavenDB's sake^[The database 
