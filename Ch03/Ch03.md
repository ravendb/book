...
information relevant to processing it. We aren't limited by format of schema; we can represent arbitrarily complex data
...
transparentely move between different documents. Each document is a standalone entity. This helps ensure you don't create silent
...
> a child to a parent, we can use the `Include` feature to reduce the number of calls. On the other side, `Include` wouldn't work, but we
...
more detail in ??TODO??, but RavenDB has built-in versioning capability. This can be _very_ useful when you're talking about reference
...
reports, for example) are easy and natural, and it works well with caching and the use of `Include`.
...
This works as long as we only need to handle a single level, but it doesn't handle hierarchial queries. Finding all the departments under
...
ourselves up to choose whether we want the point-in-time information or the current (continously updated) version. 
...
case of an order document. In retail, an order can contain up to a few hundreds items, but in business-to-business scenarios, it's not
...
> much what we're doing. Instead, we'll typically accept indiviual requests from each contact person/department, and associate them 
