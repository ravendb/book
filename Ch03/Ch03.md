...
aggregation. We dedicate the whole of [Chapter 13](#map-reduce) to discussing Map/Reduce, so I'll go into the details here, but the impact
...
done using an `Include` call, which we already covered in [Chapter 2](#zero-to-ravendb). I'm pointing this out again because this is a 
...
We'll discuss listeners and their use in more dpeth in [Chapter 4](#client-api).
...
We'll see how to use the expiration feature in [Chapter 4](#client-api). There isn't much to say about the expiration feature beside that
...
the `WaitForReplicationAfterSaveChanges` method) for additional safety. We discuss  all such details in [Chapter 5](#clustering-setup).
...
This is an optional feature, you can ask RavenDB to wait for the indexes to update (using the `WaitForIndexesAfterSaveChanges` method) so you
have the option of choosing. If this is a write that you intend to immediately query, you can force a wait, but usually that isn't require, so
