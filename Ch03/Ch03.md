...
> A join will modify the shape of the output, it combine each match row from one side with each matching row on the other, sometimes creating Cartesian Products that are can cause night sweats for DBAs.
> 
...
So far, we have shown only synchronous work with the client API. But async support is crucial for high perfomance applications, and RavenDB has full support for that.
