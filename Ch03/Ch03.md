...
more detail in ??TODO??, but RavenDB has built-in revisions capability. This can be _very_ useful when you're talking about reference
...
In either case, the RavenDB client API will send the expected change vector to the server, which will compare it to the current change vecto. If the 
current change vector does not match the expected change vecto, a `ConcurrencyException` will be thrown, aborting the entire transaction. 
