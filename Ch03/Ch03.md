...
clear separation of documents. But you can't always split things apart on such a convienent boundary. What happens if there isn't a
...
> time a document is modified, it's associated with the current etag value. The etag is used for optimisitic concurrency control, for
...
RavenDB uses `REST` over `HTTP` for communcation between client and server. That means whenever you load a document or perform a
...
that something have changed and that we need to call back to the database to see what that change was. For certain types of data&mdash;config/reference documents in particular&mdash;that can provide major performance savings.
