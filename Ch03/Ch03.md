...
(Maybe talk here about blog and blog posts, which we want to separate to avoid concurrent issue when modifing an exisitng blog item which might also get comments from the users - this will be repeated also later on in concurrency section)
...
will execute, and if revisions feature is enabled each operation would create a revision. (I think that we better off not mention revisions here.)

(Also it worth document here the blog and blog posts case from the comment before)

> **ETags in RavenDB** (That was changed, etags are not viewable to the user anymore)
> 
...
is generating the ID, it _knows_ that it intend to create a new document, and it sets the expected (CV) accordingly (actually an empty etag), so if the document
...
Or on a single particular operation:
	...
	session.Store(entity, CV);

In either case, the RavenDB Client API will send the expected CV to the server, which will compare it to the current CV. If the 
current CV does not match the expected CV, a `ConcurrencyException` will be thrown, aborting the entire transaction.
...
document with the -etag- as it was when we rendered the page to the user, and then call `SaveChanges`. If the document has been modified by
...
> it may take longer, because the write (why talking here just on writes and not mention that we can wait also on reads?) will have to wait for the index to catch up not just with our write, but with all the writes that 
...
> allow RavenDB the freedom to optimize its operations. (we say the same in the next paragraph)
...
to store time dependent data.(we say this also later)
