...
https://www.amazon.com/Domain-Driven-Design-Tackling-Complexity-Software/dp/0321125215[Domain Driven Design book] is a bit dry, 
but I remember being very impressed when I read it the first time.] and, in particular, about the concept of an `Aggregate Root`.
...
with the simplest of all access models. Typically, the complexity in key/value modeling resides in generating the appropriate key and in 
...
key/value operations. If you're storing shopping cart data inside RavenDB, you can also start pulling data from the stored carts. 
You can see the most popular item being purchased or do projections on inventory or any of a whole host of things that will provide you 
...
with RavenDB? We'll talk about that a lot more in <<etl-integrations>>, but the answer is that each application
...
independent existence. And that's how you treat documents: as entities. If a document exists only as part of a larger whole, then the 
whole thing should be re-factored/re-modeled to a single document.
...
the process a normal government behavior/bureaucratic/Kafkaesque.footnote:[Circle the appropriate choice.] If I gave you a form and told you 
...
Anyone taking a relational modeling course knows that "store a fact only once" is a mantra that's just short of being sacred (or just plain sacred).
The basic idea is that if you store a fact only a single time, you are preventing update anomalies, such as when you updated 
...
it _looks_ the same. Let's consider the notion of a `Customer`, an `Order` and what the `Address` property means in this case. The 
`Customer` entity has an `Address` property that holds the address of the customer, and the `Order` has a `ShipTo` property for the 
shipping address.
...
covered later in <<querying-in-ravendb>>), but you need to consider whether it make sense to traverse document references in your model or
...
We're going to keep using the same online retail order system we've been working with so far. You're already familiar with the concepts of orders, 
companies, employees, shipping details, etc. That means we can skip past the domain and focus on the interesting bits: the modeling 
decisions themselves. 

However, we're _not_ going to use the same model from <<zero-to-ravendb>>. Instead, we'll start with a deliberately 
naive version shown in <<simple-order-model>>, and then walk through the modeling decisions that will improve it step by step.
...
public class ShippingDetails
{
    ...
    public ShippingPriority Priority { get; set; }
}
...
The code in <<simple-order-model>> is obviously a simplified model, but it's a good place to start our discussion. A core tenet of modeling in 
...
With the basic model in <<simple-order-model>> and our understanding of an order system, we can now proceed to explore aspects of document modeling. 
We'll start with the most obvious one. In this model, we don't have documents, we have _a_ document: just a single one per order. That's 
because we're embedding all information about the order in the document itself. Why would we do that?
...
each class shown in <<simple-order-model>>. A concrete example might work better, and <<order-record>> shows how this model works 
for an order in our system. 

[[order-record]]
.Order record for Alfreds Futterkiste
[source,json]
----
// orders/10248-A
{ 
    "OrderedAt": "2024-07-04T00:00:00.0000000Z",
    "Customer": {
        "CompanyName": "Alfreds Futterkiste"
    },
    "Employee": "Anne Dodsworth",
    "Shipping":	{ 
        "RequireAt": "2024-08-01T00:00:00.0000000Z",
        "Priority": "Standard"
    }
}
----

All of the information is in one place. For the document ID you can see that we used a sequential ID, `orders/10248-A`. This 
works well for order data. The data itself is centrally located and easy to access. Looking at the data,
it's easy to see all relevant information about our order.
...
In the case of our order record, the obvious example here is the company. "Alfreds Futterkiste" is an independent entity and is not 
fully owned by the order record. We need to split it into its own document. On the other side of things, "Alfreds Futterkiste" has placed more
orders than just this one. So we need to consider how to model such information.


#### Many-to-one relationship

How do we model an order system where multiple orders are placed by the same company, "Alfreds Futterkiste"? The technical term for this 
relationship is `many to one`. Unlike the previous example, where we embedded the customer info inside the order document, now we want to model
the data so there's a distinct difference between the different entities. You can see the document model as JSON in <<many-to-one-model>>.

[[many-to-one-model]]
.Many to one modeling with orders and companies
[source,json]
----
// orders/10248-A
{
    ...
    "Employee": "employees/8-A",
}

// orders/10249-A
{
    "OrderedAt": "2024-07-05T00:00:00.0000000Z",
    "Company": "companies/85-A",
    "Employee": "employees/9-A",
}
----

<<many-to-one-model>> shows footnote:[I removed all extra information from the documents to make it clearer.] two orders (you 
can figure out what other orders look like on your own) both referencing the same company. 
The orders only hold the _identifier_ for the related company and employee documents, nothing else.

How is this model reflected in our code? Let's look at <<many-to-one-code>> to see that (again, with some information redacted to make it easier
to focus on the relevant parts).

[[many-to-one-code]]
.Order class representing the model
[source,csharp]
----
public class Order
{
...
Instead of storing the company as an embedded document, we just hold the ID to that company. And when we need to traverse from the order
document to the company document, we do that by following the ID. To make things faster, we'll commonly use the `Include` feature to make
sure that we load all those documents in one remote call, but that doesn't impact the data model we use.

What about the other side, when we need to find all orders placed by a particular company? We use a query, as shown in <<company-orders>>.

[[company-orders]]
.Loading a company and its orders
[source,csharp]
----
using (var session = store.OpenSession())
{
    var company = session.Load<Company>("companies/85-A");
    var companyOrders = (
        from o in session.Query<Order>()
        where o.Company == company.Id 
        select o
    ).ToList();
}
----

You can also run this query directly in the Studio by going to `Indexes` and then `Query` and using the following query: 
`from Orders where Company = 'companies/85-A'`. 

As you can see in <<many-to-one-code>> and <<company-orders>>, we're being very explicit when we move between documents. 
RavenDB doesn't allow you to transparently move between different documents. Each document is a standalone entity. This helps 
ensure you don't create silent dependencies in your model since each document is clearly delineated. 

.Using lazy operations
****
In <<company-orders>>, you can see we have to make two separate calls to the server to get all the information we want. When going from
an order to a company, we can use the `Include` feature to reduce the number of calls. Going the other way, `Include` wouldn't work, but we
...
whether the association should cross a document boundary or remain inside the document. In the case of companies and orders, it's 
...
and, frequently, it's highly dependent on the way you _use_ the data. An equally valid decision in the order case would be to embed the
company's name in the order document and duplicate that information across all orders from
the same company.

It depends what kind of work is done with the company. If all or nearly all the work is done with the orders, there's no point in 
creating a separate company document. Consider a ticket sales system for a concert venue — each ticket purchase is a one-time event, and 
the buyer's name and contact details are only relevant in the context of that specific ticket. There's no ongoing relationship to track, so
embedding the buyer info directly in the ticket document is the natural choice. However, if we 
wanted to keep track of companies as well (for example, maybe we want to note their credit terms or preferred shipping method), then we'll likely use a separate 
document.

Orders and companies are all well and good, but what about when we ramp the example up a bit? An order can involve multiple employees
(the salesperson who took it, the warehouse worker who fulfilled it, the support rep who followed up), and each employee works on 
many orders. Now we need to talk about many-to-many relationships.
...
In our retail example, let's reconsider employees and orders. Up until now, we stored a single `Employee` on an order. But in practice, 
an order can involve multiple employees and each employee works on many orders. 

When we were working with companies and orders, it was obvious that we needed to place the association on the orders. But how should we model 
employee–order assignments? We can add an `Employees` array to the order document, which will hold all the 
document IDs of the employees involved. We can add an `Orders` array to the employee document and store the order IDs there. Or
we can do both. What should we choose?

In the context of many-to-many associations, we always place the record of the association on the smaller side. In other words, since an 
order is likely to involve fewer employees than an employee has orders, the association should be kept on the order 
document. 

.The users and groups model example
****
...
Traversing a many-to-many association from the order (smaller) side can be done by simply including and loading the employees,
as shown in <<order-employees>>.

[[order-employees]]
.An order and its employees
[source,csharp]
----
using (var session = store.OpenSession())
{
    Order order = session
        .Include<Order>(o => o.Employees)
        .Load("orders/10248-A");
    Dictionary<string, Employee> employees =  
        session.Load<Employee>(order.Employees);
}
----

Following the association from the other side requires us to query, as shown in <<employee-orders>>.

[[employee-orders]]
.Orders for an employee
[source,csharp]
----
using (var session = store.OpenSession())
{
    Employee employee = session.Load<Employee>("employees/7-A");
    List<Order> orders = (
        from o in session.Query<Order>()
        where o.Employees.Contains(employee.Id)
        select o
    ).ToList();
}
----
...
You can also run this query directly in the Studio by going to `Indexes` and then `Query` and using the following query: 
`from Orders where Employees[] in ('employees/7-A')`. 

The code in <<employee-orders>> will load an employee and all of their orders. In this case, we see a slightly more complex query,
but the essence of it remains the same as the one in <<company-orders>>. We have separate documents and clear separation between them. In other 
...
In this case, it might make sense to create a document just for the order header (call it `orders/82-A/header`). But using a projection
will be almost as good, and we'll discuss those in the next chapter. This saves us the need to split our data. The typical way you'll
build one-to-one relationships in RavenDB is to utilize document ID postfixes to create a set of related documents (`orders/82-A` and 
`orders/82-A/header`, for example).
...
Another scenario that might require you to split a document is if you have a real need to have concurrent activities on a document. 
This is generally pretty rare. Since a document is the unit of concurrency in RavenDB, you'll model your documents so they'll have a single reason
to change. But sometimes there are reasons to allow concurrent updates on a document. For example, if you have properties on the document
...
It's typical to ignore the modeling concerns for such items. Just throw them in a pile somewhere and not give it any thought. With 
...
configuration documents. You can see the example in <<states-configuration-document>>.

[[states-configuration-document]]
.Storing the states list as a configuration document
[source,json]
----
// config/states
{
...
Modeling reference data in the manner shown in <<states-configuration-document>> has several advantages. It's much easier to work with the data. 
Instead of issuing a query, we can just load the document in one shot, and it's ready for our consumption. It means the database has less work to
...
We'll discuss this in more detail in the next chapter. This can be _very_ useful when you're talking about reference 
...
This works as long as we only need to handle a single level, but it doesn't handle hierarchical queries.  In the sample dataset 
(the `Northwind` we created in <<zero-to-ravendb>>), we have the `Employees` collection, which has a hierarchical relationship. 
Each employee has a `ReportsTo` property, which holds the ID of the manager for that employee. You can see in <<employee-reporting-hierarchy>> 
how we use `declare function` to walk through the hierarchy and build a list of all the bosses for each employee.

[[employee-reporting-hierarchy]]
.Querying employees and their reporting hierarchy
[source,sql]
----
declare function reportingHierarchy(bossId) {
 const bosses = [];
 while(bossId){
     const e = load(bossId);
     bosses.push(e.FirstName + " " + e.LastName);
     bossId = e.ReportsTo;
 }
 return bosses;
}
...
select {
    ...
    Bosses: reportingHierarchy(e.ReportsTo)
}
----

<<employee-reporting-hierarchy>> uses a number of features that we haven't talked about until now. We use `declare function` to create a JavaScript
function that we can call from the query. This function uses `load` to load documents by their ID. In the `select`, we are using JavaScript notation
and can call perform computation (concatenation of the employee's name) or call the previous defined function.

Notice that we are querying from the bottom _up_, so we can walk the hierarchy. If we wanted to go from the top down, we'd need to do something else.
Asking for all the employees that report to `Andrew Fuller` (directly or indirectly) is a bit more complex. We'll need to define an index that is
...
use `Recurse` to work with hierarchical data and leave that topic to <<indexing-in-ravendb>>, where we'll cover it in depth.
...
.Temporal data vs. time-series
****
In the previous chapter, we talked about time-series data. Temporal data is different from time-series data. Time-series records the changes in a
value over time. Temporal data is data that has a relation to time, but it isn't necessarily a sequence of values. The classic examples for each is
tracking the changes in a stock's price over time (time-series) vs. the terms of a contract (temporal).
****
...
The same applies to contracts and other things that mutate over time. Consider the documents seen in <<contract-documents>>. They represent the same 
...
### Additional Document Modeling Considerations
...
****
RavenDB has a hard limit on document sizes, and it's around the 2 GB mark. (Documents are internally compressed by RavenDB, so a 2 GB JSON document
will typically be smaller inside RavenDB.) But the biggest document I recall seeing in the field was about 800 MB. We had a 
...
* Parsing 12 MB of JSON into domain entities (this often means using a lot more memory than just the 12 MB of serialized JSON).
...
a warning in the Studio when it encounters documents that are too large,footnote:[By default, that's set to 5 MB, and it's configurable.]
but it has no impact whatsoever on the behavior or performance of the system. 

There are two common reasons why a document can be very large. Either it holds a single (or a few) very large properties, such as a lot of text, 
binary data, etc., or it contains a collection whose size is not bounded.
...
a size limit and aren't accessed when loading the document, so that's a net win. We already talked about attachments in <<documents-extensions>>
and we'll discuss the role of attachment in document modeling toward the end of this chapter.
...
there's another, more theoretical aspect to the problems they bring: who _owns_ a very big document?
...
aggregation. We dedicated the whole of <<map-reduce>> to discussing this topic, so I'll go into the details there, but the impact
...
RavenDB is inherently concurrent, capable of handling hundreds of thousands(!)footnote:[Not a typo! In our benchmarks, we're always saturating the 
...
That means that in certain cases, it will leave marks. Consider two concurrent updates to `orders/10248-A`.
For example, consider the `Order` document from our order model. Suppose two users concurrently update the order record from
<<order-record>>. One transaction is updating the `Shipping.Priority` property to "High" as well as the `Employee` to
`employees/5-A` while the other transaction is updating the `OrderedAt` property to "2024-06-01" and the `Shipping.Priority` to "Rush".

.You won't usually see this behavior in practice, though.
****
Even when you run into concurrent updates, the idiomatic manner to update documents in RavenDB updates the _entire_ document, so you'll not see
partial modification of the document. If you'll use patching to update the document (discussed in <<advanced-client-api>>), you might get this behavior.
However, typically the only visible mark of the `last write wins` strategy is when you have revisions.
****

If the first transaction happens to be the last one to save, the resulting document will look like like <<order-after-concurrent-updates>>.

[[order-after-concurrent-updates]]
.Order record after concurrent updates
[source,json]
----
{
    ...
    "Shipping": {
        "Priority": "High",
        "RequireAt": "2024-07-04"
    }
}
----

In other words, the `Shipping.Priority` property was modified by both transactions, and the last one to save won. The `OrderedAt` and the `Employee`
properties were modified by only one transaction, so they were preserved.
...
This change vector uniquely marks the specific version of the document globally and is used for optimistic concurrency control, for
various internal operations and caching. We'll discuss change vectors in detail in the <<ravendb-clusters>>, since they are crucial to
understanding how RavenDB handles data replication.
****
...
The ability to specify the change vector on a specific entity is quite important because it allows you to perform _offline_ optimistic concurrency
...
document with the change vector as it was when we rendered the page to the user and call `SaveChanges`. If the document has been modified by
...
`Etag` header. In the case of loading a single document, that `ETag` header is also the document's change vector, but if we're querying or loading
multiple documents, that `ETag` header will contain a hash of the current server's state and the relevant documents.
...
that something has changed and that we need to call back to the database to see what that change was. For certain types of data 
(config/reference documents in particular, like the `config/states` we saw in <<states-configuration-document>>) that can provide major performance savings.
...
document, many-to-one and many-to-many relationships and more. While we were focused on building our model, I skipped a bit on explaining 
how we work with relationships beyond the diagram stage. I'd like to return to that now.
...
document. Going back to the order example, searching for all the orders whose company names contain "Chevalier" would be a great example. 
The order document no longer contains the name of the company, so how are we going to search for that? RavenDB doesn't allow you to perform
joins, but it does allow you to index related data during the indexing phase and then query on that. So querying the orders by their
company names is quite easy to do, using the `LoadDocument` feature. `LoadDocument` is discussed in full in 
<<indexing-in-ravendb>>.
...
be something we need to keep.footnote:[A signed PDF invoice is pretty common, and you're required to keep it for tax purposes and can't just
...
We discussed attachments previous in <<documents-extensions>>, I'm touching the topic here as well because it also has important implications
for modeling. Attachments are binary data that can be attached to documents. An attachment is always associated with a document, and beyond the binary data, 
there's an attachment name (which must be unique within the scope of the parent document). Attachments are
kept on the same storage as the documents (but can be set up to retire to S3 in a transparent manner), and can be worked on together with documents.
...
Tracking changes in data over time is a challenging task. Depending on the domain in question,footnote:[This is common in insurance and healthcare, for example.] 
we need to be able to show all changes that happened to a document. RavenDB supports that with its revisions
feature and we discussed that in <<documents-extensions>>.
...
to have a copy of all changes on a per-document level.footnote:[Obviously, this does not alleviate the need to have proper backups.] 
...
We'll discuss listeners and their use in more depth in <<advanced-client-api>>.
...
We'll see how to use the expiration feature in <<advanced-client-api>>. There isn't much to say about it besides that
...
can be quite expensive, since you need to talk to multiple machines for each operation, for example. On the other hand, `BASE` give us a lot 
more freedom, and that translates into a lot more optimization opportunities. 
...
fashion.footnote:[That means that the data has been written to disk and `fsync()` or the equivalent was called, so the data is safe from power loss.]
You can also ask the write to be acknowledged only when the write has been made durable on multiple nodes (using 
cluster-wide transactions or `WaitForReplicationAfterSaveChanges` method) for additional safety. We discuss all such details in 
<<ravendb-clusters>>.
...
****
Waiting for indexing, cluster-wide transaction or waiting for replication is something that you have to explicitly ask for because it costs. But what _are_ those
costs? In the case of cluster-write transactions or replication, it means waiting for another server (or servers) to get the information and persist it safely
...
you don't need to pay the cost here. You can also ask RavenDB to wait for the indexes during _queries_ (using `WaitForNonStaleResults` method). 

The overall idea is that you have the choice to decide for specific operations how you want to handle the tradeoff between waiting and consistency.
You need to be aware of the distinctions here between queries (which can be `BASE` and lag a bit footnote:[The typical lag time for indexing is under 1 ms.]) and 
...
From the trivial model of key/value store, we moved to considering real document models. By looking at how documents in the real world (physical ones, printed on actual paper) 
are  modeled, we made it much easier to explain document models to the business. Reducing the impedance mismatch between the
...
in RavenDB. Reference handling at indexing and query time was briefly covered (we'll cover it in depth in <<indexing-in-ravendb>>), as it's important
to how you model your documents.

A repeating theme was the use of semantic IDs to give you better control over documents and their meaning. Not so much for RavenDB's sake
foonote:[The database doesn't care what your document IDs look like.] but because it increases understanding and visibility in your domain. Using document IDs to "nest" documents
