...
parents, registrations, etc. On the face of it, this is a pretty simple model. We can model it using the code in <<simple-kindergarten-model>>.

[[simple-kindergarten-model]]
.Simple kindergarten model
[source,csharp]
----
public class Parent
...
The code in <<simple-kindergarten-model>> is obviously a simplified model, but it's a good place to start our discussion. A core tenet of modeling in 
...
With the basic model in <<simple-kindergarten-model>> and our understanding of a kindergarten, we can now proceed to explore aspects of document modeling. 
...
each class shown in <<simple-kindergarten-model>>. A concrete example might work better, and <<alice-kindergarten-record>> shows how this model works 
for registering Alice to our kindergarten. 

[[alice-kindergarten-record]]
.Kindergarten record of Alice in Wonderland
[source,json]
----
...
it's easy to see all relevant information about our Alice.footnote:[Except maybe which rabbit hole she wandered down...]
...
children than just Alice: there's also Harry and Edith.footnote:[The real Harry and Lorina had a total of 
https://en.wikipedia.org/wiki/Alice_Liddell[10 children], by the way.] So we need to consider how to model such information.
...
[[many-to-one-model]]
.Many to one modeling with children and parents
[source,json]
----
// children/alice-liddell
{
...
<<many-to-one-model>> shows footnote:[I removed all extra information from the documents to make it clearer.] both Alice and Henry (you 
can figure out what Edith's document looks like on your own) with references to their parents. 
...
[[many-to-one-code]]
.Child class representing the model
[source,csharp]
----
public class Child
{
...
What about the other side, when we need to find all of Lorina's children? We use a query, as shown in <<lorina-children>>.

[[lorina-children]]
.Loading Lorina and her children
[source,csharp]
----
...
`from Children where MotherId = 'parents/1923-A'`. 

As you can see in <<many-to-one-code>> and <<lorina-children>>, we're being very explicit when we move between documents. 
...
****
In <<lorina-children>>, you can see we have to make two separate calls to the server to get all the information we want. When going from
...
as shown in <<alice-grandparents>>.

[[alice-grandparents]]
.Alice and her grandparents
[source,csharp]
----
using (var session = store.OpenSession())
{
...
Following the association from the other side requires us to query, as shown in <<grandchildren-for-grandparent>>.

[[grandchildren-for-grandparent]]
.Grandchildren for a grandparent
[source,csharp]
----
using (var session = store.OpenSession())
{
...
You can also run this query directly in the Studio by going to `Indexes` and then `Query` and using the following query: 
`from Children where Grandparents[] in ('parents/1923-A')`. 

The code in <<grandchildren-for-grandparent>> will load Alice's mother and all of her grandchildren. In this case, we see a slightly more complex query,
but the essence of it remains the same as the one in <<lorina-children>>. We have separate documents and clear separation between them. In other 
...
That means that in certain cases, it will leave marks. Consider two concurrent update to the `children/alice`
For example, consider the `Child` document from our kindergarten model. Suppose two users concurrently updates to the Alice record from
<<alice-kindergarten-record>>. One transaction is updating the `Name` property to "Alice Kingsley" as well as the `Registration.Type` to
`HalfDay` while the other transaction is updating the `Birthday` property to "2022-06-01" and the `Name` to `Alice K. Liddell`.
...
If the first transaction happens to be the last one to save, the resulting document will look like like <<alice-after-concurrent-updates>>.

[[alice-after-concurrent-updates]]
.Alice record after concurrent updates
[source,json]
----
{
    "Name": "Alice Kingsley",
...
In other words, the `Name` property was modified by both transactions, and the last one to save won. The `Birthday` and the `Registration.Type`
