...
JavaScript expression and send it to the server, to be evaluated on each doucment in turn. Any matching document will be send for the client for processing.
Of course, this is just part of the work, of course, we still need to handle the subscription itself. This is done in Listing 4.36.
...
Alternatively, you can also write the code in Listing 4.36 using a `Patch` command, as you can see in Listing 4.37.
...
Subscriptions will ensure that we'll get the matching documents from the server in a reliable manner. If the RavenDB server goes down, another
node in the cluster will take over as the subscription target and your code wouldn't really notice anything. If the client went down, on 
restart, the subscription will resume from the lat acknowledged batch without a hitch. 
