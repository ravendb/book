...
`session.Load<Customer>()` method. This is translated into a server side load operation that fetch the related `Customer` docuemnt and fetch just the `Name` field from
it.^[This is possible because we are using a syncronous session and queries. If we were using async queries, we'll need to use `RavenQuery.Load<Customer>` in the Linq 
query.]
...
Another example of using listeners is to ensure that we always include a particular clause in our queries, as you can see in Listing 4.29.
