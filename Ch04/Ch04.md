...
Go into the RavenDB Studio in the browser, create a new database, as we have seen in [Zero to RavenDB](#zero-to-ravendb). Now, go into the database, and on the
...
The RavenDB Server and the RavenDB C# Client API use a dedicated binary format to represent JSON in memory. The whole of
[RavenDB's JSON Internals](#blittable) chapter is dedicted to this format, but it is worth understanding a bit about how RavenDB think 
...
We have gone over a lot of the advanced feature in the client API in this chapter. Working with attachments and understanding how we can use them to store
binary data then moving on to working with the document metadata in general. The document metadata is a convenient place to stash things about our 
documents that don't actually belong to the document itself. Auditing is one such prime example and we saw how we can implement that using listeners on
the events that the client API exposes to us.

We looked at how change tracking is managed and how we can get detailed information from the session about what exact changed in our documents and then
started to look at how we should handle concurrency in our application. We looked at optimistic concurrency in RavenDB and even implemented _pessimistic_ 
...
Another way to handle concurrency, or just to save shuffling a lot of data between client and server is to use patching. The client API offers patching 
at several level. Setting a value or incrementing a number is supported with a strongly typed API, but more complex tasks can be handled using `Defer`
and offer you the ability to write a JavaScript code that will be executed on the server to mutate your documents.
...
Bulk Insert API to efficiently push as data into RavenDB efficeintly. We saw that the major limiting factor was typically the cost of going to the 
database, and that different approaches to that could produce significantly different performance profiles. 

After looking at the various ways we could write data to RavenDB it was time to look at the other side, and we started seeing how we can optimize reads
from the server. We already gone over `Include` in [Zero to RavenDB](#zero-to-ravendb) and in this one we looked at lazy requests, allowing us to combine 
several different requests to the server into a single round trip.

The mirror image to bulk insert is the streaming feature, suitable for handling very large amount data, streaming allows us to start process the request
from the server immediately, without having to wait for the complete results. This allow us to parallelize the work between client and server and give
us the option to immediately start sending results to the user.

Following reading and writing of documents, we looked into caching them. The client API have sophisticated caching behaviors and we delved into exactly
how that work and how this reduce the time we need to provide answers to the user. Caching allows us to let the server know that we already have the 
result of a previous call to the same URL, and allow the server to let us know if that hasn't been modified. If that is the case, we don't need to send
any results on the wire and the client can use the cached (parsed and processed) data immeidately. RavenDB also supports the notion of aggresive 
caching, which allow us to skip even going to the server entirely. This is done by asking the server to let the client know when things change, and 
only going to the server then.

That option is also exposed in the client API, using the Changes API. The Changes API allow you to ask the server to let you know when a particular 
document, collection or a set of documents with a given prefix have changed. This can be very useful for letting the users know that someone has changed
the document that they are working on, implement features such as "this data have changed", etc.

Next we look at how we can tranform the results of queries and document loads on the server side using Transformers. Transformers allow you to modify
the shape of the data that RavenDB will return to the client. This can be done by just returning some subset of the data from the documents or even by
loading additional documents and merging the data from associated documents into a single result. 

We looked at cross cutting conerns on the client, how we can apply behavior through the client, such as controling how RavenDB will decide what
class belongs in what collection or how to control serialization and deserialiation. Listeners allow you to attach behavior to certain actions in 
the client API, giving you the option to customize certain behaviors. We looked at adding auditing to our application in about 3 lines of code and 
...
ask the server to create a new revision of a document on each change. Those revisions are immutable and create an audit trail and change log for the 
documents in question. While typically this is primarily a server side feature, we looked at how we can expose the revisions to the user through the client
API and allow the users to view pervious versions of a document. 

Our final topic was to cover at a high level the native serialization format that RavenDB uses, the blittable format. This format is meant to be extremely
efficent in representing JSON, it is typically stored in native memory to reduce managed memory consumption and garbage collection costs down the road. You
don't typically need to concern yourself with that, except to remember that it is important to dispose of sessions when you are done with them.

This chapter is a long one, but it is still not enough to cover the full functionality. There are a lot of features that are very useful, but they tend to
be useful in very specific and narrow circumstances or only make sense to talk about in a larger scope. We have hardly talked about queries so far, but 
we'll do so extensively when we get to [Map/Reduce](#map-reduce) and beyond and discuss indexing.

The next chapter is a good example of that, we'll dedicate that chapter to handling data subscriptions and all the myriad ways that they make data
processing tasks easier for you.