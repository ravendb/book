...
several steps. In this case, while we saved the document to RavenDB, it's still in draft status. Typical modeling advice would be to model this 
...
have something close enough to be interesting to talk about. The following is a recipe for using
...
Early source control systems (SourceSafe is a good example) used locks as their concurrency model, and that led to a lot of problems. Say Joe in accounting locks
...
> order in which the patches will run, but there isn't going to be concurrent/interleaved executing of the scripts on the same document.
...
etc. The reason for the speed up is actually related to thread pool handling on the client side. Since we make a lot of blocking requests, the thread pool figures out that
...
sequential writes. The code in Listing 4.15 is still using synchronous calls, which means the client side is spinning threads to handle the load and we're
...
when we discuss map/reduce indexes, in the ["Map/Reduce"](#map-reduce) chapter] but this is a pretty complex solution, and it isn't relevant when you have different 
...
we did them. Unfortunately, caching is _hard_. Phil Karlton [said](https://martinfowler.com/bliki/TwoHardThings.html):
...
for all the cached responses that are earlier than the last notification. When we do this, we still send the cached etag. So if that particular response hasn't 
...
For critical operations&mdash;ones where you can't afford to miss even a single change&mdash;you can use `Subscriptions`, which are covered at the end of this chapter. They're 
...
the call and the associated customer. You should also note that we have a `Result` class there, which matches the result of the output of the transformer. This will
...
definition will have no effect on the server. This is typically done as part of the document store initialization in your application. For example, assuming your
...
>
> Transformers are created on the server side, and they're always scoping to the database on which they're created. It's safe to have two different
...
as `order by`, `where`, `group by` and the like on transformers typically have no real impact on the results.

> **Transformers in a cluster**
> 
> Calling `new RecentOrdersTransformer().Execute(store)` will cause the client to send the transformer definition to one of the servers in the cluster. However,
...
for all collections, as seen in Figure 4.3.  Enable versioning by and click `Save`. 
...
In terms of cost, versioning obviously increase the amount of disk space required. But given today's disk sizes, that isn't usually a significant concern. Aside
...
Next, we looked at how we can transform the results of queries and document loads on the server side using transformers. Transformers allow you to modify
