...
* The last modifed date - stored in the `Last-Modified` metadata property^[This is actually stored twice, once as `Last-Modified` and once as `Raven-Last-Modified`, the first is following the RFC 2616 format and is only accurate to the second. The second is accurate to the millisecond.].
...
At that point, we have a cache hit, and we _could_ just immediately return the result, but that wouldn't be safe. What happened if the document has changed in the database? We would return an outdated document. That would never work. What happens is that we _make_ a request to the server, even though we already have the previous responce cached. But we send that request with an `If-None-Match` header set to the previous' request etag.
