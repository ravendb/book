...
In either case, when this flag is set and `SaveChanges` is called, we'll send the modified documents to the server alongside their etags when we
...
small change, we can avoid that cost. Another reason to avoid the full document save is because we have a scenario that calls for concurrent work
on a document.
...
In this way, whenever the server lets the client know that a change happened, the client knows to check with the server for anything in the cache older than that notification. When we do this, we still send the cached etag. So if that particular response hasn't 
...
shows a transformer that will give us just the necessary data to show to the user.
...
>
> Transformers are created on the server side, and they're always scoped to the database in which they're created. It's safe to have two different
...
of `SupportCall` but `RecentOrdersTransformer.Result`, which already includes the `CustomerName` that we want. The key part, as far as we're concerned, is the call to the 
...
Another example is to ensure that we always include a particular clause in our queries, as you can see in Listing 4.30.
