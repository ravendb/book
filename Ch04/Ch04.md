...
We'll cover that extensively in the [Map/Reduce](#map-reduce) chapter, so let's keep it there. You already know the basics of 
querying in RavenDB, but there's a _lot_ more power waiting for you to discover there.
...
brief sections that each deal with a specific feature or behavior. I suggest reading this over to note the capabilities of RavenDB 
...
writing data. For example, the user might have sent us some screenshots that we want to include in the support call. 
...
store it in the document itself, not throw it to the sidelines in the metadata. Typical use cases for storing data in the metadata are cross-cutting
concerns. The preeminent one is auditing. You may want to see who edited a document, for example. 

In order to demonstrate working with the metadata, we'll consider creating a support call. This is a complex process that has to go through
...
These operations tend to only be useful in specific cases, but they are very powerful when utilized properly. 

#### Optimistic concurrency

We covered optimistic concurrency in the chapter about [document modeling](#modeling), but only in the most general terms. Now, let's take a look and see how we can use optimistic
...
uses the `Last Write Win` model. You can control that by setting `store.Conventions.UseOptimisticConcurrency` to "true," which will affect all sessions, or you can change it 
...
RavenDB does _not_ support pessimistic locking, but while you really need support from the database engine to properly implement it, we can pretend to
have something that is near enough to be interesting to talk about, if not actually use. The following is a recipe for using
pessimistic locking in RavenDB&mdash;not so much because it's a good idea, but because it allows us to explore several different features and see how they
work together.

Using pessimistic locking, we can lock a document for modification until we release the lock or a certain time has gone by. We can build a pessimistic 
...
> behavior (and might overwrite the changes made by the thread using optimistic concurency).
>
...
There's quite a bit of code in 4.9, but there isn't actually a lot that gets done. We load a document and check if its metadata contains the
...
some kind. If you haven't, I suggest you pick up a book about source control and prioritize it over this one: learning about that is far more important.

Early source control systems (SourceSafe as a good example) used locks as their concurrency model, and that led to a lot of problems. Say Joe in accounting locks
a file and then leaves on vacation. It's a typical problem that arises in such cases. The same happens when you have pessimistic locks.
...
functions around it. It's typically easier to implement optimistic concurrency or merging, and it matches most users expectations more.
...
All you need to do is send the etag of the document to the user and accept it back when the user wants to save the document. Listing 4.10 shows an
...
Both sessions are then closed, and we open a _new_ session. We call `Store`, passing the entity instance and the etag that we got from the first session, as 
...
a web scenario, you'll typically send the etag alongside the actual data and get it back from the client to do the check. You might also want to check out the 
...
However, there are a few scenarios in which we don't want to send the entire document back to the server. If our document is very large and we want to make a 
small change, we can avoid that cost. Another reason to avoid the full document save is because we have a scenario that calls out for concurrent work
on a document.
...
this operation to the document. This patch request is safe to call concurrently, since there's no loss of data as a result of executing the patches. 
...
> documents in RavenDB is well trodden and has been plenty optimized. That means in many cases it might be easier to just load and modify the
...
> percieved performance benefits. That resulted in an extremely hard-to-understand system that was resistent to change. The general recommendation is to
...
>
> Here's another important consideration for patching. You don't need to worry about concurrency between patches on the same document. There's no guarantee as to the
...
> are multiple patches on different documents, they'll all be executed on the same transaction, as a single unit.

There are two ways to avoid the race condition. We can send an etag to the server asking it to fail with a concurrency exception if the document has been 
...
The code in Listing 4.13 passes a `PatchCommandData` to `Defer` and passes the relevant document. The key part is in the `PatchRequest` itself. We do the check on
...
concatenation here, since we can pass arguments to the scripts directly. There's also the option to run a script if the document does not exists. This gives you 
the option to do a "modify or create" style of operations.

Using this method, we can be sure that we'll never violate the rule about adding comments to a closed call. But this is more complex than anything we've dealt with
before, and I would only recommend doing this if you really must. A cautionary word, though. This is a powerful technique, but it's also open to abuse. 
...
more generic then this. It's a general mechanism for the user to register arbitrary commands that will take part of the same transaction as the `SaveChanges`. 

> **RavenDB API is like an ogre. It has layers**
>
> The RavenDB API is composed of layers. At the top, you have the document store and the document session. Those are built using the operations concept, which you
> typically won't use directly. And the operations are handled via the request executer, which allows you to generate requests directly
...
> **Stacking the deck**
>
> I'm going to be talking about a lot of performance numbers in this section, and I wanted to make it clear that I've intentionally chosen the worst possible
> situation for RavenDB and then compounded the issue by using the wrong approaches. This is so I can show off the real costs in a manner that's visible.
>
...
What's more, the database server handles just a single request concurrently, which means that we have no way to amortize I/O costs across multiple requests. This
...
between the client and the application. On each request, we have to make another REST call, send a packet to the server, etc. On the other side, the server accepts
a new request, processes it and commits it to disk. During the entire process, it's effectively idle, since most of the time is spent in waiting for I/O. That's a big waste all around.

You can see the various nicely when looking at the Fiddler^[The [Fiddler web proxy](http://www.telerik.com/fiddler) is a great debugging tool in general, and it's
very useful to peek into the communication between RavenDB server and clients.] statistics. Each request takes about 220&ndash;260 millseconds to run. Writing the first
...
etc. The reason for the speed up is actually related to thread pool handling on the client side. Since we make a lot of blocking requests, the thread pool figures out
we have plenty of blocking work and creates more threads. That means we have the chance to do more concurrent work. So as times goes by, more threads are
created and we make additional concurrent requests to RavenDB. 

The total time to write 10,000 documents in this setup was two minutes and 52 seconds. So we're just over half the time to write a tenth of this number when using 
sequential writes. The code in Listing 4.15 is still using syncronous calls, which means the client side is spinning threads to handle the load and we're
...
However, we really had to make an effort. We wrote explicit async code and managed it, rate-limited our behavior and jumped through several hoops to get a more reasonable
performance. Note that we went from over 40 minutes to less than 30 seconds in the span of a few pages. Note that we haven't actually modified _what_ we're doing&mdash;we only changed how we're talking to the server&mdash;but it had a huge impact on performance.

You can take it as a given that RavenDB is able to process as much data as you can feed it. The typical problem in handling writes is how fast we can get the 
...
would complete in about 18 seconds. The code is fairly trivial to write, similar to our first sample in Listing 4.14, but the performance is many times higher.

To compare the costs, I run the same code against a local machine, giving me a total time of 11 seconds to insert 100,000 documents (instead of two minutes 
remotely). If we want to compare apples to apples, then the cost for writing 10,000 documents is shown in Table. 4.1.
...
You can see that, while bulk insert is significantly faster in all cases, being over three times faster than the session option (Listing 4.14) locally loses 
its meaning when you consider that it's over 130 times faster in the remote case. The major difference, as you can imagine, is the cost of going over the network, 
...
The way bulk insert works is by opening a TCP connection to the server and writing the raw data directly into the database. That means we don't need to go 
...
and write it to disk when it's best to do so. In other words, bulk inserts are _not_ transactional. A bulk insert is actually comopsed of many smaller transactions,
...
to know? We already know how to load and query a document; we covered that in ["Zero to RavenDB"](#zero-to-ravendb). We also covered `Include` and how
...
development enviornment. When you go to production, your database is typically going to run on a dedicated machine,^[In fact, it's likely that a database cluster will be used on a set of machines.] so you'll have to go over the network to get it. And that dramatically increases the cost of going to the database.
...
scenario. We can define a map/reduce index and run a query and `Include` on it. We haven't yet gone over exactly what this means,^[We'll cover this technique
when we'll discuss map/reduce indexes, in the [map/reduce](#map-reduce) chapter] but this is a pretty complex solution, and it isn't relevant when you have different 
types of queries. If we wanted to also load the logged in user, for example, that wouldn't work.

The solution RavenDB has for this issue is the notion of lazy requests. A lazy request isn't actually being executed when you make it. Instead, it's 
...
by accessing the `Value` property on any of the lazy instances we got.

> **Why do we need ExecuteAllPendingLazyOperations?**
>
> The existence of ExecuteAllPendingLazyOperations is strange. It's explicitly doing something that will happen implicitly anyway, so why is this needed?
> This method here is to allow users to have fine-grained control over the execution of requests. In particular, it allows you  to set up a stage in your pipeline
...
versions of RavenDB, we had guards in place to prevent this scenario entirely. It's easy to start writing code like the one in Listing 4.20 and
over time have more and more results come in. Our logic was that, at some point, there needed to be a cutoff point and throw before this kind of behavior poisoned your application.

As it turns out, our users _really_ didn't like this behavior. In many cases, they would rather the application do more work (typically uneccesarily) than to have it
...
the application can all work in parallel with one another to process the results of the query. This technique is suitable for processing very large number of 
...
* The results of a streaming query are _not_ tracked by the session. Changes made to them will not be sent to the server when `SaveChanges` is called. This is
  because we expect streaming queries to have a high number of results, and we don't want to hold all the references for them in the session. Otherwise,
...
we did them. Unforutnately, caching is _hard_. Phil Karlton [said](https://martinfowler.com/bliki/TwoHardThings.html):
...
from the cache, the results can range from nothing much to criticial, depending on what exactly you're doing. 
...
When the server gets a request with an etag, it has a dedicated code path, optimized specifically for that, to check whatever the results of the operation have 
changed. If they didn't change, the server can return to the client immediately, letting it know that it's safe to use the cached copy. In this manner, we save computation costs on the server and network transfer cost between the server and the client. 
...
>
> Caching by default can be a problem with a particular set of queries&mdash;those that use the notion of the current time. Consider the case of the follwing query:
>
> ```
...
changed. If they didn't, we return back a `304 Not Modified` response. The client will then just use the cached response that it already has.
...
For a heavily used page, showing data that might be stale for a few minutes is absolutely fine, and the performance benefits gained from not having to go to the server 
can be quite nice. In order to support this scenario, RavenDB supports the notion of aggressive caching. You can see an example of that in Listing 4.23.
...
up to date. Instead, it will immediately serve the request from the cache, skipping all network traffic. This can significantly speed up operations in which you
can handle a certain period of stale view of the world. 

However, just having this mode will be pretty bad if you'll never see new results. This leads us back to the problem of cache invalidation. Aggressive caching 
...
> local to that server and don't make sense globally.
...
possibly updated value. Take note of that, because you might be caching a lot of different queries, documents, etc. The client isn't asking specifically for changes it has in its cache. Instead, it's asking for all changes in the server.

In this way, whenever the server lets the client know that a change happened, we'll force a check with the server the next time a cached response will be used
...
> the requirement that aggressive caching must be turned on explicitly, you're aware that there's a period of time where the response you received might have
...
if something might have changed, we'll check the server for the most up-to-date response. Given typical behavioral patterns for the application, we'll often be able to use the aggresive cache for quite some time before a write will come in and make us check with the server.
...
ask it to let us know when a particular event has happened. Listing 4.24 shows how we can ask the server to tell us when a particular document have changed.
...
For criticial operations&mdash;ones where you can't afford to miss even a single change&mdash;you can use `Subscriptions`, which are covered at the end of this chapter. They're 
...
to the application. If those documents are large, that can be expensive. Another way to handle this is by letting the server know exactly what we need to returned
and then letting it do the work on the server side. 

This is the role of transformers. A transformer is a bit of code that's going to run on the server and transform the result that we'll get back. Listing 4.26 
...
The transformer is going over the list of calls it was provided and loading the associated customer. Then it projects out a new object, with the properties from both
...
It's important to understand that the `RecentOrdersTransformer` in Listing 4.26 isn't the actual transformer; it's the transformer _definition_. It's a way
...
Once that line is executed, we know that the transformer has been properly sent to the server. Multiple calls to this method  without changing the transformer 
defintion will have no effect on the server. This is typically done as part of the document store initialization in your application. For example, assuming your
application uses the approach outlined in Chapter 2, Listing 2.10, you'll add this line as the last thing in the `CreateDocumentStore` method.

> **Transformers and databases**
>
> Transformers are created on the server side, and they're always scoping to the database on which they are created. It's safe to have two different
> transformers with the same name on two distinct databases, although that might be confusing for the consumer. 
>
> Once created, transformers exist on their database and are available for use. Unless an operation asks for a transformer to be applied, a transformer has 
> no impact on the server.

After we create the transformer, we can now use it, as you can see in Listing 4.27.
...
of `SupportCall`s but `RecentOrdersTransformer.Result`, which already includes the `CustomerName` that we want. The key part, as far as we're concerned, is the call to the 
`TransformWith` method. This lets RavenDB know we want to apply the `RecentOrdersTransformer` transformer on the server side to the result of the query. 

In terms of order of operations, this call should typically be last. That's not a requirement, but given that this is the order of operations on the server, it 
allows us to have a more intuitive understanding of what's going on.^[Changing the order of the calls on the client side will _not_ change the order of operations
on the server side, to be clear.] The transformer is applied on the query after the query has been filtered, sorted and had paging applied to it. Clauses such
...
The last generic parameter of `TransformWith` is `RecentOrdersTransformer.Result`, which is the type the client will use to deserialize the results from the 
server. This is a pure convention of having an inner class named `Result` with the same output as the transformer. It isn't required. But having the `Result` class
as an inner class inside the transformer makes it clear that they're closely tied and typically changed together.

As you saw, a transformer can do quite a lot. It can modify the output of the query, it can load additional documents, it can perform computation, etc. Transformers can also be 
...
It's common to use transformers in high value pages to get just the right information from the server without having to send full documents over the wire. They're a powerful tool to have handy. 

The output of the transformer is not a document, and as a result of that, it isn't tracked by the session. That means changes to the result of a transformer
won't be saved back to the server when `SaveChanges` is called. You _can_ call `Store` on the result of a transformer, but be aware that this will create a 
new document in a separate collection. There's usually little reason to want to do that.
...
work within at any given point in time. There is quite a lot that you can configure and customize in the client behavior. The most common customization is to 
...
If you need fine-grained control over the serialization and deserialization of your documents, this is the place to look. And it holds important configurations, 
such as the maximum number of request to allow per server or whether we should allow queries without a `Take` clause. Listing 4.28 shows an example of controlling
...
In Listing 4.28, we're letting RavenDB know that `Customer` or any derived type should be in the "customers" collection. That means that we can create a class 
...
Alongside conventions, which are typically reactive, you can use listeners to perform various operations during the execution of requests by RavenDB. The 
listeners are available as events that can be subscribed to at the document store level or the individual session level. 
...
This allows us to register to be called whenever a particular event happens, and that in turn gives us a lot of power. Listing 4.29 shows how we can implement
...
In the previous section, we briefly mentioned auditing. In this section, we're going to take notion of auditing and dial it up a notch, just to see what will 
...
With RavenDB, this kind of system is much easier. That's because RavenDB has built-in support for versioning. Allow me to walk you through setting up such a system.
...
left side menu, click `Settings` and then `Versioning`. 

You can configure versioning globally for all collections or a single particular collection. For the purpose of this exercise, we'll define versioning
for all collections, as see in Figure 4.3.  Enable versioning by and click `Save`. 

![Setting up versioning for all documents](./Ch04/img03.png)

Now that we've enabled versioning, let's see what this means. Go ahead and create a simple customer document, as seen in Figure 4.4.

![Versioned customer document](./Ch04/img04.png)

You can see that this document has a `@flags` metadata property that is set to `Versioned`. And if you look at the right-hand side, you'll see a revisions
...
> resolution. We'll cover this behavior in depth in ["Your First RavenDB Cluster"](#clustering-intro).

As part of configuring versioning on the database, you can select how many revisions we'll retain and for what period of time. For example, you may choose to
keep around 15 revisions for seven days. Under those conditions, we'll delete all revisions that are _both_ older than seven days and have more than 15 revisions 
...
when it's over seven days old.

From the client side, you don't really need to consider revisions at all. This behavior is happening purely on the server side and requires no involvement from
...
The code in Listing 4.31 will fetch the most recent revisions and provide you with the revisions of the 
...
I've found that some applications, without the regulations requiring versioning, can make use of revisions just because they give the users an easy way to look at
...
feature was very valuable to her since it allowed here to see exactly what happened and who took the action. (The application utilized a listener to implement audits, as shown
...
The RavenDB server and the RavenDB C# client API use a dedicated binary format to represent JSON in memory. The whole of the
["RavenDB's JSON Internals"](#blittable) chapter is dedicated to this format, but it's worth understanding a bit about how RavenDB thinks 
...
The C# client is also using blittable format internally since that greatly helps with memory consumption and control. You generally won't see
...
On the client side, using the blittable format means we have to deal with reduced memory consumption and reduced fragmentation. It also reduces
...
session, or we can send the etag value to the client and get it back on the next request, thus implementing offline optimistic concurrency.
...
We also looked at various way to get a lot of data into RavenDB, from a sequential `SaveChanges` per document to running them in parallel to using the
bulk insert API to efficiently push as data into RavenDB efficiently. We saw that the major limiting factor was typically the cost of going to the 
...
any results on the wire, and the client can use the cached (parsed and processed) data immediately. RavenDB also supports the notion of aggresive 
...
That option is also exposed in the client API, using the changes API. The changes API gives you the ability to ask the server tell you when a particular 
...
Next, we looked at how we can tranform the results of queries and document loads on the server side using transformers. Transformers allow you to modify
...
We looked at cross-cutting concerns on the client and how we can apply behavior through the client, such as controlling how RavenDB decides what
class belongs in what collection, as well as serialization and deserialization. Listeners allow you to attach behavior to certain actions in 
...
Following cross-cutting behaviors, we moved to look at the versioning feature in RavenDB and how to use it from the client side. The versioning feature
...
efficent in representing JSON. It's typically stored in native memory to reduce managed memory consumption and garbage collection costs down the road. You
...
be useful in specific, narrow circumstances or only make sense to talk about in a larger scope. We've hardly talked about queries so far, but 
we'll do so extensively when we get to ["Map/Reduce"](#map-reduce) and, beyond that, when we discuss indexing.
