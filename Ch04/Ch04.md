...
We'll cover that extensively in Chapter 10, so let's keep it there. You already know the basics of 
...
when we discuss MapReduce indexes in Chapter 12.] but this is a pretty complex solution, and it isn't relevant when you have different 
...
> resolution. We'll cover this behavior in depth in Chapter 7.
...
when replicating changes in the cluster^[See "Transaction atomicity and replication" in Chapter 7.], handling historical subscriptions^[See "Versioned Subscriptions" in the next chapter.] and ETL processes.
...
we'll do so extensively when we get to Chapter 12 and when we discuss indexing.

In the next chapter, we are going to look into a powerful set of features, how you can extend RavenDB's documents with counters and time series data.