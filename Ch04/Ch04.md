...
We'll cover that extensively in the ["Glorious Indexing"](#advanced-indexing) chapter, so let's keep it there. You already know the basics of 
...
In either case, when this flag is set and `SaveChanges` is called, we'll send the modified documents to the server alongside their change vectors when we
...
Both sessions are then closed, and we open a _new_ session. We call `Store`, passing the entity instance and the change vector that we got from the first session, as 
...
>
> For the most parts, the etags generated for HTTP requests among servers should be identical for identical data, since it uses the change vectors of the documents
> to compute it. However, different servers may recieve documents in different order, which may result in difference in the actual results. That shouldn't impact
> the behavior of the system, but led us to skip doing cross node caching for now. 
...
Another example of using listeners is to ensure that we always include a particular clause in our queries, as you can see in Listing 4.30.
...
keep around 15 revisions for seven days. Under those conditions, RavenDB'll delete all revisions that are _both_ older than seven days and have more than 15 revisions 
...
In terms of cost, revisions obviously increases the amount of disk space required. But given today's disk sizes, that isn't usually a significant concern. Aside
...
when replicating changes in the cluster^[See "Transaction atomicity and replication" in ["Clustering Setup"]:()clustering-setup) chapter.], handling historical subscriptions^[See "Versioned Subscriptions" in the next chapter.] and ETL processes.
